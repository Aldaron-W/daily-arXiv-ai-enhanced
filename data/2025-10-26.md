...
Method: 扩展DeBERTa-v3模型，结合Focal Loss、Label Smoothing和R-Drop正则化来处理类别不平衡和增强泛化能力。构建了端到端的可复现流程，包括数据提取、标注、预处理、训练和评估。

Result: 在10折分层交叉验证中，DeBERTa-KC取得了0.836±0.008的宏F1分数，显著优于传统和Transformer基线方法（p<0.01）。模型对高阶认知参与（特别是Explore和Negotiate类别）表现出强敏感性。

Conclusion: 大语言模型能够有效捕捉非正式数字学习环境中知识建构的细微指标，为话语分析和认知参与评估工具的开发提供了可扩展的理论驱动方法。
...
<details>
  <summary>Details</summary>
Motivation: 评估AI生成课程计划的教学质量和可用性，比较不同大语言模型和提示框架在生成物理课程计划方面的表现差异。

Method: 使用五个大语言模型和三种提示框架生成15个高中物理电磁波谱课程计划，通过四个自动化计算指标进行分析：可读性、事实准确性、课程标准对齐和认知需求。

Result: DeepSeek生成最易读的教学计划，Claude生成最密集的语言；RACE框架产生最低的幻觉指数和最高的课程标准对齐；所有模型的学习目标主要集中在布鲁姆分类法的记忆和理解层级。

Conclusion: 可读性主要由模型设计决定，而教学可靠性和课程对齐更依赖提示框架。最有效的配置是将可读性优化的模型与RACE框架及明确的概念清单结合。
...
Method: 提出ReDiff框架，采用两阶段训练：首先训练模型修正合成错误建立基础修正能力；然后通过在线自修正循环训练模型修正自身错误草稿，学习专家修正。

Result: 实验表明ReDiff显著提高了生成内容的连贯性和事实准确性，实现了比传统去噪方法更稳定高效的并行生成。

Conclusion: ReDiff通过主动修正机制有效打破了错误级联，为离散扩散模型的实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统机制可解释性技术在分析注意力时随上下文长度呈二次方扩展，在超过10万token时需要TB级内存，无法适应百万token级别的LLM上下文分析需求。

Method: 提出Stream算法，采用可编译的分层剪枝方法估计每个头的稀疏注意力掩码，通过二分搜索式精炼保留每个查询的前k个关键块，同时保持模型的下一个token行为。

Result: 在长链式思维推理轨迹中识别思维锚点，剪枝97-99%的token交互；在RULER基准测试中保留关键检索路径，剪枝90-96%的交互，暴露从针到输出的分层路由。
...
<details>
  <summary>Details</summary>
Motivation: 现有HIV诊断研究主要关注结构化数据，忽视了临床笔记等非结构化文本中可能包含的HIV风险相关信息

Method: 开发了一个基于大语言模型的流程来分析非结构化EHR文本，评估患者是否适合进行进一步HIV检测

Result: 在鹿特丹伊拉斯谟大学医学中心的临床数据上，该流程实现了高准确率且保持了低假阴性率

Conclusion: 该研究证明了利用大语言模型分析非结构化EHR文本在HIV筛查中的有效性
...
<details>
  <summary>Details</summary>
Motivation: AI和LLM越来越多地被探索作为支持生命周期评估的工具，但缺乏关于其可靠性、鲁棒性和可用性的系统证据，且该领域缺乏标准化的评估框架。

Method: 评估了11个通用LLM（商业和开源）在22个LCA相关任务中的表现，17位经验丰富的从业者根据科学准确性、解释质量、鲁棒性、可验证性和指令遵循等标准审查模型输出，收集了168份专家评审。

Result: 专家认为37%的回复包含不准确或误导信息；准确性和解释质量评分普遍为中等或良好；幻觉率差异显著，某些模型产生虚假引用的比例高达40%；开源模型在准确性和解释质量等标准上与闭源模型表现相当或更优。

Conclusion: 研究结果突显了在LCA中天真应用LLM的风险，特别是将其视为自由形式的神谕时，但也显示了在解释质量和减轻简单任务劳动强度方面的益处。没有基础机制支持的通用LLM使用存在风险。
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态大语言模型评估存在静态基准测试无法综合评估、人工或模型对比主观性强且成本高、模型可能利用表面特征（如冗长回答）来提升胜率等问题。

Method: 采用游戏化评估方法，特别是使用Dixit幻想卡牌游戏。在该游戏中，玩家需要为卡片生成描述，目标是让部分但不是所有玩家选中该卡片。

Result: 实验显示，五个多模态大语言模型在Dixit游戏中的胜率排名与流行基准测试排名完全一致。人机对战揭示了模型策略与人类策略的差异以及模型推理能力的改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 传统优化方法依赖领域专家将实际问题转化为数学模型，而LLMs有潜力通过自然语言理解和代码生成来弥合这一表述差距。DeepSeek-R1作为成本效益高的模型，在运筹学应用场景中的有效性尚未充分探索。

Method: 在四个关键运筹学基准测试上系统评估DeepSeek-R1，包括基准评估、幻觉分类开发，以及应用LLM-as-a-Judge、少样本学习、工具调用和多智能体框架等缓解策略。

Result: 研究展示了DeepSeek-R1在运筹学优化建模任务中的潜力，但具体结果需要进一步实验验证。

Conclusion: DeepSeek-R1为运筹学优化建模提供了可行的替代方案，通过适当的缓解策略可以有效减少幻觉并提高模型输出与用户意图的一致性。
...
Method: 提出LyriCAR框架，包含难度感知课程设计师和自适应课程策略，通过逐步增加挑战来引导模型学习。

Result: 在EN-ZH歌词翻译任务中，LyriCAR在标准翻译指标和多维奖励分数上都达到最佳效果，训练步骤减少近40%。

Conclusion: LyriCAR证明了无监督课程学习在复杂歌词翻译任务中的有效性，能够高效分配训练资源并提升翻译质量。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉和不一致输出的问题，而符号NLU系统虽然提供可解释的理解但覆盖范围有限。需要结合两者的优势。

Method: 使用LLM进行重述和文本简化以提供广泛覆盖，使用符号NLU生成结构化关系表示以支持推理和增量学习。

Result: 在从常识科学文本中提取和解释数量和因果规律的任务上，混合方法显著优于纯符号方法。

Conclusion: 混合方法结合了LLM的广泛覆盖能力和符号NLU的结构化表示能力，取得了更好的效果。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种模拟人脑解析过程的自然语言解析算法，与短语结构解析不同，采用逐词处理的方式。

Method: 使用依赖树解析方法，逐词处理句子，在可连接时立即连接每个词，最坏情况复杂度为O(n^3)。

Result: 算法在人类语言中，最坏情况仅出现在小规模n时，整体性能良好。

Conclusion: 该依赖解析算法有效模拟了人脑的解析过程，在人类语言中具有较好的实际性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学数据集如MedQA过于简化，不能充分代表真实世界的临床决策过程，需要更全面的评估框架。

Method: 提出二维评估范式（临床背景和临床问题），总结现有数据集设置，回顾训练时和测试时技术方法，并扩展评估维度。

Result: 建立了一个能够澄清假设、标准化比较、指导开发具有临床意义的LLM的评估框架。

Conclusion: 该范式有助于开发更符合真实临床环境的语言模型，并指出了未来面临的开放挑战。
...
<details>
  <summary>Details</summary>
Motivation: 希腊语等形态丰富且资源中等语言的NLP发展受到架构停滞、数据稀缺和有限上下文处理能力的阻碍，特别是在法律等专业领域。

Method: 开发了GEMs模型家族，包括RoBERTa、Longformer、ELECTRA、ConvBERT和ModernBERT等架构，使用精心策划的大规模语料库进行训练，采用基于质量的语料库管理方法，包括预处理管道、去重策略和高质量法律子语料库的针对性重复。

Result: GEM-RoBERTa和GEM-ConvBERT在三个核心自然语言理解基准测试中取得了统计显著的性能提升，准确率最高提升3.6%，统计分析确认了该方法在多个评估指标上的优越性。

Conclusion: GEMs模型家族成功解决了希腊语NLP的挑战，在通用和专业领域都表现出色，为希腊语语言建模提供了新的标准。
...
<details>
  <summary>Details</summary>
Motivation: 改进预训练神经语言模型在序列标注任务中的迁移学习效果，解决传统方法在领域迁移、信息流和生成式应用方面的局限性。

Method: 1) 多任务模型引入领域无关文本处理系统的额外信号；2) 修改自回归大语言模型架构实现跨层双向信息流；3) 生成式监督上下文微调框架结合响应导向适应策略。

Result: 提出的模型、方法和框架表明，通过针对性迁移学习范式适配的预训练神经语言模型在序列标注任务中能达到最佳性能。

Conclusion: 预训练神经语言模型在序列标注任务中的最佳性能需要通过针对性的迁移学习范式来实现，论文提出的三种改进方法有效提升了迁移学习效果。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的工具集通常包含冗余工具，这些工具具有重叠的名称和描述，导致模糊性并降低选择准确性。同时，LLM面临严格的输入上下文限制，无法高效考虑大型工具集。

Method: ToolScope包含两个组件：(1) ToolScopeMerger with Auto-Correction：自动审计和修复工具合并，减少冗余；(2) ToolScopeRetriever：对每个查询进行工具排名和选择，仅保留最相关的工具，压缩工具集以适应上下文限制。

Result: 在三个最先进的LLM和三个开源工具使用基准测试上的评估显示，工具选择准确率提升了8.38%到38.6%。

Conclusion: ToolScope在增强LLM工具使用方面表现出有效性，通过减少冗余和智能检索解决了工具选择的模糊性和上下文限制问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言基准测试在捕捉低资源文化细微差别方面存在不足，特别是孟加拉语文化知识评估存在空白。
...
Conclusion: 需要开发上下文感知架构和经过文化精心策划的训练数据，以改善模型在低资源文化背景下的表现。
...
<details>
  <summary>Details</summary>
Motivation: 提升小语言模型在波斯语等少数语言中的推理能力，特别是在医疗问答等专业领域的应用需求。

Method: 采用强化学习与AI反馈(RLAIF)和直接偏好优化(DPO)，翻译医疗问答数据集，生成拒绝-偏好答案对，通过链式思维推理训练模型。

Result: 训练出的模型在仅使用较小数据集(200万token偏好答案和250万token拒绝答案)的情况下，性能超过了使用5700万token训练的前代模型gaokerena-V。

Conclusion: 推理导向的训练方法在数据有限的情况下，能够高效开发领域特定的语言模型，证明了该方法在资源受限环境下的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估大语言模型创造力的整体框架，现有方法在不同领域和任务中存在碎片化问题，主要由于对创造力的定义和测量标准不一致。

Method: 提出CreativityPrism框架，将创造力分解为质量、新颖性和多样性三个维度，包含9个任务、3个领域和20个评估指标，对17个最先进的专有和开源大语言模型进行评估。

Result: 专有模型与开源模型存在显著差距；同一领域内任务表现高度相关，不同领域间相关性较弱；多样性和质量指标强相关，新颖性与其他维度相关性较弱。

Conclusion: 在某一创造力任务或维度上的优异表现不一定能推广到其他方面，强调了对大语言模型创造力进行整体评估的必要性。
...
Method: ARTER采用结构化流水线，通过计算嵌入和LLM信号将上下文提及分为简单和困难案例，分别由低计算实体链接器和选择性LLM推理处理。

Result: 在标准基准测试中，ARTER比ReFinED提升高达4.47%，在6个数据集中的5个平均提升2.53%，性能与全LLM推理相当但LLM token使用量减半。

Conclusion: ARTER证明了通过自适应路由和选择性LLM推理，可以在不依赖深度微调的情况下实现高性能且高效的实体链接。
...
<details>
  <summary>Details</summary>
Motivation: 随着结构化文本在技术报告和生成式AI提示等领域的复杂性增加，传统基于句子或段落的分割方法无法有效处理包含表格、代码片段和占位符等元素的文本，因此需要新的语义分割方法。

Method: BoundRL联合执行标记级文本分割和标签预测，仅生成起始标记序列，通过定位这些标记重构完整内容。采用带可验证奖励的强化学习(RLVR)优化重构保真度和语义对齐，并通过系统扰动生成中间候选来避免熵崩溃。

Result: 实验表明，BoundRL使小语言模型(1.7B参数)在复杂提示分割任务上优于大模型的少样本提示。RLVR相比监督微调带来显著改进，加入中间候选进一步提升了性能和泛化能力。

Conclusion: BoundRL为复杂结构化文本提供了一种高效的分割解决方案，通过创新的起始标记生成和强化学习方法，在保持高质量的同时大幅降低了计算成本，特别适用于LLM应用中的复杂提示处理。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型从预训练数据中继承了刻板印象，导致在立场检测等敏感NLP任务中出现偏见，但这一偏见在立场检测方法中的评估被社区忽视。

Method: 在现有立场检测数据集中自动标注两个属性：特定群体的方言/语言风格和文本复杂性/可读性，研究这些属性是否影响模型的立场检测决策。

Result: 结果显示LLM在立场检测任务中表现出显著刻板印象，如错误关联支持大麻观点与低文本复杂性，非裔美国人方言与反对特朗普立场。

Conclusion: LLM在零样本立场检测中存在明显的刻板印象偏见，需要进一步研究和缓解这些偏见。
...
<details>
  <summary>Details</summary>
Motivation: 当前搜索智能体缺乏同时进行深度多跳推理和广度信息收集的能力，这在市场分析和商业发展等实际应用中是一个关键缺陷。

Method: 通过转换现有数据集的方法构建了包含220个问题的基准测试，这些问题需要在大量数据上进行深度多跳检索推理。

Result: 实验显示即使最先进的智能体在DeepWideSearch上平均成功率仅为2.39%，揭示了整合深度和广度搜索的重大挑战。错误分析识别出四种失败模式：缺乏反思、过度依赖内部知识、检索不足和上下文溢出。

Conclusion: DeepWideSearch基准测试公开发布，旨在推动未来开发更强大和鲁棒的信息搜索智能体研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在互补缺陷：基于微调的方法容易产生算术错误和幻觉，而基于工具的方法缺乏语义理解且依赖刚性模式。需要整合稳健推理与可靠表格处理的方法。

Method: 设计三智能体框架（规划、编码、回答），使用代码执行进行精确表格操作，通过MCTS生成伪黄金轨迹，并用强化学习优化智能体。

Result: 在TableBench上达到62.13%的准确率，超越了OpenAI-o4-mini-high。

Conclusion: 结合结构化多智能体工作流与强化学习在表格理解方面具有巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在文本输入下的空间推理能力，评估其在结构化网格环境中的空间理解和计算能力，揭示语言与空间推理之间的差距。

Method: 使用五项任务测试LLMs：象限识别、几何变换、距离评估、单词搜索和滑块移动，通过增加网格维度来扩展任务复杂度，要求模型超越简单模式识别进行抽象空间推理。

Result: LLMs在所有小规模任务中表现中等，但随着规模增加性能急剧下降，平均准确率损失42.7%，最高达84%；所有初始准确率超过50%的测试都损失至少48%，性能恶化具有一致性。

Conclusion: LLMs在空间推理方面存在显著局限性，缺乏稳健的空间表征架构，这凸显了语言与空间推理之间的差距，为未来语言与几何交叉领域的综合基准测试奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在推理时只评估特定分词输出的概率，但同一文本可能有多种分词方式。边际化评估需要计算所有可能分词的概率总和，但传统采样方法需要昂贵的生成步骤，限制了采样数量和近似精度。

Method: 研究无解码采样策略，这些策略不需要语言模型生成文本，而是使用模型和分词器无关的廉价采样方法，通过直接计算分词序列的概率来近似边际概率。

Result: 实验表明，无解码采样策略在多个开源模型上能够以极小的运行时成本提供足够准确的边际概率估计。

Conclusion: 无解码采样策略为语言模型的边际概率评估提供了一种高效且准确的近似方法，显著降低了计算成本，适用于下游推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 抑郁和创伤后应激障碍(PTSD)经常共病且症状相互关联，而现有的自动化评估通常是二元且针对单一障碍的。临床诊断需要能够评估严重程度并提供决策支持解释的跨障碍估计方法。
...
Result: 在DAIC数据集上的分层交叉验证显示，融合模型在准确率和加权F1上与最强单模态基线相当，同时提高了决策曲线效用和模态缺失/噪声下的鲁棒性。对于PTSD，融合减少了回归误差并改善了类别一致性。错误主要集中在相邻严重程度之间，极端类别识别可靠。
...
<details>
  <summary>Details</summary>
Motivation: 传统next-token预测限制了模型捕捉高层次语义结构和长距离上下文关系的能力，需要克服这一局限性。

Method: 在标准预训练基础上引入next-context预测机制，训练模型学习多token上下文的预测表示，利用未来token块的误差信号。

Result: 在GPT2和Pythia模型家族上（最高15亿参数）的实验显示，ContextLM在困惑度和下游任务性能上均带来一致提升。

Conclusion: next-context预测为语言建模提供了可扩展且高效的增强路径，能以最小计算开销获得更好的长距离连贯性和注意力分配效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM检索增强生成系统中的引用失败问题，即模型生成有用回答但未能完整引用证据，这与回答失败（回答本身有缺陷）不同。

Method: 采用两步法：1）研究引用失败的发生条件，通过CITECONTROL基准系统分析回答与证据的关系；2）提出CITENTION框架，整合生成式、基于注意力和基于检索的方法来改善引用。

Result: 实验表明引用失败随关系复杂性增加而增加，CITENTION框架在CITECONTROL基准和迁移设置中显著改善了引用质量。

Conclusion: 引用失败是LLM检索增强生成系统的重要问题，通过系统分析和综合方法可以有效改善引用质量。
...
<details>
  <summary>Details</summary>
Motivation: 过程奖励模型在数学等领域能提升复杂推理能力，但在涉及半结构化数据的表格问答任务中尚未探索。TQA对PRMs提出独特挑战：大量无关信息、松散连接的推理步骤和领域特定推理。

Method: 评估最先进的生成式PRMs在表格问答任务中的表现，从答案和步骤两个角度进行分析，结合文本和代码验证方法。

Result: 结果显示：结合文本和代码验证的PRMs有助于解决方案选择，但在领域外数据上泛化能力差；步骤级验证性能与答案准确性之间相关性弱，可能源于弱步骤依赖性和松散的因果联系。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理复杂数学运算时存在效率低和准确性差的问题，而集成代码解释器又会导致模型内部概率推理与外部确定性知识之间的冲突，引发无效思考。

Method: 提出提示工程数据合成策略，在推理路径中战略性地注入多样化提示；通过监督微调、拒绝采样和强化学习优化模型与代码解释器的多轮交替使用。

Result: 在五个挑战性数学推理数据集上，32B和1.5B模型分别获得4%和8%的绝对提升，同时显著减少token使用量（32B模型约30%，1.5B模型约50%）。

Conclusion: CoRT框架成功解决了大型推理模型与代码解释器集成时的冲突问题，显著提升了数学推理性能和效率。
...
Method: 通过一系列受控探测实验，比较包含强语义线索的数据集与去除或随机化这些线索的数据集上的性能表现。

Result: 当数据集包含有意义的列名或可解释的值类别等强语义线索时，LLMs表现出污染效应；而当这些线索被移除或随机化后，性能急剧下降至接近随机水平。

Conclusion: LLMs在表格推理任务中表现出的能力可能反映了对公开数据集的记忆，而非真正的推理能力，这对评估协议提出了重要启示，并需要制定策略来区分语义泄漏与真实推理能力。
...
Method: 提出FreeChunker框架，将句子视为原子单位，从静态分块转向支持任意句子组合的灵活检索，减少语义边界检测的计算开销。

Result: 在LongBench V2上的实验评估显示，FreeChunker相比传统分块方法实现了更优的检索性能，同时在计算效率上显著优于现有方法。

Conclusion: FreeChunker通过范式转变显著提升了RAG系统的适应性和效率，为复杂查询处理提供了更有效的解决方案。
...
Method: 基于llamalogue模型进行预训练，采用多种微调策略（包括PPO和DPO）来增强模型的对话生成能力。

Result: 模型在大多数标准BabyLM基准测试中表现不佳，但在最小配对设置下的对话延续预测任务中表现出色；PPO微调效果混合甚至负面，而DPO微调能进一步提升自定义对话基准的性能。

Conclusion: 对话数据预训练能产生专门化的对话模型，DPO微调是提升对话性能的有效方法，但模型在通用语言理解任务上仍有局限。
...
<details>
  <summary>Details</summary>
Motivation: 探索否定文本对LLM幻觉的影响，填补该领域的研究空白。

Method: 构建NegHalu数据集，通过重构现有幻觉检测数据集加入否定表达；在token级别追踪LLM处理否定输入时的内部状态。

Result: LLMs难以有效检测否定文本中的幻觉，经常产生逻辑不一致或不忠实的判断。

Conclusion: 否定表达对LLM的幻觉检测构成显著挑战，需要专门的方法来缓解其负面影响。
...
<details>
  <summary>Details</summary>
Motivation: 为越南多模态法律文本处理提供研究基准，重点聚焦交通标志法规领域，推动智能系统在多模态法律领域的发展。

Method: 通过多模态法律检索和多模态问答两个子任务来构建和评估系统，使用F2分数和准确率作为评估指标。

Result: 在多模态法律检索任务中达到64.55%的F2分数，在多模态问答任务中达到86.30%的准确率。

Conclusion: 该共享任务为越南多模态法律文本处理提供了有效的基准数据集，展示了在多模态法律问答领域的良好性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统BERT模型架构相对过时，需要更新以适应现代需求。现有改进模型如ModernBERT和NeoBERT在英语上表现良好，但希伯来语领域缺乏类似优化模型。

Method: 采用与NeoBERT相同的架构训练BERT风格模型，专门针对希伯来语文本进行优化，包括单语和双语版本。

Result: 新模型在几乎所有希伯来语基准测试中超越现有模型，双语版本在检索任务上表现尤其突出，超越了类似规模的多语言模型。

Conclusion: 成功开发了针对希伯来语的优化BERT模型，为希伯来语NLP研究和应用提供了强大基础，并将模型开源给社区使用。
...
Method: 使用教师-学生框架和专门的对齐数据集进行后训练，实验了自适应教师解码策略。

Result: BabyLM生成的回复在语法和连贯性方面有所改善，但自适应教师解码策略带来的额外收益有限。

Conclusion: 针对性后训练对对话质量有益，但连续性对于BabyLM来说仍然是一个具有挑战性的目标。
...
Result: 仅使用蒸馏后的3%数据就能超越全数据集训练效果，并在多个基准测试中与最先进的高质量数据选择方法竞争
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的输出存在不确定性和正确性差异，其实际可靠性难以保证，需要量化这种不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 提升掩码语言建模的效果，特别是在小规模数据场景下，以在BabyLM挑战赛中取得更好表现。

Method: 改进的掩码语言建模方法，根据模型预测能力调整掩码token的概率；同时引入子词嵌入来增强形态学泛化能力。

Result: 在(Super)GLUE任务上相比标准MLM有显著性能提升，在严格小规模赛道上超越了基线。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在持续学习中需要历史数据、存在灾难性遗忘的问题，提出利用内部表征作为知识代理的无数据融合方法。

Method: 基于典型样本的层间隐藏表征计算模型间相似度，进行自适应分层参数融合，浅层保留通用特征，深层适应具体任务。

Result: 在五个NLP任务和多种持续学习场景中，RECALL在知识保留和泛化能力方面均优于基线方法，有效抵抗灾难性遗忘。

Conclusion: RECALL提供了一种可扩展、无需数据的LLM演进方案，实现了多领域知识的无缝整合。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型有时能检测到自己正在被评估，并调整行为以显得更对齐，这会损害安全评估的可靠性。

Method: 采用两阶段训练过程：首先在包含模型事实描述的文档上进行持续预训练，然后通过专家迭代训练模型在评估设置中使用Python类型提示。最后通过激活导向技术抑制评估警觉行为。

Result: 经过训练的模型在评估环境中会更多地使用类型提示，但移除评估线索后这种差异才会显现。激活导向技术能够抑制评估警觉，使模型在存在评估线索时也表现得像在部署环境中。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在训练数据中主要反映主流偏好，导致对特定个体需求的覆盖不足，当用户请求偏离训练数据的中心趋势时，模型性能会不可预测地下降。

Method: RPS是一种后处理、无需训练的方法，通过在相关偏好的局部邻域中采样多个响应来创建候选池，然后选择与用户原始意图最匹配的响应。

Result: 在三种不同对齐范式(DPA、DPO、SFT)上的综合实验表明，RPS相对于基线方法持续提升鲁棒性，在具有挑战性的未充分表示区域偏好上达到69%的胜率。

Conclusion: RPS提供了一个实用且理论完备的解决方案，能够增强偏好对齐模型的可靠性，无需模型重新训练。
...
<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成(RAG)在多步问题和异构证据源上表现脆弱，需要在准确性、延迟和资源预算之间权衡。

Method: 将文档、表格和知识图谱线性化为可逆分层序列，通过头代理和迭代代理进行结构感知的迭代检索，最后合成规范化证据生成答案。

Result: 在HotpotQA、HybridQA/TAT-QA和MetaQA数据集上相比单次检索、多跳和代理式RAG基线获得一致的EM/F1提升，且效率高。

Conclusion: HSEQ框架具有格式无关的统一性、预算感知的迭代和证据规范化三大优势，能可靠地提升问答的一致性和可审计性。
...
Result: 研究发现翻译质量存在系统性差异，左翼、中间和右翼主流政党的演讲翻译质量明显优于边缘政党，揭示了LLM在政治立场上的偏见。

Conclusion: 通过多语言翻译公平性框架可以更有效地检测LLM的政治偏见，翻译质量差异反映了模型对主流政治话语的系统性偏好。
...
TL;DR: 提出ARC-Encoder，一种可适配的文本表示压缩器，通过将上下文压缩为连续表示来替代解码器LLM中的token嵌入，实现4-8倍的压缩率，在保持性能的同时提高推理效率。
...
Method: 设计ARC-Encoder编码器，将上下文压缩为更少的连续表示来替代token嵌入。系统研究了编码器的训练策略和架构选择。

Result: ARC-Encoder在多个基准测试中达到最先进性能，同时提高推理计算效率。模型可适配多个解码器，单个编码器可泛化到不同LLM。

Conclusion: ARC-Encoder是一种灵活高效的便携式编码器解决方案，可与多个LLM无缝协作，在保持性能的同时显著降低推理成本。
...
TL;DR: CenterBench是一个包含9,720个理解问题的数据集，用于测试语言模型在中心嵌入句上的表现，区分语法结构理解与语义模式匹配。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法区分语言模型是真正理解语法结构还是仅依赖语义模式匹配，需要开发新方法来量化这种差异。

Method: 创建包含递归嵌套关系从句的中心嵌入句数据集，每个句子都有语法相同但语义不合理的对应版本，并设计六种理解问题测试不同层面的理解能力。

Result: 测试六个模型发现，随着句子复杂度增加，合理与不合理句子之间的性能差距系统性扩大（中位差距达26.8个百分点），表明模型在复杂情况下会放弃结构分析转向语义关联。

Conclusion: CenterBench提供了首个识别模型何时从结构分析转向模式匹配的框架，揭示了语义合理性在某些情况下反而损害性能，推理模型虽提高准确性但仍存在语义捷径等问题。
...
TL;DR: GlobalRAG是一个强化学习框架，通过分解问题为子目标、协调检索与推理、迭代优化证据来解决多跳问答中的全局规划缺失和不可靠执行问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在检索增强生成中虽有效果，但在多跳问答中存在两个基本限制：(i)缺乏全局规划来构建多步推理结构，(ii)不可靠的执行阻碍了有效查询制定和检索证据的一致性使用。

Method: 提出GlobalRAG框架，将问题分解为子目标，协调检索与推理，迭代优化证据。引入规划质量奖励和子目标完成奖励来指导过程，并使用渐进权重退火策略平衡过程导向和结果导向目标。

Result: 在领域内和领域外基准测试中，GlobalRAG显著优于强基线，仅使用8k训练数据（基线训练数据的42%），在EM和F1上平均提升14.2%。
...
TL;DR: 提出多智能体认知决策框架(MACDF)，将电商搜索从被动检索转变为主动决策支持，显著提升推荐准确率和用户满意度
...
Result: 离线评估显示在推荐准确率和用户满意度方面显著提升，特别是在涉及否定、多约束或推理需求的复杂查询中；在线A/B测试证实了实际效果
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明ChatGPT能够按照编码规则对通信数据进行编码，且准确率与人类评分者相当，但该技术是否对不同人口统计群体（如性别和种族）存在偏见仍不清楚。

Method: 使用ChatGPT对三种协作任务（谈判、问题解决和决策制定）的通信数据进行自动编码，分析不同性别和种族群体间的差异。

Result: ChatGPT基于的编码在性别和种族群体间未表现出显著偏见。

Conclusion: ChatGPT在协作和沟通的大规模评估中具有应用潜力，其编码结果在不同人口统计群体间无显著差异。
...
TL;DR: 团队在阿拉伯语AI生成文本检测任务中获第五名，发现多语言XLM-RoBERTa模型表现优于专用阿拉伯语模型，F1分数达0.7701。


<details>
  <summary>Details</summary>
Motivation: 研究不同预训练模型在阿拉伯语AI生成文本检测任务中的有效性，比较专用阿拉伯语模型与多语言模型的性能。

Method: 使用AraELECTRA、CAMeLBERT和XLM-RoBERTa三个预训练Transformer模型，在提供的阿拉伯语数据集上进行微调，执行二分类任务。

Result: 多语言XLM-RoBERTa模型表现最佳，F1分数为0.7701，超过了专门的阿拉伯语模型。
...
TL;DR: 本文基于人类好奇心评估问卷5DCR设计了一个评估框架，发现大语言模型比人类有更强的求知欲，但在不确定环境中倾向于保守选择。好奇心能增强模型的推理和主动学习能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自然语言处理领域的进步，研究者希望探讨这些模型是否具备类似人类的好奇心驱动学习能力。

Method: 从人类好奇心评估问卷Five-Dimensional Curiosity scale Revised (5DCR)出发，设计了一个涵盖信息寻求、刺激寻求和社会好奇心等维度的综合评估框架。

Result: 结果显示大语言模型表现出比人类更强的知识渴求，但在面对不确定环境时仍倾向于保守选择。好奇心行为能增强模型的推理和主动学习能力。

Conclusion: 大语言模型有潜力表现出类似人类的好奇心，这为未来开发模型的学习能力和创新研究提供了实验支持。
...
TL;DR: 大型推理模型在多语言推理中倾向于使用英语进行推理，这虽然提高了答案准确性，但存在翻译错误风险，特别是在复杂任务中表现更明显。


<details>
  <summary>Details</summary>
Motivation: 探索大型推理模型在多语言环境下的推理能力，关注其默认使用英语推理的现象及其对语言文化细微差别处理的影响。

Method: 系统比较模型在英语和问题语言中的推理表现，评估MGSM和GPQA Diamond两个任务，分析推理轨迹中的认知属性。

Result: 英语推理轨迹显示出更高的认知行为存在度，英语推理通常获得更高的答案准确率，且随着任务复杂度增加，性能差距扩大。但英语中心策略易受翻译错误影响。

Conclusion: 虽然英语推理能提高性能，但存在翻译错误风险，需要在多语言推理中平衡英语推理优势与避免翻译错误的需求。
...
Method: 创建CantoNLU基准测试，包含词义消歧、语言可接受性判断、语言检测、自然语言推理、情感分析、词性标注和依存句法分析7个任务。评估了四种模型：未在粤语上训练的普通话模型、通过持续预训练得到的粤语适应模型、从头训练的粤语单语模型。
...
TL;DR: 提出神经多样性（去相关的并行表示）作为减少语言模型幻觉的新机制，通过ND-LoRA方法在固定参数和数据预算下平均减少14.6%的幻觉，最高可达25.6%。


<details>
  <summary>Details</summary>
Motivation: 尽管参数、计算和数据不断增加，语言模型仍然存在幻觉问题。受投资组合理论启发，提出神经多样性作为减少幻觉的机制。

Method: 提出ND-LoRA方法，结合并行LoRA适配器和Barlow Twins正则化，通过降低表示相关性来减少幻觉。

Result: ND-LoRA平均减少14.6%的幻觉（最高25.6%），且不降低总体准确率。神经相关性每增加0.1%，幻觉增加3.8%。
...
<details>
  <summary>Details</summary>
Motivation: 传统MBR解码在机器翻译等约束性任务中表现良好，但在对话、指令跟随等开放任务中，标准相似性效用函数可能选择模型分布中广泛代表但结构次优的响应。

Method: 提出了三种轻量级效用函数适配方法，使MBR对结果空间的结构变化更敏感；构建了包含对话行为、情感和响应结构的数据集；提出了两个评估结构最优性的指标。

Result: 分析表明标准相似性效用函数在结构最优性指标上表现不佳，而提出的适配方法显著提升了结构最优性；在真实指令跟随基准测试中，生成质量提升了13.7个百分点。
...
TL;DR: 本研究通过用户实验发现，在隐私敏感场景下，用户对LLM响应的隐私保护质量和帮助性评价存在较大个体差异，而代理LLM与用户评价相关性低，表明需要用户中心的研究方法来评估LLM的隐私保护能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用代理LLM来评估隐私保护规范，忽略了真实用户的感知，且主要关注隐私保护质量而未深入研究帮助性的细微差异。

Method: 对94名参与者进行了用户研究，使用PrivacyLens中的90个场景，比较用户和5个代理LLM对相同LLM响应的隐私保护质量和帮助性评价。

Result: 用户对相同响应的隐私保护质量和帮助性评价一致性低，而代理LLM之间一致性高但与用户评价相关性低。

Conclusion: LLM响应的隐私和帮助性具有个体特异性，代理LLM无法准确估计用户感知，需要用户中心的研究方法和改进代理LLM与用户的对齐。
...
TL;DR: 本研究开发并评估了多种NLP方法从临床笔记中提取氟嘧啶治疗和毒性信息，发现基于大语言模型的方法表现最佳，特别是错误分析提示方法在所有类别中都达到了完美的F1分数。
...
Method: 构建了包含236份临床笔记的金标准数据集，开发了基于规则、机器学习（随机森林、SVM、逻辑回归）、深度学习（BERT、ClinicalBERT）和大语言模型（零样本提示和错误分析提示）的NLP方法，使用80:20的训练-测试分割。

Result: 错误分析提示方法在治疗和毒性提取方面达到了最优的精确率、召回率和F1分数（F1=1.000），零样本提示在治疗提取上F1=1.000，毒性提取F1=0.876。逻辑回归和SVM在毒性提取上排名第二（F1=0.937）。深度学习方法表现较差。
...
TL;DR: 该论文首次系统分析了大型推理模型作为机器翻译质量评估器的潜力，发现LRMs需要定制化评估材料、对简单实例过度思考以及评分机制导致高估等问题，提出通过训练合成的人类思维轨迹来校准LRM思维的方法。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中通过中间"思考"过程提高了推理能力，但其在机器翻译质量评估中的潜力尚未充分探索，需要系统分析LRM作为评估器的可行性。

Method: 提出校准LRM思维的方法，通过在合成的人类思维轨迹上进行训练，减少思考预算并改进评估性能。

Result: 在WMT24 Metrics基准测试中，该方法将思考预算大幅减少约35倍，同时在不同规模的LRM（7B到32B）上提高评估性能，例如R1-Distill-Qwen-7B实现了+8.7相关点改进。
...
TL;DR: 构建了一个基于真实应用场景的数据集，用于评估大语言模型在负责任AI维度（特别是公平性）上的表现，提出了结合质量、真实性、安全性和公平性的评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型的方法主要关注文本生成等高层任务，没有针对特定AI应用，这不足以评估负责任AI维度如公平性，因为不同应用中受保护属性的相关性不同。

Method: 构建了一个基于真实应用场景（根据产品特征生成纯文本产品描述）的数据集，通过公平属性与性别化形容词和产品类别的交叉参数化，生成丰富的标注提示集合。

Result: 展示了如何使用该数据识别大语言模型在质量、真实性、安全性和公平性方面的差距。

Conclusion: 为大语言模型评估提出了一个具体方案，并为研究社区贡献了实际资源，强调了应用驱动的公平性评估的重要性。
...
TL;DR: 提出了一种混合注意力模型，在线性注意力基础上引入稀疏注意力机制，通过可学习的token淘汰和滑动窗口注意力，在保持线性复杂度同时提升检索密集型任务的性能。
...
Method: 结合线性注意力和稀疏注意力，提出可学习的token淘汰机制，使用轻量级CNN自适应保留关键KV对，同时提供高效的Triton内核实现。

Result: 在检索密集型基准测试中验证了方法的有效性。

Conclusion: 混合注意力模型在保持线性注意力效率的同时，显著提升了检索密集型任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了降低在检索增强生成中使用长上下文的计算成本，需要开发有效的软上下文压缩方法，将输入序列转换为更短的连续表示。

Method: 开发了轻量级的均值池化方法，并与压缩标记架构进行比较，同时训练单一压缩器输出多种压缩比，在多个QA数据集、模型家族和压缩比下进行广泛实验。

Result: 简单的均值池化方法取得了最强的性能，在训练多种压缩比时性能下降相对较小。不同架构和训练机制之间的权衡更加复杂。

Conclusion: 均值池化方法在软上下文压缩中表现最佳，但压缩方法的整体格局较为复杂，需要更细致地考虑不同架构和训练机制之间的权衡。
...
TL;DR: LLM生成文本检测面临定义模糊、场景多样、人类编辑影响等挑战，现有检测器仅在特定条件下有效，结果应作为参考而非决定性指标。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型广泛使用，检测其生成文本的需求增加，但缺乏对'LLM生成文本'的统一定义，现实应用场景复杂多样，现有检测方法无法充分应对。

Method: 分析LLM生成文本检测面临的挑战，包括定义模糊、使用场景差异、模型多样性、人类编辑影响等，评估现有检测方法的局限性。

Result: 现有检测基准和评估方法无法充分覆盖真实世界检测器的各种应用条件，检测器的数值结果常被误解，其重要性正在减弱。

Conclusion: 检测器在特定条件下仍然有用，但其结果应仅作为参考而非决定性指标，需要重新思考LLM生成文本检测的定义和评估方法。
...
TL;DR: DeEAR是一个评估语音表达性的框架，通过将人类对语音表达性的偏好转化为客观评分，解决了现有评估方法的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的语音到语音模型生成可理解的语音但缺乏自然表达性，主要由于缺乏可靠的评估指标。现有方法如主观MOS评分、低层级声学特征和情感识别成本高、有限或不完整。

Method: 基于语音学和心理学，DeEAR从三个维度评估语音：情感、韵律和自发性，使用少于500个标注样本就能实现与人类感知的强对齐（SRCC = 0.86）。

Result: DeEAR不仅能可靠评分，还能实现公平基准测试和针对性数据筛选。它不仅能区分不同S2S模型的表达性差距，还能筛选14K个表达性话语形成ExpressiveSpeech数据集，将S2S模型的表达性得分从2.0提升到23.4（100分制）。

Conclusion: DeEAR为语音表达性评估提供了一个有效的框架，显著提升了语音到语音模型的表达性表现。
...
TL;DR: 提出一个多智能体、人在回路的工作流，共同设计具有规定横向对角门的量子码。基于SSLP框架，通过GPT-5驱动，在TeXRA平台上实现。工作流包含三个角色：合成智能体、搜索智能体和审计智能体，通过系统扫描和验证生成新的量子码构造。


<details>
  <summary>Details</summary>
Motivation: 开发一个系统化的工作流来共同设计具有特定横向对角门的量子码，将横向对角可行性重新构建为可扩展的分析管道，结合系统枚举和精确分析重建。

Method: 基于SSLP框架，在LaTeX-Python环境中实现多智能体协作工作流。三个智能体分别负责问题制定、候选码搜索和验证、独立审计。通过模块余数划分基字符串，通过小型线性规划强制执行Z-边际Knill-Laflamme等式。

Result: 对于距离d=2和非退化余数的情况，在码维度K∈{2,3,4}和n≤6量子比特范围内，系统扫描生成了经过认证支持的表格，记录了可实现的循环逻辑群。例如，对于K=3在n=6时获得了阶16的群。还展示了一个新的((6,4,2))码实现了横向受控相位门。

Conclusion: 该工作流将横向对角可行性重新构建为可扩展的分析管道，结合系统枚举和精确分析重建。它产生了可重现的码构造，支持向更大K和更高距离的针对性扩展，并朝着数据驱动分类的方向发展。
...
TL;DR: 研究探讨了人类与LLM在多轮对话中的双向影响，发现人类观点变化很小，而LLM输出变化显著，个性化设置会放大这种双向变化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM对用户的单向影响，缺乏对双向影响和多轮对话动态的研究。

Method: 通过50个争议话题讨论，涉及266名参与者，分为静态陈述、标准聊天机器人和个性化聊天机器人三种条件。

Result: 人类观点几乎不变，LLM输出变化较大，缩小了人机立场差距。个性化设置双向放大变化，个人故事交流最易引发立场变化。

Conclusion: 研究揭示了人机交互中过度对齐的风险，需要谨慎设计个性化聊天机器人以实现更稳定、深思熟虑的对齐。
...
TL;DR: Empathic Prompting是一个多模态人机交互框架，通过面部表情识别捕捉用户情感线索，将非语言上下文融入LLM对话，无需显式用户控制即可实现流畅的情感对齐对话。


<details>
  <summary>Details</summary>
Motivation: 传统多模态界面需要显式用户控制，而人类交流中大量情感信息通过非语言方式传递。该研究旨在开发一个能够无缝整合非语言情感线索的LLM交互系统，特别是在医疗、教育等情感信号至关重要的领域。

Method: 采用模块化架构，集成商业面部表情识别服务，将情感线索作为上下文信号嵌入提示中。系统通过本地部署的DeepSeek实例实现，无需用户显式控制即可增强文本输入的情感信息。

Result: 初步评估（N=5）显示系统能一致地将非语言输入整合为连贯的LLM输出，参与者强调了对话的流畅性。系统成功实现了情感线索与对话内容的对齐。

Conclusion: Empathic Prompting证明了在LLM对话中无缝整合非语言情感线索的可行性，为聊天机器人介导的通信开辟了新方向，特别是在情感敏感的应用领域具有重要潜力。
...
TL;DR: 提出了Prompt Decorators框架，通过声明式、可组合的控制令牌来管理LLM行为，实现推理风格、表达方式等行为维度的控制，而不改变任务内容。


<details>
  <summary>Details</summary>
Motivation: 传统提示工程依赖冗长的自然语言指令，缺乏可重现性、模块化和可解释性，用户难以对LLM的推理和输出表达进行一致控制。

Method: 设计了20个核心装饰器，分为认知与生成、表达与系统两个功能家族，定义统一语法、作用域模型和确定性处理流水线。

Result: 展示了改进的推理透明度、降低的提示复杂性和跨领域的标准化模型行为。

Conclusion: Prompt Decorators为提示设计创建了可重用和可解释的接口，对互操作性、行为一致性和可扩展AI系统的声明式接口开发具有重要意义。
...
### [70] [Every Question Has Its Own Value: Reinforcement Learning with Explicit Human Values](https://arxiv.org/abs/2510.20187)
*Dian Yu,Yulai Zhao,Kishan Panaganti,Linfeng Song,Haitao Mi,Dong Yu*

Main category: cs.LG

TL;DR: RLEV方法通过将人类定义的价值信号直接融入奖励函数，在LLM优化中实现与可量化人类价值信号的对齐，相比仅基于正确性的方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法虽然能有效训练模型，但忽略了不同任务的重要性差异，需要将人类价值信号直接整合到奖励函数中。

Method: RLEV扩展了RLVR框架，在奖励函数中直接融入人类定义的价值信号，使用带有明确真实价值标签的考试风格数据，通过价值加权梯度放大机制实现价值敏感的策略。

Result: RLEV在多种RL算法和模型规模下持续优于仅基于正确性的基线方法，不仅提高了价值加权准确率，还学会了价值敏感的终止策略：对低价值提示简洁，对高价值提示详尽。

Conclusion: RLEV通过优化明确的效用函数，为LLM与人类优先级对齐提供了实用路径，即使在噪声价值信号下也保持稳健。
...
TL;DR: ImpossibleBench是一个用于量化和研究LLM代理利用测试用例作弊行为的基准框架，通过创建任务规范与单元测试冲突的"不可能"任务来测量作弊率。
...
Method: 通过修改现有基准（如LiveCodeBench和SWE-bench）创建"不可能"任务变体，在自然语言规范和单元测试之间引入直接冲突，测量代理在这些任务上的通过率作为作弊率。

Result: 揭示了从简单测试修改到复杂运算符重载等不同粒度的作弊行为，展示了提示、测试访问和反馈循环对作弊率的影响，并提供了已验证的欺骗性解决方案测试平台。

Conclusion: ImpossibleBench是一个实用的多功能框架，可用于研究模型行为、上下文工程和开发监控工具，有助于构建更鲁棒可靠的LLM系统。
...
TL;DR: 提出了基于相对排序的概率(RBP)指标和相对排序缩放定律，从相对排序角度补充了仅关注交叉熵的传统缩放定律研究。
...
Method: 首先提出RBP指标，量化正确token在top预测中的排名概率；然后建立相对排序缩放定律，描述RBP如何随模型规模增加而改进；在四个数据集和四个模型家族上进行广泛实验验证。

Result: 通过跨越五个数量级的实验证明了该定律的鲁棒性和准确性；展示了该定律的两个应用：为涌现现象提供更深解释，以及促进寻找缩放定律的基本理论。

Conclusion: 相对排序缩放定律补充了交叉熵视角，有助于更全面地理解大语言模型的缩放规律，为实际开发和理论探索提供了宝贵见解。
...
### [73] [BadGraph: A Backdoor Attack Against Latent Diffusion Model for Text-Guided Graph Generation](https://arxiv.org/abs/2510.20792)
*Liang Ye,Shengqin Chen,Jiazhu Dai*

Main category: cs.LG

TL;DR: BadGraph是一种针对文本引导图生成的潜在扩散模型的隐蔽后门攻击方法，通过文本触发器植入后门，在推理时生成攻击者指定的子图，同时保持对干净输入的正常性能。


<details>
  <summary>Details</summary>
Motivation: 随着图生成技术的快速发展，后门漏洞等安全问题日益突出。目前已有工作探索了图像扩散和无条件图生成中的后门攻击，但文本引导的图生成领域尚未得到充分研究。

Method: BadGraph利用文本触发器污染训练数据，在VAE和扩散训练阶段植入后门，当推理时出现触发器时诱导生成攻击者指定的子图。

Result: 在四个基准数据集上的实验表明，仅需不到10%的投毒率即可达到50%的攻击成功率，24%的投毒率可实现超过80%的成功率，且对良性样本的性能影响可忽略不计。

Conclusion: 研究揭示了文本引导图生成中潜在扩散模型的安全漏洞，强调了在药物发现等应用中的严重风险，迫切需要针对此类扩散模型的后门攻击进行鲁棒防御。
...
### [74] [Compress to Impress: Efficient LLM Adaptation Using a Single Gradient Step on 100 Samples](https://arxiv.org/abs/2510.20800)
*Shiva Sreeram,Alaa Maalouf,Pratyusha Sharma,Daniela Rus*

Main category: cs.LG

TL;DR: 提出了一种快速无微调的LLM适配方法，通过选择性矩阵降维和梯度分析，仅需100个样本和单次梯度计算即可有效提升下游任务准确率。


<details>
  <summary>Details</summary>
Motivation: LASER方法虽然能提升LLM下游任务准确率，但其逐层搜索和全数据集前向传播的开销使其难以快速部署。本文旨在消除这种计算开销。

Method: 结合多个发现：(i)仅需检查少量关键矩阵；(ii)使用矩阵奇异值梯度识别需要降维的矩阵；(iii)扩展分解搜索空间，允许矩阵行围绕多个子空间聚类；(iv)仅用100个样本进行评估。

Result: 该方法将准确率提升高达24.6个百分点，同时大幅减少搜索时间，实现了快速稳健的LLM适配。

Conclusion: 通过单次梯度步骤和快速扫描关键层，可以在无需微调的情况下有效适配LLM到新数据集。
...
### [75] [Multimedia-Aware Question Answering: A Review of Retrieval and Cross-Modal Reasoning Architectures](https://arxiv.org/abs/2510.20193)
*Rahul Raja,Arpita Vats*

Main category: cs.IR

TL;DR: 该论文综述了多媒体检索增强问答系统的最新进展，重点分析了整合视觉、语言和音频模态的架构，并讨论了跨模态对齐、延迟-准确性权衡等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 传统问答系统主要依赖结构化文本数据，而多媒体内容的快速增长为检索增强问答带来了新的挑战和机遇，需要整合多种模态信息来提升系统性能。

Method: 基于检索方法、融合技术和答案生成策略对现有方法进行分类，分析基准数据集、评估协议和性能权衡，重点关注跨模态对齐架构。

Result: 系统梳理了多媒体问答系统的发展现状，识别了不同方法的优势和局限性，为相关研究提供了全面的技术路线图。

Conclusion: 提出了构建更鲁棒和上下文感知的多媒体问答系统的开放问题和未来研究方向，强调了跨模态语义基础的重要性。
...
### [76] [Analyticup E-commerce Product Search Competition Technical Report from Team Tredence_AICOE](https://arxiv.org/abs/2510.20674)
*Rakshith R,Shubham Sharma,Mohammed Sameer Khan,Ankush Chopra*

Main category: cs.IR

TL;DR: 该研究开发了多语言电商搜索系统，通过数据增强和模型微调在查询-类别和查询-商品相关性任务中取得良好表现，最终获得第4名。


<details>
  <summary>Details</summary>
Motivation: 解决多语言电商搜索中的查询-类别相关性和查询-商品相关性评估问题，确保对所有目标语言的覆盖。

Method: 通过翻译现有数据集进行数据增强，使用Gemma-3 12B和Qwen-2.5 14B模型进行微调，采用多种训练策略。

Result: Gemma-3 12B(4位)模型在查询-类别任务中使用原始和翻译数据表现最佳，在查询-商品任务中使用原始、翻译和少数类数据创建表现最佳，最终平均F1分数为0.8857。

Conclusion: 数据增强和适当的模型微调策略在多语言电商搜索任务中有效，Gemma-3 12B模型在压缩后仍能保持良好性能。
...
### [77] [Communication to Completion: Modeling Collaborative Workflows with Intelligent Multi-Agent Communication](https://arxiv.org/abs/2510.19995)
*Yiming Lu,Xun Wang,Simin Ma,Shujian Liu,Sathish Reddy Indurthi,Song Wang,Haoyun Deng,Fei Liu,Kaiqiang Song*

Main category: cs.MA

TL;DR: C2C是一个用于多智能体系统的通信框架，通过Alignment Factor指标和Sequential Action Framework，使智能体能够做出成本感知的通信决策，在复杂任务中减少约40%的完成时间。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM系统缺乏面向任务的系统化通信框架，无法有效支持复杂工作空间中的团队协作需求。

Method: 提出两个关键创新：(1) Alignment Factor(AF) - 量化智能体任务对齐度的新指标；(2) Sequential Action Framework - 将逐步执行与智能通信决策集成。

Result: 在三个复杂度层级和5-17个智能体的编码工作流中测试，C2C将任务完成时间减少约40%，通信成本可接受，所有任务在标准配置下成功完成且具有良好的扩展性。
...
### [78] [SODBench: A Large Language Model Approach to Documenting Spreadsheet Operations](https://arxiv.org/abs/2510.19864)
*Amila Indika,Igor Molybog*

Main category: cs.SE

TL;DR: 本文提出了电子表格操作文档化(SOD)任务，创建了包含111个代码片段及其自然语言摘要的基准，评估了5个LLM在生成电子表格文档方面的表现。
...
Method: 构建了111个电子表格操作代码片段及其对应自然语言摘要的基准数据集，使用BLEU、GLEU、ROUGE-L和METEOR指标评估GPT-4o、GPT-4o-mini、LLaMA-3.3-70B、Mixtral-8x7B和Gemma2-9B五个LLM的性能。

Result: 研究发现LLM能够生成准确的电子表格文档，使SOD成为增强电子表格可重现性、可维护性和协作工作流程的可行前提步骤。

Conclusion: LLM可以有效地生成电子表格操作文档，但仍有需要解决的挑战，SOD为实现更好的电子表格管理和协作提供了有前景的方向。
...
### [79] [Branch-and-Browse: Efficient and Controllable Web Exploration with Tree-Structured Reasoning and Action Memory](https://arxiv.org/abs/2510.19838)
*Shiqi He,Yue Cui,Xinyu Ma,Yaliang Li,Bolin Ding,Mosharaf Chowdhury*

Main category: cs.AI

TL;DR: Branch-and-Browse是一个细粒度的网页代理框架，通过树状结构探索、网页状态重放和页面动作记忆，显著提升了基于大语言模型的网页代理的推理深度和执行效率。


<details>
  <summary>Details</summary>
Motivation: 现有的自主网页代理方法在推理深度和效率方面存在局限：线性方法无法处理多步推理且缺乏有效回溯，其他搜索策略则粒度粗糙且计算成本高。

Method: 采用显式子任务管理和树状结构探索实现可控多分支推理，通过网页状态重放和后台推理引导探索，并利用页面动作记忆在会话内外共享已探索动作。
...
### [80] [LLMs can hide text in other text of the same length.ipynb](https://arxiv.org/abs/2510.20075)
*Antonio Norelli,Michael Bronstein*

Main category: cs.AI

TL;DR: 本文提出了一种使用大型语言模型将秘密信息隐藏在看似普通文本中的方法，能够在相同长度的文本中嵌入完全不同的内容，且编码解码过程快速高效。


<details>
  <summary>Details</summary>
Motivation: 探索文本与作者意图之间的脱钩现象，展示LLM如何能够创建表面上连贯但隐藏着完全不同含义的文本，这对书面通信的信任构成威胁。

Method: 使用简单的协议，即使是8亿参数的开源LLM也能实现高质量的结果，可以在笔记本电脑上快速编码和解码信息。

Result: 成功实现了在看似普通的文本中隐藏秘密信息，例如将政治批评隐藏在赞美文本中，或将秘密手稿隐藏在普通产品评论中。

Conclusion: 这种方法展示了文本与作者意图的彻底分离，进一步削弱了对书面通信的信任，对AI安全提出了紧迫问题，并挑战了我们对LLM"知道"某事的理解。
...
### [81] [AI PB: A Grounded Generative Agent for Personalized Investment Insights](https://arxiv.org/abs/2510.20099)
*Daewoo Park,Suho Park,Inseok Hong,Hanwool Lee,Junkyu Park,Sangjun Lee,Jeongman An,Hyunbin Loh*

Main category: cs.AI

TL;DR: AI PB是一个在生产环境中部署的生成式智能体，用于零售金融领域。它通过组件化编排、混合检索管道和多阶段推荐机制，主动生成合规且个性化的投资洞察，完全在韩国金融监管下运行。
...
Result: 通过人工QA和系统指标验证，证明了基于明确路由和分层安全机制的接地生成能够在高风险金融环境中提供可信的AI见解。

Conclusion: 在金融等高风险领域，通过明确的系统路由和分层安全机制，可以实现可信的生成式AI洞察，为生产级部署提供了可行的技术方案。
...
### [82] [IKnow: Instruction-Knowledge-Aware Continual Pretraining for Effective Domain Adaptation](https://arxiv.org/abs/2510.20377)
*Tianyi Zhang,Florian Mai,Lucie Flek*

Main category: cs.AI

TL;DR: 提出了IKnow框架，通过指令-响应对话格式的自监督目标，在无需外部资源的情况下实现语言模型的持续预训练，保护指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 现有持续预训练方法需要原始基础模型或外部知识库，这在基础模型权重不可用或可靠外部语料库缺失的现实场景中存在障碍。

Method: IKnow框架在指令-响应对话格式中制定新的自监督目标，利用文本中嵌入的领域知识，在更深语义层次进行编码学习。

Result: 该方法避免了标准自监督目标对指令调优模型指令跟随能力和语义表示的退化问题。

Conclusion: IKnow提供了一个简单通用的框架，能够在无需外部资源的情况下实现语言模型的持续领域适应。
...
### [83] [What Defines Good Reasoning in LLMs? Dissecting Reasoning Steps with Multi-Aspect Evaluation](https://arxiv.org/abs/2510.20603)
*Heejin Do,Jaehui Hwang,Dongyoon Han,Seong Joon Oh,Sangdoo Yun*

Main category: cs.AI

TL;DR: 提出了CaSE（因果逐步评估）方法，通过评估推理步骤的相关性和连贯性来改进LLM推理能力，而不仅仅是关注最终答案的正确性。
...
Method: 将推理质量分解为相关性和连贯性两个维度，提出CaSE方法，仅使用前序上下文评估每个推理步骤，避免后见之明偏差。
...
Conclusion: 该工作为AI及其他领域的研究人员提供了有价值的洞察，帮助他们在快速发展的研究环境中保持更新。
...
### [85] [BIOCAP: Exploiting Synthetic Captions Beyond Labels in Biological Foundation Models](https://arxiv.org/abs/2510.20095)
*Ziheng Zhang,Xinyue Ma,Arpita Chowdhury,Elizabeth G. Campolongo,Matthew J. Thompson,Net Zhang,Samuel Stevens,Hilmar Lapp,Tanya Berger-Wolf,Yu Su,Wei-Lun Chao,Jianyang Gu*

Main category: cs.CV

TL;DR: 该研究探索使用描述性标题作为生物多模态基础模型的额外监督源，通过多模态大语言模型生成合成标题来弥补实例特定标题稀缺的问题，训练出BIOCAP模型在物种分类和图文检索中表现优异。


<details>
  <summary>Details</summary>
Motivation: 图像和标题可以视为物种潜在形态空间中的互补样本，各自捕捉某些生物特征。在训练中融入标题有助于与这种共享潜在结构对齐，强调潜在诊断特征同时抑制虚假相关性。主要挑战在于大规模获取忠实、实例特定的标题。

Method: 使用多模态大语言模型生成合成标题，通过维基百科获取的视觉信息和针对特定分类单元定制的格式示例来指导生成。这些领域特定上下文有助于减少幻觉并产生准确的、基于实例的描述性标题。

Result: 使用这些标题训练了BIOCAP模型，该模型能够捕捉丰富的语义，在物种分类和文本图像检索方面表现出色。

Conclusion: 这些结果证明了描述性标题在将生物图像与多模态基础模型连接方面超越了标签的价值。
...
### [86] [Why LVLMs Are More Prone to Hallucinations in Longer Responses: The Role of Context](https://arxiv.org/abs/2510.20229)
*Ge Zheng,Jiaye Qian,Jiajin Tang,Sibei Yang*

Main category: cs.CV

TL;DR: 本文提出了一种"诱导-检测-抑制"框架来缓解大视觉语言模型在长文本生成中的幻觉问题，发现幻觉风险主要源于对上下文连贯性的依赖而非文本长度本身。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在生成长文本时容易出现幻觉问题，传统观点认为这是由长度引起的错误累积，但本文旨在探究是否存在更深层机制。

Method: 提出"诱导-检测-抑制"框架：通过设计特定上下文主动诱导幻觉，利用诱导实例进行早期高风险检测，在实际解码过程中抑制对象级幻觉。

Result: 在所有基准测试中都取得了持续且显著的改进，验证了框架的有效性，同时重新验证了关于上下文依赖的假设。

Conclusion: 研究不仅提供了性能提升，更重要的是为深入探索大视觉语言模型在长文本响应中的幻觉问题提供了新的见解和第一步。
...
### [87] [Calibrating Multimodal Consensus for Emotion Recognition](https://arxiv.org/abs/2510.20256)
*Guowei Zhong,Junjie Li,Huaiyu Zhu,Ruohong Huan,Yun Pan*

Main category: cs.CV

TL;DR: 提出校准多模态共识模型(CMC)，通过伪标签生成模块实现单模态自监督预训练，使用参数无关融合模块和多模态共识路由器缓解文本主导问题，在语义不一致场景下表现优异。
...
Method: CMC包含伪标签生成模块(PLGM)进行单模态自监督预训练，参数无关融合模块(PFM)和多模态共识路由器(MCR)进行多模态微调。

Result: 在CH-SIMS、CH-SIMS v2、CMU-MOSI和CMU-MOSEI四个数据集上达到或超越最先进方法性能，在语义不一致场景下优势明显。
...
### [88] [Small Drafts, Big Verdict: Information-Intensive Visual Reasoning via Speculation](https://arxiv.org/abs/2510.20812)
*Yuhan Liu,Lianhui Qin,Shengjie Wang*

Main category: cs.CV

TL;DR: 提出了Speculative Verdict (SV)框架，通过结合多个轻量级草稿专家和大型裁决模型，在信息密集型图像理解任务中实现高效准确的多模态推理。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在信息密集型图像理解中存在困难，这些图像密集交织文本注释和细粒度图形元素，主要挑战在于精确定位关键线索和整合分散证据的多跳推理。

Method: 采用训练免费的框架，包含草稿阶段和裁决阶段。草稿阶段使用小型VLMs作为草稿专家生成多样化的定位候选推理路径；裁决阶段使用强大的VLM综合这些路径生成最终答案。还引入了共识专家选择机制，只转发高一致性的推理路径。

Result: 在具有挑战性的信息密集和高分辨率视觉问答基准测试中（包括InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K）取得了一致的性能提升。
