...
TL;DR: 提出了StreetMath基准来评估LLM在现实近似场景下的近似推理能力，发现LLM倾向于计算精确值而非近似，且近似与精确运算使用不同的神经组件。
...
Method: 引入StreetMath基准，在多种LLM架构上进行评估，并应用机制解释性技术分析内部计算状态。
...
TL;DR: 提出了一种基于模糊逻辑和句法依赖解析的细粒度情感分析方法，能够对实体评论进行强度分级（非常弱、弱、中等、强、非常强），而不仅仅是简单的正负情感分类。


<details>
  <summary>Details</summary>
Motivation: 传统的基于词典的情感分析方法无法考虑意见的强度差异，即无法区分非常强烈、强烈、中等、非常弱和弱等不同程度的情感表达。

Method: 结合模糊逻辑算法将意见词分类到不同强度类别，使用句法依赖解析找到与目标方面词相关的关系，通过考虑与特定方面相关的意见词来计算实体在该方面的得分。

Result: 该方法能够对实体评论进行更细粒度的情感强度分类，为基于用户查询的实体排名提供更精确的依据。

Conclusion: 提出的方法通过结合模糊逻辑和句法分析，实现了对意见强度和方向的更精细分析，提升了情感分析的准确性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有立场检测研究主要集中于目标相关任务且多为英文数据集，难以在韩语等低资源语言中开发模型。

Method: 从韩国政党新闻稿中收集数据，构建大规模韩语立场检测数据集，并训练最先进的深度学习模型。

Result: 成功构建了LASTIST数据集，支持目标无关立场检测和历时演化立场检测等多种任务。

Conclusion: LASTIST数据集为韩语立场检测研究提供了重要资源，推动了低资源语言立场检测领域的发展。
...
TL;DR: 提出zFLoRA方法，在基础模型上实现零延迟的低秩适配器，解决现有适配器在推理时计算开销过大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有任务特定适配器虽然参数量小（通常小于基础模型的1%），但在推理时会带来不成比例的计算开销（可达基础模型的2.5倍）。

Method: 提出零延迟融合低秩适配器（zFLoRA），通过融合技术消除适配器在推理时的额外延迟。

Result: 在1B、3B和7B参数量的LLM上，zFLoRA在常识推理、数学推理和摘要对话等18个任务上表现优于LoRA和全微调。在NPU和GPU平台上均实现零到可忽略的延迟开销。

Conclusion: zFLoRA方法有效解决了适配器在推理时的延迟问题，在保持性能的同时实现了高效的推理部署。
...
TL;DR: 提出了三种改进电路发现的方法：使用自助法识别一致归因边、引入基于比率的正边选择策略、用整数线性规划替代贪婪选择，在多个任务和模型上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决机制可解释性中电路发现的关键挑战，即确定模型中执行特定任务的部分，基于MIB基准进行改进。

Method: 1. 使用自助法识别具有一致归因得分的边；2. 引入基于比率的简单选择策略，优先选择强正得分边；3. 用整数线性规划替代标准贪婪选择方法。

Result: 生成的电路更加忠实，在多个MIB任务和模型上优于先前方法。
...
TL;DR: LISTEN框架使用LLM作为零样本偏好预测器，通过自然语言指导专家在多目标决策中选择最佳选项，包含参数化（LISTEN-U）和非参数化（LISTEN-T）两种迭代算法。
...
Method: 提出LISTEN框架，使用LLM作为零样本偏好预测器，包含两种算法：LISTEN-U通过LLM优化参数化效用函数，LISTEN-T使用锦标赛式选择方法处理小批量解决方案。

Result: 在航班预订、购物和考试安排等任务中评估，LISTEN-U在偏好参数化对齐时表现优异，LISTEN-T提供更稳健的性能。
...
TL;DR: LongFilter是一个用于长上下文预训练的数据筛选框架，通过对比模型在长上下文和短上下文下的预测差异来识别具有真正长距离依赖关系的数据样本，从而提升训练效率。
...
Method: 通过对比模型在长上下文和短上下文设置下的预测结果，衡量扩展上下文带来的信息增益，识别那些长距离依赖关系至关重要的样本。

Result: 在LLaMA-3-8B模型上实验，将上下文长度从8K扩展到64K，LongFilter能高效选择高质量数据，在HELMET、LongBench和RULER等基准测试上取得显著改进。

Conclusion: LongFilter框架能够有效筛选适合长上下文预训练的数据，提高训练效率并在多个基准测试上带来实质性提升。
...
TL;DR: 研究发现，尽管人物角色对LLM的整体分类准确性影响不大，但会引入微妙的意识形态偏见，导致模型在不同政治立场的人物角色下对有害内容的判断产生系统性差异。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地用于内容审核系统，确保其公平性和中立性变得至关重要。本研究旨在探索人物角色采用如何影响不同LLM架构、模型大小和内容模态中有害内容分类的一致性和公平性。

Method: 通过分析不同意识形态倾向的人物角色对有害内容分类的影响，考察模型大小、架构和内容模态的影响。进行额外研究验证人物角色在政治针对性任务中的行为模式。

Result: 表面指标显示人物角色对分类准确性影响不大，但深入分析发现：不同意识形态的人物角色对有害内容的标记倾向存在差异；较大模型更倾向于与同意识形态的人物角色保持一致；人物角色会强化自身意识形态观点，同时弱化对立观点的危害性。

Conclusion: 人物角色调节会在LLM输出中引入微妙的意识形态偏见，这引发了对AI系统可能以中立为幌子强化党派观点的担忧，对内容审核系统的公平性构成挑战。
...
TL;DR: CLEAR方法通过实体感知检索在临床问答中实现了更好的性能，F1分数达到0.90，比基于嵌入的检索高出0.04，同时减少了70%以上的token使用量。
...
Method: 开发了临床笔记问答评估平台，将CLEAR与零样本大上下文推理和传统基于分块的检索增强生成进行对比验证。测试了12个包含10,000到65,000个token的临床笔记。

Result: CLEAR获得了58.3%的胜率，平均语义相似度为0.878，比宽上下文处理减少了78%的token使用量。在超过65,000个token的长文档上表现最佳，胜率达到75%。

Conclusion: 实体感知检索提高了临床自然语言处理的效率和准确性。该评估框架为评估临床问答系统提供了可重用且透明的基准，在语义精度和计算效率至关重要的场景中特别有价值。
...
TL;DR: 本文首次从数据中心的视角系统性地综述了数据高效的大语言模型后训练方法，提出了包括数据选择、数据质量增强、合成数据生成、数据蒸馏与压缩以及自演化数据生态系统在内的分类法，并总结了各类代表性方法及未来研究方向。
...
Method: 从数据中心的视角提出数据高效LLM后训练的分类法，涵盖数据选择、数据质量增强、合成数据生成、数据蒸馏与压缩、自演化数据生态系统等五类方法。

Result: 系统总结了各类数据高效后训练方法的代表性技术，并整理了相关论文列表。

Conclusion: 通过分析数据高效LLM后训练的挑战，指出了开放性问题并提出了潜在的研究方向，旨在激发对大规模模型训练中数据利用潜力最大化的进一步探索。
...
TL;DR: 评估LLM在FrameNet语义标注中的表现，比较手动、自动和半自动三种设置，发现半自动方法在框架多样性和标注覆盖率上表现最佳


<details>
  <summary>Details</summary>
Motivation: 尽管LLM工具在语言资源创建中应用广泛，但对其在标注数据集创建中的性能和影响缺乏全面评估，特别是在视角化NLP方法下

Method: 采用三种实验设置比较：手动标注、自动标注和半自动标注，评估标注时间、覆盖率和多样性等指标

Result: 半自动标注设置相比纯人工设置增加了框架多样性并保持相似标注覆盖率，而自动设置在除标注时间外的所有指标上都表现较差

Conclusion: 半自动标注方法结合了人工和自动标注的优势，在语义角色标注任务中表现出色，而纯自动方法在质量上仍有不足
...
TL;DR: RECAP是一种代理式管道，旨在从大型语言模型输出中提取和验证记忆的训练数据，通过反馈驱动的循环和越狱模块显著提高提取效果。


<details>
  <summary>Details</summary>
Motivation: 由于无法检查大型语言模型的训练数据，需要一种方法来确认模型是否见过特定内容，最有力的证据是模型自由重现目标内容。
...
Conclusion: RECAP能够有效提取和验证LLM记忆的训练数据，通过反馈循环和越狱机制显著改善提取性能，为检测模型训练数据提供了可行方案。
...
TL;DR: 该研究挑战了多语言预训练中的常见假设，发现在1.1B和3B参数规模下，适当平衡的多语言数据不会导致性能下降，英语作为枢纽语言有益于跨语言泛化，且未观察到显著的'多语言诅咒'现象。


<details>
  <summary>Details</summary>
Motivation: 探讨多语言数据混合对大型语言模型预训练的影响，特别是关于语言覆盖范围与模型性能之间权衡的争议，以及所谓的'多语言诅咒'问题。

Method: 训练1.1B和3B参数的LLM，使用包含25到400种语言的多语言语料库，研究不同语言组合对模型性能的影响，特别关注英语作为枢纽语言的作用。

Result: 英语与多语言数据结合不会降低任何语言组的表现（前提是语言在预训练语料中有足够token）；英语作为枢纽语言对所有语系都有益；未发现显著的'多语言诅咒'现象。
...
TL;DR: 机器翻译在低资源语言中广泛用于生成合成数据，但文化差异会导致语义标签在翻译过程中发生漂移，特别是在文化敏感领域。


<details>
  <summary>Details</summary>
Motivation: 研究文化对齐在机器翻译中对语义标签保持的影响，发现文化差异是导致标签漂移的关键因素。

Method: 通过跨文化敏感领域和中性领域的实验，分析不同机器翻译系统（包括现代大语言模型）对标签保持的影响。

Result: 发现机器翻译系统（包括LLMs）会在翻译过程中诱导标签漂移，特别是在文化敏感领域；LLMs编码的文化知识会放大这种漂移；语言间的文化相似性是标签保持的关键决定因素。
...
TL;DR: SymCode是一个神经符号框架，将数学问题解决重构为使用SymPy库的可验证代码生成任务，显著提升了LLM在数学推理中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理中经常失败，基于文本的生成导致未经验证和算术上不可靠的解决方案，缺乏确定性验证机制。
...
Conclusion: 通过将LLM推理建立在确定性符号引擎上，SymCode代表了在形式化领域实现更准确和可信赖AI的关键一步。
...
<details>
  <summary>Details</summary>
Motivation: Trainium加速器虽然为AI工作负载提供了成本效益和高性能解决方案，但其脉动阵列架构和特殊数据布局要求使得实现高性能具有挑战性。

Method: 基于内核融合和新型缓存策略，针对Trainium定制化设计矩阵乘法内核，减少软件管理内存层次中的数据移动，最大化SRAM带宽，避免昂贵的矩阵转置。

Result: 在9个数据集和4个最新LLM上的评估显示，相比AWS现有实现，矩阵乘法内核平均加速1.35倍（最高2.22倍），端到端LLM推理平均加速1.66倍（最高2.49倍）。

Conclusion: 针对Trainium架构的定制化优化技术能够显著提升矩阵乘法性能和LLM推理效率。
...
TL;DR: AttnCache通过检索和重用相似的注意力图来加速LLM推理的prefill阶段，在CPU和GPU上分别实现了1.2倍和1.6倍的端到端加速，注意力计算速度提升2-3倍。


<details>
  <summary>Details</summary>
Motivation: 许多实际工作负载（如分类、问答、推荐等）仅依赖LLM推理的prefill阶段，其中自注意力计算因其二次复杂度成为主要性能瓶颈。

Method: 基于观察到语义不同的句子在不同层和头中产生相似注意力图的洞察，提出AttnCache框架，通过注意力图记忆数据库和高效缓存与相似性搜索技术来重用预缓存的注意力图。

Result: 实验结果显示，AttnCache在CPU上平均实现1.2倍端到端和2倍注意力加速，在GPU上实现1.6倍端到端和3倍注意力加速，精度损失可忽略。

Conclusion: AttnCache通过重用相似注意力图有效加速LLM的prefill阶段推理，显著降低自注意力计算开销。
...
TL;DR: 提出监督强化学习(SRL)框架，将问题解决重新定义为生成逻辑动作序列，通过内部推理独白和基于专家动作相似性的平滑奖励，解决小规模开源模型在多步推理任务中的学习困难。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多步推理任务上表现不佳，小规模开源模型面临两个问题：RLVR在正确解极少被采样时失败，SFT容易通过逐词模仿长演示而过度拟合。

Method: SRL将问题解决建模为生成逻辑动作序列，训练模型在提交每个动作前生成内部推理独白，基于模型动作与SFT数据集中专家动作的相似性提供逐步平滑奖励。

Result: SRL使小模型能够学习SFT或RLVR无法学习的挑战性问题，先用SRL初始化训练再用RLVR精炼可获得最强性能，且在智能软件工程任务上有效泛化。

Conclusion: SRL是一个稳健且通用的面向推理的LLM训练框架，通过结合监督学习和强化学习的优势，解决了现有方法在多步推理任务中的局限性。
...
TL;DR: PORTool是一种强化学习方法，通过鼓励工具使用LLM探索多种正确轨迹来提升动态环境中的性能
...
Method: 使用强化学习：1)为查询生成多个轨迹形成树状结构；2)基于产生正确答案和成功工具调用的能力分配步骤奖励；3)计算分叉相对优势和轨迹相对优势来训练LLM

Result: 实验使用17种工具处理用户查询，涵盖时间敏感和不变主题。PORTool在最终准确率和工具调用步骤数量上显著优于其他训练方法

Conclusion: PORTool通过强化学习探索多种轨迹，显著提升了工具使用LLM在动态环境中的性能
...
TL;DR: 本文分析了跨语言对齐可能导致的文化擦除问题，提出了评估框架和新的推理时方法来解决这一权衡问题。


<details>
  <summary>Details</summary>
Motivation: 跨语言对齐虽然能促进多语言表示对齐，但可能导致文化擦除，即模型无法提供与查询语言相关的文化情境化响应。

Method: 提出了转移-本地化平面评估框架，并基于模型内部表示分析发现，开发了Surgical Steering方法，通过在不同模型层应用定向激活引导来分离事实转移和文化特定知识。

Result: 评估发现现有CLA方法在六种语言中都一致地以牺牲文化本地化为代价来改善事实转移，而新方法能更好地平衡这两个竞争维度。

Conclusion: 通过在不同模型层分别引导事实转移和文化特定知识，可以克服当前对齐技术的局限性，实现更好的平衡。
...
TL;DR: 开发并验证了一个基于自然语言处理的系统来识别放射学报告中的偶发甲状腺发现，发现7.8%的患者有偶发甲状腺发现，这些发现与甲状腺癌过度诊断相关。


<details>
  <summary>Details</summary>
Motivation: 随着影像学检查的增加，偶发甲状腺发现的检出率上升，但其流行率、特征和临床后果尚不明确，需要系统评估。

Method: 回顾性队列研究，使用基于transformer的自然语言处理流程识别多模态影像报告中的偶发甲状腺发现，并提取结节特征。

Result: 在115,683名患者中，7.8%有偶发甲状腺发现，其中92.9%为结节。这些发现与更高的甲状腺结节诊断、活检、甲状腺切除术和甲状腺癌诊断几率相关。

Conclusion: 偶发甲状腺发现常见且与导致小、低风险癌症检测的级联效应密切相关，强调了其在甲状腺癌过度诊断中的作用以及标准化报告和更有选择性随访的必要性。
...
TL;DR: QCoder Benchmark是一个评估大语言模型在量子编程任务上的框架，通过量子模拟器提供电路深度、执行时间和错误分类等反馈，实验显示推理模型表现优于人类代码。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在与硬件设备交互的领域（如量子编程）中代码生成能力评估不足的问题。

Method: 引入QCoder Benchmark评估框架，支持量子模拟器环境评估，收集真实编程竞赛中的人类代码作为对比基准。

Result: GPT-4o准确率仅18.97%，而推理模型如o3达到78%准确率，优于人类代码平均成功率39.98%。

Conclusion: 量子编程对LLMs具有挑战性，推理模型表现更佳，QCoder Benchmark为相关研究提供了数据集和评估API。
...
TL;DR: 提出1PNS训练范式，通过增加推理路径多样性来提升模型性能，引入RPD指标衡量思维链差异，实验显示在pass@16上平均提升2.80%。


<details>
  <summary>Details</summary>
Motivation: 传统1P1S训练导致模型输出多样性不足，限制了测试时扩展的效果。

Method: 提出1PNS训练范式，引入RPD指标衡量思维链差异，选择多样化解决方案进行微调。

Result: 在Qwen3-4B-Base上实验，pass@16平均提升2.80%，AIME24上提升4.99%。

Conclusion: 1PNS训练能有效提升模型推理多样性，增强测试时扩展效果。
...
TL;DR: 使用LLM和提示工程分析说服技术与篇章关系之间的联系，创建了包含PTs和DRs标注的银数据集，发现六种篇章关系在说服性文本中起关键作用。


<details>
  <summary>Details</summary>
Motivation: 由于没有同时标注说服技术和篇章关系的数据集，研究旨在探索两者之间的关系，以帮助检测网络宣传和错误信息。

Method: 基于SemEval 2023 Task 3数据集，使用4个LLM和10种提示开发40个DR分类器，通过集成模型创建5个银数据集进行统计分析。

Result: 统计显示六种篇章关系（Cause、Purpose、Contrast、Cause+Belief、Concession、Condition）在说服性文本中至关重要，特别是在Loaded Language、Exaggeration/Minimisation、Repetition和Doubt等说服技术中。

Conclusion: 该研究为检测网络宣传和错误信息提供了重要见解，并增进了对有效沟通的理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有SSM/GRM方法通常只模拟单个注意力头，可能限制表达能力，需要更有效的多头注意力模拟机制。

Method: 使用混合专家(MoE)实现，不仅在通道混合MLP块中，还在时间混合SSM核中实现多个"注意力头"。

Result: 在语言建模和下游评估中优于类似规模的Transformer和SSM架构，更大版本在万亿token训练中证实可扩展性和优越性能，在移动设备和GPU上显示良好的运行速度和资源使用。

Conclusion: MossNet是高效、高性能循环LLM架构的有前景新方向。
...
TL;DR: 提出相似性-距离-幅度(SDM)语言模型，通过微调使生成内容落在高概率区域，使用SDM激活层进行二元分类来评估指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 改进现有预训练语言模型的指令遵循能力，减少模型拒绝回答的情况，提高统计效率。

Method: 对预训练的仅解码器Transformer模型进行监督微调，使用SDM激活层估计监督下一个token损失的基变换，结合对比输入编码方案和在线生成的硬负例。

Result: 与强监督基线相比，SDM模型减少了拒绝回答的情况，提高了统计效率。

Conclusion: SDM语言模型能有效提升预训练模型的指令遵循能力，通过监督微调和SDM激活层实现更好的校准和高概率生成。
...
Method: 通过系统地将基准问题转换为多种指令风格，并引入跨响应相似性(CRS)来测量风格自一致性。
...
TL;DR: 本文提出Token Timestep Allocation (TTA)方法来解决扩散语言模型中的更新遗忘问题，通过为不同token分配不同的时间步调度，实现更稳定可控的文本生成。
...
Method: 提出Token Timestep Allocation (TTA)方法，通过为每个token分配时间步调度来实现软语义排序：关键token早期冻结，不确定token持续细化。该方法可在推理时实现，支持固定策略或自适应策略。
...
TL;DR: WIMHF方法使用稀疏自编码器解释人类反馈数据，识别可解释的特征来理解偏好数据的编码内容，揭示不同数据集中的偏好差异，并可用于数据筛选和个性化建模。


<details>
  <summary>Details</summary>
Motivation: 人类反馈可能以不可预测和不良的方式改变语言模型，但实践者缺乏对反馈数据编码内容的清晰理解。现有方法需要预先指定假设，难以自动提取相关特征。

Method: 引入WIMHF方法，使用稀疏自编码器解释反馈数据，表征数据集能够测量的偏好和标注者实际表达的偏好。

Result: 在7个数据集上，WIMHF识别出少量人类可解释的特征，这些特征解释了黑盒模型偏好预测信号的大部分。发现不同数据集偏好差异显著，并识别出潜在的不安全偏好。

Conclusion: WIMHF为实践者提供了一种以人为中心的分析方法，帮助更好地理解和利用偏好数据，实现有效的数据筛选和细粒度个性化。
...
Method: 提出GlobalRAG框架：通过块级检索保持结构连贯性，集成LLM驱动的智能过滤器消除噪声文档，并加入聚合模块进行精确符号计算。

Result: 在Qwen2.5-14B模型上，GlobalRAG达到6.63 F1分数，而最强基线仅为1.51 F1，验证了方法的有效性。

Conclusion: 全局RAG是一个重要但被忽视的能力，现有方法在此类任务上表现较差，GlobalRAG框架为解决这一挑战提供了有效方案。
...
TL;DR: 通过将语用学理论作为提示提供给语言模型，可以显著提升其在理解隐含含义任务上的表现，相比基准方法最高提升9.6%。


<details>
  <summary>Details</summary>
Motivation: 准确理解隐含含义在人类交流中至关重要，语言模型也需要具备这种能力。研究探索如何通过语用学理论指导来提升语言模型的语用推理能力。

Method: 提出将语用学理论（如格赖斯语用学和关联理论）概述作为提示，引导语言模型进行逐步推理以得出最终解释。

Result: 相比仅提示中间推理的基准方法（0-shot Chain-of-Thought），该方法使语言模型在语用推理任务上得分最高提升9.6%。即使在提示中仅提及语用学理论名称，在较大模型中也能带来1-3%的性能提升。

Conclusion: 提供语用学理论作为提示是一种有效的上下文学习方法，能够显著提升语言模型理解隐含含义的能力。
...
TL;DR: 本文研究发现预训练语言模型在识别外来词方面表现不佳，即使在有明确指令和上下文信息的情况下，模型也难以区分外来词和母语词汇。


<details>
  <summary>Details</summary>
Motivation: 研究预训练语言模型是否具备类似人类识别外来词的能力，特别是在双语社区中，主导语言持续对少数民族语言施加词汇影响的情况下。

Method: 评估了多个模型在10种语言上的表现，使用明确指令和上下文信息来测试模型区分外来词和母语词汇的能力。

Result: 模型在外来词识别任务上表现不佳，这证实了现代NLP系统存在偏向外来词而非母语等效词的偏见。

Conclusion: 研究结果对开发少数民族语言的NLP工具以及在主导语言词汇压力下支持语言保护具有重要启示。
...
TL;DR: 本文对知识蒸馏在多语言视觉语言模型中的行为进行了实证研究，分析了五种蒸馏方法在模型压缩时对跨语言表示一致性和下游性能稳定性的影响。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在不同语言间表现不均，模型尺寸减小时问题更严重。知识蒸馏在将知识从大模型迁移到小模型方面有潜力，但在多语言环境中的应用研究不足。

Method: 研究了CLIP和SigLIP2模型上的五种蒸馏方法，评估了领域内检索和领域外视觉问答任务，分析了模型压缩对跨语言表示一致性的影响。

Result: 发现某些配置在模型尺寸减半时仍能保持甚至提升多语言检索鲁棒性，但其他配置无法维持跨任务稳定性，揭示了仅靠聚合准确率无法反映的设计敏感权衡。

Conclusion: 知识蒸馏在多语言视觉语言模型压缩中具有潜力，但需要仔细设计以平衡不同任务间的性能稳定性，单纯追求准确率可能掩盖重要的性能权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于外部信号（如token概率、熵或自评估）的集成解码方法在训练后可能校准不佳，而内部神经元激活包含更丰富的信息。

Method: NAD方法基于三个发现：外部信号是内部动态的低维投影；正确答案激活的独特神经元更少；正确答案的激活在样本间一致性更强。通过分析神经元激活的稀疏性和跨样本一致性来选择最佳候选。

Result: 在数学和科学基准测试中与多数投票相当；在开放式编码基准测试中优于Avg@64；通过早期停止减少99%的token使用，质量损失极小。

Conclusion: 内部信号为无标签集成解码提供了可靠、可扩展且高效的指导，支持早期正确性预测和激进早期停止。
...
TL;DR: 尽管LLMs在数字处理上经常出错，但研究发现它们学习到了系统化、高精度且可互换的数字表示，这些表示在隐藏状态和输入上下文中具有普遍性，可用于创建通用探针来追踪错误来源。


<details>
  <summary>Details</summary>
Motivation: 解释LLMs在数字嵌入表示准确性与输出错误之间的冲突，探索语言模型如何处理数字并量化这些机制的准确性下限。

Method: 分析不同语言模型的数字表示，创建通用探针来追踪信息流，识别导致输出错误的特定层。

Result: 发现尽管存在表面错误，但不同LLMs学习到了可互换、系统化且高精度的数字表示，这些表示在隐藏状态和输入上下文中具有普遍性。
...
TL;DR: 对ChatGPT Atlas在网页交互能力的早期评估，发现在逻辑推理任务中表现出色，但在需要精确时机和运动控制的实时游戏中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 虽然ChatGPT Atlas在信息检索任务中已展示能力，但其在动态交互环境中的性能仍需探索。

Method: 使用浏览器游戏作为测试场景，包括T-Rex Runner、数独、Flappy Bird和Stein.world，以游戏内性能分数作为量化指标。

Result: Atlas在数独等逻辑推理任务中表现强劲，完成速度显著快于人类基准，但在需要精确时机和运动控制的实时游戏中表现不佳，往往无法通过初始障碍。

Conclusion: Atlas展示出强大的分析处理能力，但在需要实时交互的动态网页环境中仍存在明显局限性。
...
Method: ACER首先为学科生成目录，然后基于布鲁姆分类法创建问答对，构建全面的合成语料库，采用交错课程安排进行持续预训练。

Result: 在Llama 3.2模型上，ACER在微观经济学等挑战性领域提升准确率5个百分点，所有目标领域平均提升3个百分点，并在非目标领域实现0.7个百分点的知识迁移。

Conclusion: ACER提供了一种可扩展且有效的方法，能够显著缩小LLMs在关键专业领域的性能差距，同时保持通用推理能力。
...
TL;DR: MisSynth是一个使用检索增强生成(RAG)生成合成谬误样本的管道，通过轻量级微调显著提升LLM识别科学错误信息的能力，在MISSCI数据集上实现了35%以上的F1分数绝对提升。


<details>
  <summary>Details</summary>
Motivation: 健康相关的错误信息非常普遍且有害，特别是那些扭曲或误解科学发现的言论难以识别。现有标注数据有限，需要有效方法来增强LLM识别谬误论点的能力。

Method: 提出MisSynth管道，应用检索增强生成(RAG)生成合成谬误样本，然后用这些样本对LLM进行轻量级微调。

Result: 微调后的模型相比原始基线有显著准确率提升，LLaMA 3.1 8B模型在MISSCI测试集上实现了超过35%的F1分数绝对提升。

Conclusion: 通过合成谬误数据增强有限标注资源，可以显著提升LLM在真实世界科学错误信息分类任务中的零样本性能，即使计算资源有限也能实现良好效果。
...
TL;DR: 提出了一种基于交互的自动团队构建框架，通过分析语言模型之间的对话语义一致性来发现协同模型集群，无需事先了解模型内部架构或训练数据。


<details>
  <summary>Details</summary>
Motivation: 多智能体方法虽然能超越单个模型能力，但团队组成的协同性至关重要。由于大多数模型的不透明性，难以了解其内部特性以进行有效协作。

Method: 构建'语言模型图'来映射模型间关系，基于成对对话的语义一致性，然后应用社区检测识别协同模型集群。

Result: 实验表明该方法能发现功能一致的模型群组，反映其潜在专长。特定主题引导的对话识别出的协同团队在下游基准测试中优于随机基线，与基于已知模型专长手动构建的团队准确率相当。
...
TL;DR: 该论文研究了在科学写作中使用预训练语言模型和大语言模型进行话语关系分类，重点关注上下文对分类任务的帮助作用。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在科学工作流中的广泛应用，需要利用话语层面的信息为AI生成的科学主张提供证据支持，而理解科学写作的话语结构是实现这一目标的第一步。

Method: 使用预训练语言模型和大语言模型进行话语关系分类实验，特别关注上下文（定义为话语结构）对分类任务的影响。

Result: 实验表明，基于话语结构的上下文通常对话语关系分类任务有帮助，并分析了哪些科学话语关系类型最能从上下文中受益。
...
TL;DR: OmniEduBench是一个全面的中文教育基准测试，包含24.602K高质量问答对，涵盖知识和素养两个维度，共61个学科和11种题型。实验显示主流LLMs在教育领域仍有显著差距，特别是在素养维度上落后人类近30%。
...
Method: 构建包含24.602K问答对的综合性基准，分为知识维度(18.121K)和素养维度(6.481K)，每个维度细分为6个类别，覆盖61个学科和11种题型。

Result: 在知识维度，仅Gemini-2.5 Pro超过60%准确率；在素养维度，表现最佳的QWQ模型仍落后人类智能近30%。

Conclusion: LLMs在教育应用方面仍有巨大改进空间，特别是在素养能力方面面临显著挑战。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能优异，但部署受到带宽和计算需求的限制。现有方法如剪枝和低秩逼近各自表现良好，但两者的协同作用尚未充分探索。

Method: 基于理论分析，将低秩逼近和稀疏优化统一为一个问题，并通过迭代优化算法求解，结合两种方法的优势。

Result: 在LLaMA和Qwen2.5模型(7B-70B)上的实验显示，SSLC无需额外训练步骤，始终优于单独方法，达到最先进结果。Qwen2.5压缩50%无性能损失，获得至少1.63倍加速。

Conclusion: SSLC为大语言模型的高效部署提供了实用解决方案，通过稀疏化和低秩逼近的协同作用实现高效压缩。
...
TL;DR: 提出了贝叶斯网络LLM融合框架，通过概率机制整合三个LLM的预测结果进行情感分析，在三个金融语料库上比基线LLM准确率提升约6%。
...
Result: 在三个具有不同语言和上下文特征的人工标注金融语料库上评估，相比基线LLM，准确率一致提升约6%。

Conclusion: BNLF框架对数据集变异性具有鲁棒性，概率融合方法对可解释的情感分类有效。
...
TL;DR: 研究评估了使用多个LLM代理集合来评估AI分诊工具的性能，发现LLM集合比单一LLM提供更可靠的结果。


<details>
  <summary>Details</summary>
Motivation: 确定多个LLM代理集合是否能够比单一LLM更可靠地评估基于像素的AI分诊工具。

Method: 分析了14家医院的29,766个非对比CT头部检查，使用8个开源LLM模型和GPT-4o评估颅内出血检测AI工具的性能，并手动审查了1,726个案例。

Result: Llama3.3:70b和GPT-4o表现最佳（AUC=0.78）。LLM集合（Top-3、Full-9、Consensus）在MCC指标上优于单一GPT-4o，且集合之间无显著差异。

Conclusion: 中等至大型开源LLM集合为临床AI分诊工具的回顾性评估提供了比单一LLM更一致和可靠的方法。
...
TL;DR: 本文通过消融实验分析了CORE-KG框架的两个关键组件（共指消解模块和结构化提示）在构建法律知识图谱中的贡献，发现共指消解主要减少节点重复，结构化提示主要降低噪声节点。


<details>
  <summary>Details</summary>
Motivation: 解决从复杂法律文本中自动构建知识图谱时面临的节点重复和噪声问题，现有LLM方法缺乏引导式提取和共指消解能力。

Method: 对CORE-KG框架进行系统性消融研究，分别移除共指消解模块和结构化提示，量化分析两个组件对节点重复率和噪声率的影响。

Result: 移除共指消解导致节点重复率增加28.32%，噪声节点增加4.32%；移除结构化提示导致节点重复率增加4.34%，噪声节点增加73.33%。

Conclusion: 共指消解和结构化提示在LLM知识图谱构建中各有侧重，为设计复杂法律文本的结构化提取管道提供了实证依据。
...
Method: 将任务构建为零样本分类问题，在单词级别操作，从动态生成的候选集中为每个未变音单词选择最合适的变音模式，使用希伯来视觉语言模型将未变音文本作为图像处理。

Result: 在"oracle"设置下（正确变音形式保证在候选集中），系统达到高准确率；通过架构增强和优化训练方法显著提高了系统的泛化能力。

Conclusion: 视觉表示在准确和自动化的希伯来语变音符号恢复方面具有巨大潜力，系统无需依赖复杂的显式语言分析即可有效执行变音。
...
TL;DR: 该研究发现Transformer语言模型中的线性关系解码器并非编码特定关系，而是提取重复的粗粒度语义属性，这解释了它们的高压缩性和有限的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩展Hernandez等人[2023]的单关系发现，系统研究多个关系解码器的组织结构，解释其高压缩性的原因。

Method: 开发交叉评估协议，将每个线性解码器应用于其他关系的主题，分析解码器的语义属性提取能力，并使用简单的三阶张量网络进行压缩测试。

Result: 关系解码器集合可通过三阶张量网络高度压缩而不显著损失精度；解码器提取的是重复的粗粒度语义属性而非特定关系；泛化仅限于语义相近的新关系。
...
TL;DR: InfoFlow框架通过子问题分解、失败引导提示和双智能体精炼来解决强化学习中奖励密度低的问题，显著提升了搜索性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习在深度搜索场景中面临奖励密度低的问题，智能体需要付出大量探索成本却只能获得稀疏的最终奖励，这限制了RLVR方法的应用效果。

Method: 1) 子问题分解：将长程任务分解为子任务并分配过程奖励；2) 失败引导提示：向停滞轨迹注入纠正指导；3) 双智能体精炼：使用研究者和精炼者双智能体架构压缩感知轨迹。

Result: 在多个智能搜索基准测试中，InfoFlow显著优于强基线方法，使轻量级LLM能够达到与先进专有LLM相当的性能。

Conclusion: InfoFlow通过提高奖励密度有效解决了深度搜索中的探索成本问题，为强化学习在复杂任务中的应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型因自回归设计和大型规模导致的推理延迟问题，现有方法如EAGLE-2和EAGLE-3忽略了GPU设备和批大小等关键系统变量的影响。

Method: 提出CAST动态树解码方法，综合考虑推理成本因素（如GPU配置和批大小）来动态优化树结构。

Result: 在6个不同任务和6个不同LLM上的实验表明，该方法比传统解码方法快达5.2倍，相比现有最先进技术普遍提升5%到20%。

Conclusion: CAST通过考虑系统变量动态优化树结构，有效提升大语言模型的推理效率，显著优于现有方法。
...
TL;DR: SlideAgent是一个用于理解多模态、多页面、多布局文档（特别是幻灯片）的代理框架，通过三层专业化推理（全局、页面、元素）构建结构化表示，在推理时选择性激活专业代理进行多级推理。


<details>
  <summary>Details</summary>
Motivation: 当前系统在处理复杂多页面视觉文档时存在困难，特别是在元素和页面的细粒度推理方面，而大语言模型在文档理解方面提供了机会。

Method: 采用专业化代理框架，将推理分解为全局、页面和元素三个层级，构建结构化、查询无关的表示，在推理时选择性激活专业代理进行多级推理。

Result: 实验表明SlideAgent相比专有模型整体提升7.9%，相比开源模型整体提升9.8%。

Conclusion: SlideAgent框架能够有效处理多页面视觉文档的理解任务，在细粒度推理方面表现优异。
...
TL;DR: 本文重新审视编码器-解码器架构的LLM，通过引入解码器专用LLM的最新方法进行增强，并与解码器专用LLM在不同规模下进行对比分析。


<details>
  <summary>Details</summary>
Motivation: 由于LLM研究快速从编码器-解码器架构转向解码器专用架构，缺乏从扩展角度进行的严谨对比分析，可能忽视了编码器-解码器模型的潜力。

Method: 使用前缀语言建模预训练RedLLM，使用因果语言建模预训练DecLLM，在150M到8B的不同模型规模上进行对比，使用RedPajama V1进行预训练，FLAN进行指令调优。

Result: RedLLM展现出有竞争力的扩展特性和强大性能。虽然DecLLM在预训练期间计算效率更高，但RedLLM具有相当的扩展和上下文长度外推能力。指令调优后，RedLLM在各种下游任务上达到相当甚至更好的结果，同时推理效率显著更高。
...
<details>
  <summary>Details</summary>
Motivation: 在数据敏感领域如医疗保健中，缺乏高质量领域特定训练语料限制了LLMs的专业化应用，而领域专家已将领域智慧提炼成本体规则

Method: 从原始模型中提取领域本体，使用两个核心本体规则检测不一致性，通过自蒸馏微调强化精炼后的知识

Result: 在医疗QA基准测试中，Llama3-8B-Instruct和Med42-v2模型相比未修改模型和领先监督基线持续表现更优，准确率提升高达3.7%
...
TL;DR: Kimi Linear是一种混合线性注意力架构，首次在各种场景下公平比较中超越完整注意力机制，核心是Kimi Delta Attention模块，通过改进的门控机制和高效的块状算法实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够超越传统完整注意力机制性能的线性注意力架构，同时显著减少KV缓存使用并提高解码吞吐量，特别是在长上下文场景中。

Method: 采用Kimi Delta Attention作为核心模块，扩展Gated DeltaNet并加入更细粒度的门控机制；使用专门的块状算法和Diagonal-Plus-Low-Rank变体矩阵来优化硬件效率。

Result: 3B激活参数的模型在相同训练方案下全面超越完整MLA，KV缓存减少75%，1M上下文解码吞吐量提升6倍，在所有评估任务中表现更优。

Conclusion: Kimi Linear可以作为完整注意力架构的直接替代方案，提供更优越的性能和效率，特别是在长输入输出任务中，并开源了相关实现和模型。
...
TL;DR: AutoDeco是一种新型架构，通过在标准Transformer上添加轻量级头部，使模型能够动态预测上下文特定的温度和top-p值，实现真正的端到端生成。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的"端到端"标签存在误导，实际上依赖需要手动调优的非可微解码过程，如温度和top-p参数。

Method: 在标准Transformer基础上添加轻量级头部，在每个步骤动态预测上下文特定的温度和top-p值以及下一个token的logits，将解码转变为参数化的token级过程。

Result: 在八个基准测试中，AutoDeco不仅显著优于默认解码策略，而且达到了与通过"黑客测试集"得到的oracle调优基线相当的性能。模型还展现出基于指令的解码控制能力。

Conclusion: AutoDeco开启了可引导和交互式LLM解码的新范式，使模型能够根据自然语言指令调整解码策略。
...
TL;DR: 该研究分析了LLM在训练过程中价值对齐的动态变化，发现监督微调阶段主要确立模型价值观，后续偏好优化很少重新对齐这些价值观，不同偏好优化算法会导致不同的价值对齐结果。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在社会中扮演越来越重要的角色，需要研究它们如何学习表达人类价值观。现有工作大多关注完全训练好的模型，忽视了训练过程中价值观学习的动态变化。

Method: 使用Llama-3和Qwen-3不同规模的模型，结合流行的监督微调和偏好优化数据集与算法，分析价值对齐在训练过程中的变化，并使用合成偏好数据集进行受控实验。

Result: 监督微调阶段一般确立模型价值观，后续偏好优化很少重新对齐这些价值观；即使偏好数据保持不变，不同的偏好优化算法也会导致不同的价值对齐结果。

Conclusion: 研究结果为理解价值观在训练过程中的学习机制提供了可操作的见解，有助于指导数据整理以及选择模型和算法来改进模型与人类价值观的对齐。
...
TL;DR: AMO-Bench是一个高级数学推理基准，包含50个奥林匹克竞赛难度或更高的人类编写问题，用于评估大型语言模型的数学推理能力。
...
Method: 创建包含50个原创问题的基准，所有问题都经过专家交叉验证达到IMO难度标准，且仅需最终答案以便自动评分。

Result: 在26个LLMs上的实验结果显示，最佳模型准确率仅为52.4%，大多数模型低于40%，但发现了测试时计算增加时的有前景的扩展趋势。

Conclusion: AMO-Bench突显了当前LLMs在数学推理方面的显著改进空间，为推进语言模型推理能力的研究提供了有效工具。
...
TL;DR: Gistify任务要求编码LLM从完整代码库中提取核心功能，生成一个最小化的自包含文件来复制特定命令的输出。
...
Conclusion: Gistify任务需要代码库结构理解、执行流程建模和大规模代码补丁生成能力，现有模型在这方面仍有局限。
...
### [59] [Rethinking Text-to-SQL: Dynamic Multi-turn SQL Interaction for Real-world Database Exploration](https://arxiv.org/abs/2510.26495)
*Linzhuang Sun,Tianyu Guo,Hao Liang,Yuying Li,Qifeng Cai,Jingxuan Wei,Bihui Yu,Wentao Zhang,Bin Cui*

Main category: cs.DB

TL;DR: 提出了DySQL-Bench基准测试，用于评估Text-to-SQL模型在动态交互场景下的性能，通过自动化流水线生成多轮对话任务，模拟真实世界用户意图演变的情况。


<details>
  <summary>Details</summary>
Motivation: 现有的Text-to-SQL系统在静态单轮任务中表现良好，但在真实交互场景中，用户意图会随时间演变，需要多轮查询优化，现有系统在这方面存在不足。

Method: 采用自动化两阶段流水线：任务合成和验证。基于数据库表的树状结构指导LLM生成任务，然后进行交互导向的过滤和专家验证。还提出了多轮评估框架模拟真实交互。

Result: 构建了覆盖13个领域、共1,072个任务的基准测试。GPT-4o仅达到58.34%的整体准确率和23.81%的Pass@5指标，表明基准测试具有挑战性。

Conclusion: DySQL-Bench填补了动态Text-to-SQL评估的空白，揭示了现有模型在交互场景中的局限性，为未来研究提供了重要基准。
...
TL;DR: 该研究评估了时空建模和空间注意力机制在深度学习模型中对水下物体检测的有效性，比较了YOLOv5、T-YOLOv5和带CBAM的T-YOLOv5的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决动态海洋环境中水下物体检测的挑战，特别是在突然运动、部分遮挡和渐变运动条件下，提高检测准确性和可靠性。

Method: 研究分为两个阶段：第一阶段评估时间增强的T-YOLOv5与标准YOLOv5的性能；第二阶段开发了带卷积块注意力模块(CBAM)的T-YOLOv5增强版本。

Result: 测试结果显示，YOLOv5的mAP@50-95为0.563，而T-YOLOv5和带CBAM的T-YOLOv5分别达到0.813和0.811，在复杂物体检测中表现出更高的准确性和泛化能力。

Conclusion: T-YOLOv5相比标准模型显著提高了检测可靠性，而带CBAM的T-YOLOv5在挑战性场景中进一步提升了性能，但在简单场景中精度有所损失。
...
TL;DR: CAVE是首个真实世界视觉异常基准，支持异常描述、解释和证明三个开放任务，为评估视觉语言模型在异常检测和常识推理方面的能力提供认知科学启发的框架。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉中的异常检测仅限于工业缺陷或合成异常，无法捕捉真实世界异常的丰富性和不可预测性。人类能自然识别、推理和解释环境中的异常，需要建立更真实的基准。

Method: 引入CAVE基准，提供细粒度注释用于视觉定位和异常分类，基于异常的可视表现、复杂性、严重性和常见性进行分类。这些注释借鉴了认知科学研究中人类识别和解决异常的方式。
...
TL;DR: 该论文提出了AoT-PsyPhyBENCH基准，用于评估视觉语言模型在判断视频时间方向（前进或后退）方面的能力，发现现有模型表现接近随机猜测，远落后于人类表现。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在多模态任务中表现出色，但其对视频中时间信息的理解仍然薄弱且缺乏充分评估。作者希望通过判断时间方向这一简单但具有揭示性的挑战来探索这一差距。

Method: 引入AoT-PsyPhyBENCH基准，该基准使用与人类行为基线相同的刺激物，测试视觉语言模型在自然视频中推断时间方向的能力。对开放权重和专有、推理和非推理模型进行全面评估。

Result: 大多数模型表现接近随机猜测，即使在物理不可逆过程（如自由落体、扩散/爆炸）和因果手动动作（除法/加法）等人类几乎能瞬间识别的任务上，最佳模型也远远落后于人类准确率。

Conclusion: 当前多模态系统存在根本性差距：虽然它们能够捕捉丰富的视觉-语义关联，但缺乏时间连续性和因果理解所需的归纳偏差。作者发布代码和数据以促进视觉语言模型在物理和时间推理能力方面的进一步进展。
...
TL;DR: 论文发现LVLM自改进过程中的马太效应问题，并提出分布重塑和轨迹重采样策略来解决头尾数据不平衡问题，显著提升视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 发现大型视觉语言模型在自改进过程中存在马太效应：模型擅长处理简单查询（头数据）但难以应对复杂查询（尾数据），导致优化不平衡，阻碍模型进一步改进。

Method: 提出四种高效策略，从两个角度解决头尾不平衡问题：分布重塑和轨迹重采样，在探索-学习自改进过程中实现头尾数据重新平衡。

Result: 在Qwen2-VL-7B-Instruct和InternVL2.5-4B模型上的广泛实验表明，该方法持续提升视觉推理能力，平均比原始自改进方法高出3.86分。

Conclusion: 通过解决自改进过程中的头尾数据不平衡问题，提出的方法有效缓解了马太效应，突破了性能瓶颈，显著提升了模型的视觉推理能力。
...
TL;DR: 该论文评估了视频生成模型作为零样本推理器的能力，发现它们在短时空间一致性和局部动态方面表现良好，但在长期因果推理和几何约束方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 研究视频生成模型是否可以作为零样本推理器来处理具有挑战性的视觉推理场景，特别是关注领先的Veo-3模型。

Method: 通过构建MME-CoF基准数据集，在12个维度上系统评估视频模型的推理行为，包括空间、几何、物理、时间和具身逻辑等方面。

Result: 当前视频模型在短时空间一致性、细粒度定位和局部一致动态方面显示出有前景的推理模式，但在长期因果推理、严格几何约束和抽象逻辑方面仍存在限制。

Conclusion: 视频模型还不能作为独立的零样本推理器，但可以作为专用推理模型的补充视觉引擎，显示出令人鼓舞的潜力。
...
TL;DR: ORBIT是一个统一的推荐系统基准测试框架，包含公开数据集和隐藏测试集ClueWeb-Reco，旨在解决现有数据集和评估设置不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统研究受到数据集无法捕捉真实用户行为以及评估设置不一致导致结论模糊的限制。

Method: ORBIT提供标准化评估框架，包含可复现的数据分割和透明设置。ClueWeb-Reco是基于8700万高质量网页浏览序列的合成数据集，作为隐藏测试集评估模型泛化能力。

Result: 在公开基准测试中，推荐系统普遍有所改进，但个体表现差异较大。隐藏测试结果显示现有方法在大规模网页推荐中存在局限性，而LLM集成显示出改进潜力。
...
TL;DR: 提出MemEIC方法，用于大型视觉语言模型(LVLMs)的持续和组合知识编辑，解决现有方法只关注单一模态编辑的问题。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑技术通常只单独编辑视觉或语言模态，忽视了LVLMs的多模态特性和知识的持续更新需求，导致编辑效果不佳。

Method: 使用混合外部-内部编辑器，包含双外部记忆用于跨模态证据检索和双LoRA适配器实现各模态解耦参数更新，通过脑启发知识连接器进行组合推理。

Result: 实验表明MemEIC在复杂多模态问题上显著提升性能，有效保留先前编辑内容，为LVLMs的持续组合知识编辑设立了新基准。

Conclusion: MemEIC方法成功解决了多模态知识编辑的挑战，实现了持续和组合式的知识更新，为LVLMs的知识维护提供了有效解决方案。
...
TL;DR: 提出了SPECS框架，一种基于自蒸馏和偏好的冷启动方法，通过解耦多模态学习来替代传统的SFT冷启动，显著提升模型泛化能力和下游RL性能。
...
Method: SPECS框架：1) 通过自蒸馏生成内省偏好数据对；2) 基于偏好的训练，专注于学习浅层、可转移的表面形式标准；3) 将可验证奖励交给RL进行深度推理。

Result: 在多个多模态基准测试中表现优于强基线，MEGA-Bench提升4.1%，MathVista提升12.2%，同时减少分布内"卡顿"、改善探索、稳定训练并提高性能上限。

Conclusion: 基于偏好的冷启动方法比SFT方法具有更好的泛化能力，SPECS框架通过解耦学习有效提升了多模态模型的性能。
...
Method: 在自然语言推理和图像分类任务上进行广泛实验，分析知识蒸馏前后模型的去偏能力变化，并识别内部注意力模式和电路

Result: 发现知识蒸馏会削弱模型的去偏能力，去偏模型从教师知识注入中获益有限，不同偏见的鲁棒性变化显著

Conclusion: 提出了三种改进去偏方法可蒸馏性的解决方案，为理解KD工作机制和设计更好的去偏方法提供了见解
...
TL;DR: 论文对比了序列建模中参数化记忆的关联视图与几何视图，发现Transformer模型会自发形成实体间的几何结构，即使这种结构不比简单的关联查找更简洁。这种几何结构源于谱偏置，能够将复杂的推理任务简化为易于学习的几何任务。


<details>
  <summary>Details</summary>
Motivation: 传统上，参数化记忆被抽象为实体共现的暴力查找。作者质疑这种关联视图，探索记忆是否可能以几何形式存储，特别是当模型需要处理训练数据中未共现的实体关系时。

Method: 通过分析Transformer推理的实例，识别与局部共现存储不兼容的现象。连接Node2Vec分析几何结构的来源，研究谱偏置在缺乏各种压力时如何自然产生几何结构。

Result: 发现Transformer模型确实会自发形成实体间的几何结构，这种结构能够简化复杂的推理任务。几何结构的产生源于谱偏置，即使在没有额外优化压力的情况下也会自然出现。

Conclusion: 参数化记忆的几何视图应取代默认的关联直觉，这为知识获取、容量、发现和遗忘等领域提供了新的研究方向。几何结构的存在表明Transformer记忆具有更强的几何潜力。
...
TL;DR: AI在知识推理基准测试上进步迅速，但在实际经济价值和自动化方面的表现仍然有限。远程劳动指数(RLI)作为一个多行业基准，显示AI代理的自动化率仅为2.5%。


<details>
  <summary>Details</summary>
Motivation: 虽然AI在研究型基准测试中表现优异，但其经济价值和实际自动化能力尚不明确，需要建立更贴近现实应用的评估标准。

Method: 引入远程劳动指数(RLI)，这是一个涵盖多个行业的综合性基准，包含真实世界中有经济价值的项目，用于评估AI代理在实际环境中的端到端性能。

Result: AI代理在RLI上的表现接近最低水平，表现最佳的代理仅实现了2.5%的自动化率。

Conclusion: RLI为AI自动化讨论提供了实证基础，建立了追踪AI影响的共同标准，帮助利益相关者主动应对AI驱动的劳动力自动化。
...
TL;DR: 论文发现RL微调LLMs时的不稳定性源于BF16浮点精度引入的舍入误差，导致训练和推理策略不一致。简单改用FP16即可有效消除这种不匹配，带来更稳定的优化、更快的收敛和更强的性能。


<details>
  <summary>Details</summary>
Motivation: 解决RL微调LLMs时因训练和推理策略数值不匹配导致的不稳定性问题，揭示BF16精度是根本原因。

Method: 将广泛采用的BF16浮点精度改为FP16，这一改变简单易行，只需几行代码修改，无需改动模型架构或学习算法。
...
### [72] [SP-MCQA: Evaluating Intelligibility of TTS Beyond the Word Level](https://arxiv.org/abs/2510.26190)
*Hitomi Jin Ling Tee,Chaoren Wang,Zijie Zhang,Zhizheng Wu*

Main category: cs.SD

TL;DR: 提出了SP-MCQA评估方法，通过口语段落多选题问答来评估合成语音中关键信息的准确性，发现低WER不能保证高关键信息准确率，暴露了传统指标与实际可理解性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有TTS可理解性评估过度依赖WER等逐词准确性指标，无法捕捉真实世界语音的复杂性或反映人类理解需求，需要更贴近实际的高层次评估标准。

Method: 提出口语段落多选题问答(SP-MCQA)主观评估方法，并发布了8.76小时新闻风格基准数据集SP-MCQA-Eval，用于评估合成语音中关键信息的准确性。

Result: 实验表明低WER不能保证高关键信息准确率，即使最先进的模型在文本规范化和语音准确性方面仍有不足，传统指标与实际可理解性存在差距。

Conclusion: 当前许多系统在WER上表现出色但在实际可理解性上仍有不足，迫切需要更贴近生活的高层次评估标准来评估TTS系统的真实性能。
...
### [73] [FakeZero: Real-Time, Privacy-Preserving Misinformation Detection for Facebook and X](https://arxiv.org/abs/2510.25932)
*Soufiane Essahli,Oussama Sarsar,Imane Fouad,Anas Motii,Ahmed Bentajer*

Main category: cs.CR

TL;DR: FakeZero是一个完全客户端、跨平台的浏览器扩展，能够在用户滚动时实时标记Facebook和X平台上的不可靠帖子。采用本地化计算保护用户隐私，通过三阶段训练课程达到高精度检测，在资源受限环境下仍保持优异性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体以空前速度传播信息，加速了错误信息的传播并威胁公共话语。需要开发能够在保护用户隐私的同时，实时检测不可靠内容的工具。

Method: 完全客户端浏览器扩展，使用Chromium消息API进行本地计算。采用三阶段训练课程：基线微调、域自适应训练（包含焦点损失、对抗增强）和后训练量化。评估了DistilBERT-Quant和TinyBERT-Quant两种模型。

Result: 在239,000条帖子数据集上，DistilBERT-Quant模型（67.6 MB）达到97.1%宏F1、97.4%准确率和0.996 AUROC，中位延迟约103毫秒。TinyBERT-Quant变体（14.7 MB）保持95.7%宏F1和96.1%准确率，延迟降至约40毫秒。

Conclusion: 高质量假新闻检测在严格资源预算下是可行的，性能损失有限。该扩展可为政策制定者提供有价值的工具，并在用户同意下为研究人员收集大规模假新闻数据集开辟道路。
...
### [74] [SIRAJ: Diverse and Efficient Red-Teaming for LLM Agents via Distilled Structured Reasoning](https://arxiv.org/abs/2510.26037)
*Kaiwen Zhou,Ahmed Elgohary,A S M Iftekhar,Amin Saied*

Main category: cs.CR

TL;DR: SIRAJ是一个通用的红队测试框架，用于发现黑盒LLM代理的安全漏洞。它采用动态两步流程生成多样化测试用例，并通过迭代构建对抗攻击来优化测试效果。
...
Method: 采用动态两步流程：首先生成覆盖各种风险结果和工具使用轨迹的种子测试用例，然后基于执行轨迹迭代构建和优化模型对抗攻击。使用模型蒸馏方法训练更小的红队模型。

Result: 种子测试用例生成使风险结果和工具调用轨迹的覆盖率提高了2-2.5倍。蒸馏后的8B红队模型攻击成功率提高了100%，超越了671B的Deepseek-R1模型。

Conclusion: SIRAJ框架在多样化评估代理设置中验证了迭代框架、结构化推理和红队模型泛化能力的有效性。
...
### [75] [PVMark: Enabling Public Verifiability for LLM Watermarking Schemes](https://arxiv.org/abs/2510.26274)
*Haohua Duan,Liyao Xiang,Xin Zhang*

Main category: cs.CR

TL;DR: PVMark是一个基于零知识证明的插件，使LLM水印检测过程可公开验证，解决了当前水印方案因密钥保密性导致的信任问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM水印方案存在信任问题：非公开的水印检测无法证明其检测过程的真实性。这是由于检测密钥既不能公开（否则会被攻击者利用），也不能完全保密（否则检测过程不透明）。

Method: 基于零知识证明构建水印检测的'正确执行'证明，包括映射、随机数生成、比较和求和等约束条件。实现了多种变体，涵盖三种水印方案、三种哈希函数和四种ZKP协议。

Result: PVMark在多种情况下都能有效工作，能够在不影响水印性能的前提下，高效地实现最先进LLM水印方案的公开可验证性。
...
### [76] [Nexus: Execution-Grounded Multi-Agent Test Oracle Synthesis](https://arxiv.org/abs/2510.26423)
*Dong Huang,Mingzhe Du,Jie M. Zhang,Zheng Lin,Meng Luo,Qianru Zhang,See-Kiong Ng*

Main category: cs.SE

TL;DR: Nexus是一个多智能体框架，通过专业代理的协作审议、验证和迭代自优化来生成测试预言，显著提升了测试预言准确性和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决非回归测试中测试预言生成的长期挑战，目标是生成能够准确判断被测函数是否按预期行为的测试预言。

Method: 采用多智能体框架，包含四个体现不同测试理念的专业代理进行协作审议，通过验证阶段生成候选实现并在沙箱中执行测试，失败时启动自动自优化循环。

Result: 在七个基准测试中显著优于现有方法，LiveCodeBench上测试预言准确率从46.30%提升至57.73%，HumanEval上错误检测率从90.91%提升至95.45%，自动程序修复成功率从35.23%提升至69.32%。

Conclusion: Nexus框架通过多智能体协作和迭代自优化，有效解决了测试预言生成问题，显著提升了测试准确性和下游任务性能。
...
### [77] [SecureReviewer: Enhancing Large Language Models for Secure Code Review through Secure-aware Fine-tuning](https://arxiv.org/abs/2510.26457)
*Fang Liu,Simiao Liu,Yinghao Zhu,Xiaoli Lian,Li Zhang*

Main category: cs.SE

TL;DR: SecureReviewer是一个专门针对安全代码审查的LLM方法，通过构建专用数据集、安全感知微调和RAG技术，提升LLM识别和解决安全问题的能力，并提出了SecureBLEU评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有自动化代码审查方法主要关注通用目的，在识别和解决安全相关问题方面效果不足，且面临数据稀缺和评估指标不充分等挑战。

Method: 构建安全代码审查专用数据集，采用安全感知微调策略训练LLM，集成RAG技术减少幻觉，并引入SecureBLEU评估指标。

Result: 实验结果显示SecureReviewer在安全漏洞检测准确性和生成审查评论的整体质量及实用性方面优于现有最先进基线方法。
...
TL;DR: 提出人机协作框架，从仅标签标注中推断思维轨迹，通过拒绝采样方法重构推理过程，用于微调LLM评估器和优化标注指南，显著提升LLM与人类评估的一致性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在主观评估任务中可靠性不足的问题，因为人类判断涉及超出标注标签的细微推理，而思维轨迹难以收集和整理。

Method: 使用人机协作框架和拒绝采样方法从标签标注中推断思维轨迹，应用于微调开源LLM评估器和优化专有LLM评估器的标注指南。

Result: 在多个数据集上，该方法显著提高了LLM与人类评估的一致性，优化的标注指南也提升了不同LLM模型间的一致性。

Conclusion: LLM可以作为人类思维轨迹的实用代理，将仅标签语料库扩展为思维轨迹增强资源，从而提高LLM评估器的可靠性。
...
TL;DR: 提出了一个基于角色的偏好建模框架，通过聚合多个基于评分标准的评判输出来解决LLM评判器与人类偏好对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决LLM评判器难以校准、存在评分标准敏感性、偏见和不稳定性等问题，这对于构建可靠的RLHF奖励模型和有效的模型路由系统至关重要。

Method: 开发了基于角色的偏好标签大规模合成方法，并实现了两种聚合器：广义加性模型(GAM)和多层感知器(MLP)来聚合多个评分标准条件下的评判输出。

Result: 评估了该方法相对于简单基线的性能，并通过案例研究检验了其对人类和LLM评判偏见的鲁棒性。

Conclusion: 该框架为建模多样化、基于角色的偏好提供了一种有效方法，能够更好地对齐LLM评判器与人类偏好。
...
TL;DR: 提出Reasoning Curriculum两阶段课程学习法：第一阶段通过数学领域RL训练推理能力，第二阶段通过多领域联合RL迁移和巩固推理技能。该方法简单通用，在多个模型上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习主要关注数学和代码领域，需要开发能够跨领域迁移推理能力的通用方法。

Method: 两阶段课程学习：1）数学领域RL冷启动+可验证奖励训练；2）多领域联合RL迁移和巩固技能。无需专门奖励模型，仅需标准可验证性检查。

Result: 在Qwen3-4B和Llama-3.1-8B模型上，多领域评估显示推理课程学习带来一致性能提升。消融实验表明两个阶段都必要，数学优先训练能增强解决复杂问题所需认知行为。

Conclusion: Reasoning Curriculum提供了一种紧凑、易于采用的通用推理训练方案，能够有效提升大语言模型的跨领域推理能力。
...
TL;DR: ToolRM是一个专门为工具使用场景设计的轻量级生成式奖励模型家族，通过创新的数据构建流程和TRBench基准测试，在函数调用任务中显著优于前沿模型。
...
Method: 提出新颖的数据构建流程，使用基于规则的评分和多维采样构建成对偏好数据ToolPref-Pairwise-30K，并基于BFCL评估套件构建TRBench基准。
...
TL;DR: 本文系统性地定义了情境工程，追溯其从1990年代至今的历史发展，分析了不同智能水平机器下的人机交互范式演变，并探讨了情境工程的关键设计考量。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能发展，机器需要更好地理解人类的情境和目的。情境工程旨在解决机器如何更有效地理解人类处境这一核心问题。

Method: 通过历史分析，将情境工程发展划分为不同阶段：从早期人机交互框架到当今智能代理驱动的人-代理交互范式，以及未来可能的人类水平或超人类智能。

Result: 提出了情境工程的系统性定义，梳理了其历史脉络和概念框架，并识别了实践中的关键设计考虑因素。

Conclusion: 本文为情境工程提供了概念基础，展望了其在AI系统中的发展前景，是推动AI系统情境工程系统化发展的基石。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在各种推理任务中表现出色，但其处理规范性推理（涉及义务、许可等模态）的能力尚未得到充分探索。本文旨在填补这一研究空白。

Method: 引入了一个新数据集，涵盖规范和认知领域的广泛推理模式，比较LLMs在规范性模态和认知模态下的推理表现，同时考虑影响人类推理的非形式认知因素。

Result: LLMs总体上遵循有效推理模式，但在特定规范性推理类型中存在显著不一致性，并表现出类似人类推理的认知偏差。

Conclusion: 这些发现突显了在LLMs规范性推理中实现逻辑一致性所面临的挑战，为提升其可靠性提供了见解。
...
TL;DR: 提出异步思维（AsyncThink）作为大语言模型推理的新范式，通过将内部思考过程组织为并发可执行结构，实现多智能体协作解决复杂问题。


<details>
  <summary>Details</summary>
Motivation: 设想智能体组织的新AI时代，让智能体通过协作和并发工作解决复杂问题，实现超越个体智能的成果。

Method: 提出思考协议，组织者动态分配子查询给工作者，合并中间知识并生成连贯解决方案；思考结构可通过强化学习进一步优化。

Result: 与并行思维相比，AsyncThink推理延迟降低28%，数学推理准确率提升；能够泛化学到的异步思维能力，无需额外训练即可处理未见任务。

Conclusion: AsyncThink为构建协作智能体组织提供了有效范式，在效率和泛化能力方面表现出色。
...
TL;DR: 该论文对当代基础模型的推理能力进行了跨平台评估，建立了基础设施无关的基准测试，涵盖HPC超级计算、云平台和大学集群三种计算范式，评估了15个模型在8个学术领域的79个问题上的表现。
...
Method: 采用三阶段实验设计：基线建立（6个模型在19个问题上）、基础设施验证（在不同平台上重复基准测试）和扩展评估（79个问题的全面评估）。

Result: 发现训练数据质量比模型规模更重要，挑战了传统的规模扩展假设，并确认了基础设施无关的可复现性。

Conclusion: 建立了三基础设施方法和79问题基准，能够纵向跟踪基础模型推理能力的发展，为不同应用场景的模型选择提供了行动指南。
