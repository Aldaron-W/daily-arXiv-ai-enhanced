...
Motivation: 现有研究主要关注LLM在精确算术运算上的表现，但对其在非正式、快速数学运算中的近似推理能力研究不足，特别是非自回归解码器模型。

Method: 引入StreetMath基准，在不同LLM架构上进行广泛评估，并应用机制可解释性技术探测内部计算状态。

Result: LLM通常试图计算精确值或调用外部工具，即使在需要近似的任务中；近似和精确算术运算依赖不同的神经组件；LLM在街数场景中未表现出人类般的认知吝啬。

Conclusion: LLM在近似推理方面与人类存在差异，缺乏认知吝啬特性，这为理解LLM数学推理机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于词典的情感分析方法没有考虑意见的强度差异，无法区分非常强烈、强烈、中等、非常弱和弱等不同程度的情感表达。

Method: 结合与产品特定方面相关的意见词（副词、形容词、名词和动词），使用模糊逻辑算法将意见词分类到不同类别，并通过句法依赖解析找到期望方面词的关系。

Result: 能够为评论中特定方面计算实体得分，实现更细粒度的情感强度分类。

Conclusion: 该方法能够更准确地分析用户对实体各方面的情感倾向和强度，为基于评论的实体排序提供更精细的支持。
...
<details>
  <summary>Details</summary>
Motivation: 当前立场检测研究主要集中在目标依赖的任务上，且大多数基准数据集基于英语，难以在韩语等低资源语言中开发模型。

Method: 从韩国政党的新闻稿中收集数据，构建了包含563,299个标注韩语句子的LASTIST数据集，并训练了最先进的深度学习模型。

Result: 创建了适用于目标无关立场检测和时间演化立场检测等多种任务的大规模韩语数据集。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型部署时使用任务特定适配器，虽然适配器参数通常不到基础模型的1%，但在推理时会产生高达2.5倍的计算开销，这成为显著的性能瓶颈。

Method: 开发零延迟融合低秩适配器(zFLoRA)，通过融合技术消除适配器在推理时的额外延迟，在1B、3B和7B规模的LLMs上进行实验验证。

Result: 在常识推理、数学推理和摘要对话等18个任务上，zFLoRA表现优于流行的监督微调基准，包括LoRA和全微调(FFT)，在NPU和GPU平台上均实现零到可忽略的延迟开销。

Conclusion: zFLoRA提供了一种高效的适配器解决方案，在保持性能的同时显著降低了推理延迟，为多任务LLM部署提供了实用的优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决机制可解释性中电路发现的主要挑战，即确定模型中执行特定任务的部分。

Method: 1. 使用自助法识别具有一致归因分数的边；2. 引入简单的比率选择策略优先选择强正分数边；3. 用整数线性规划替代标准贪婪选择。

Result: 在多个MIB任务和模型上，该方法产生了更忠实的电路并优于先前方法。

Conclusion: 提出的三种改进方法有效提升了电路发现的忠实度和性能，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决专家在多个竞争目标中选择最佳选项时的困难，特别是难以形式化复杂、隐含的偏好，降低传统偏好获取的认知负担。

Method: 使用LLM作为零样本偏好预言机，提出LISTEN-U（基于参数化效用函数）和LISTEN-T（基于非参数化锦标赛选择）两种迭代算法。

Result: 在航班预订、购物和考试安排等任务中，LISTEN-U在偏好参数对齐时表现优异，LISTEN-T提供更稳健的性能。
...
Method: 提出LongFilter框架，通过对比模型在长上下文和短上下文设置下的预测差异来衡量扩展上下文提供的信息增益，从而识别那些长距离依赖至关重要的样本。

Result: 在LLaMA-3-8B模型上将上下文长度从8K扩展到64K的实验表明，LongFilter能高效选择高质量数据，并在HELMET、LongBench和RULER等基准测试中取得显著改进。

Conclusion: LongFilter框架能够有效筛选适合长上下文预训练的数据，显著提升模型性能，解决了长文本数据中缺乏有意义长距离依赖的问题。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM在内容审核系统中如何确保公平性和中立性，特别关注角色设定对有害内容分类一致性和公平性的影响。

Method: 通过在不同LLM架构、模型大小和内容模态（语言vs视觉）上测试不同意识形态角色，分析角色对有害内容分类的影响，并进行一致性分析和政治倾向性任务验证。

Result: 不同意识形态角色对有害内容的标记倾向存在显著差异；较大模型更倾向于与同意识形态角色保持一致；角色不仅在同意识形态内表现更一致，还会倾向于维护自身观点而淡化对立观点的危害性。

Conclusion: 角色设定会在LLM输出中引入微妙的意识形态偏见，这对伪装中立的AI系统使用提出了担忧，可能强化党派观点。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的临床文档以base64编码附件形式存储，使得语义问答变得困难，传统向量数据库方法往往错过微妙的临床关系。

Method: 开发了临床笔记问答评估平台，验证CLEAR方法与零样本大上下文推理和传统分块检索增强生成的对比。

Result: CLEAR在12个临床笔记测试中获得了58.3%的胜率，平均语义相似度0.878，比宽上下文处理少用78%的token。在超过65,000 token的长文档中胜率达到75%。
...
Conclusion: 通过审视数据高效LLM后训练中的挑战，突出了开放性问题并提出了潜在的研究途径，希望激发对大规模模型训练中数据利用潜力最大化的进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在语言资源创建中具有潜力，但对其在语义标注任务中的性能和影响缺乏全面评估，特别是在视角化NLP方法下。

Method: 比较了三种实验设置下的标注时间、覆盖度和多样性：人工标注、自动标注和半自动标注。

Result: 半自动标注设置相比纯人工设置提高了框架多样性并保持相似的标注覆盖度，而自动设置在除标注时间外的所有指标上都表现较差。

Conclusion: 半自动标注方法在FrameNet语义标注任务中表现最优，平衡了效率和质量。
...
<details>
  <summary>Details</summary>
Motivation: 当无法检查大型语言模型的训练数据时，如何知道它见过什么内容？最有力的证据是模型自由重现目标内容。

Method: RECAP采用反馈驱动循环：初始提取尝试由次级语言模型评估，与参考段落比较并识别差异，然后将差异转化为最小修正提示反馈给目标模型指导后续生成。还包括检测和克服对齐拒绝的越狱模块。

Result: 在EchoTrace基准测试（涵盖30多本完整书籍）上，RECAP相比单次迭代方法有显著提升。使用GPT-4.1时，版权文本提取的平均ROUGE-L分数从0.38提高到0.47，增长近24%。

Conclusion: RECAP能够有效提取和验证LLM记忆的训练数据，通过反馈循环和越狱机制显著提高了提取性能。
...
<details>
  <summary>Details</summary>
Motivation: 探讨多语言数据混合对大型语言模型预训练的影响，特别是关于语言覆盖范围与模型性能之间权衡的争议

Method: 训练了1.1B和3B参数的LLM，在25到400种语言的不同多语言语料库上进行实验，研究英语作为枢纽语言的作用

Result: 英语和多语言数据结合不会降低任一组的语言内性能；英语作为枢纽语言能跨语言家族带来益处；在此规模模型中没有观察到显著的"多语言诅咒"
...
<details>
  <summary>Details</summary>
Motivation: 研究机器翻译中文化对齐对语义标签保持的影响，填补文化因素在翻译中作用的研究空白。

Method: 通过跨文化敏感领域和中性领域的一系列实验，比较不同机器翻译系统（包括LLMs）的标签漂移现象。

Result: 发现：(1)机器翻译系统（包括LLMs）在翻译中诱导标签漂移，特别是在文化敏感领域；(2)LLMs编码文化知识会放大标签漂移；(3)源语言和目标语言的文化相似性是标签保持的关键决定因素。

Conclusion: 忽视机器翻译中的文化因素不仅会削弱标签保真度，还会在下游应用中导致误解和文化冲突风险。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理中经常表现不佳，基于文本的生成导致未经验证和算术上不可靠的解决方案。现有的提示策略如思维链仍在这种不可靠的媒介中操作，缺乏确定性验证机制。

Method: 引入SymCode神经符号框架，将数学问题解决重新定义为使用SymPy库的可验证代码生成任务，将LLM推理建立在确定性符号引擎上。

Result: 在MATH-500和OlympiadBench等具有挑战性的基准测试中，相比基线方法准确率提升高达13.6个百分点，不仅更token高效，还将模型失败从模糊的逻辑谬误转变为透明的程序化错误。
...
<details>
  <summary>Details</summary>
Motivation: Trainium AI加速器虽然为LLM训练和推理提供了高性能解决方案，但其脉动阵列架构和数据布局的特殊要求使得实现高性能具有挑战性。

Method: 基于内核融合和新型缓存策略，减少软件管理内存层次中的数据移动，最大化SRAM带宽，避免昂贵的矩阵转置操作。

Result: 在9个数据集和4个最新LLM上的评估显示，相比AWS现有实现，矩阵乘法内核平均加速1.35倍（最高2.22倍），端到端LLM推理平均加速1.66倍（最高2.49倍）。

Conclusion: 针对Trainium架构定制的优化技术能够显著提升矩阵乘法性能和LLM推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 许多实际工作负载（如分类、问答、推荐等）仅使用LLM的预填充阶段，而自注意力计算因其二次复杂度成为性能瓶颈。研究发现不同语义的句子在不同层和头上会产生相似的注意力图

Method: 基于注意力图记忆数据库，采用高效的缓存和相似性搜索技术，在推理时识别并重用预缓存的相似注意力图

Result: 实验结果显示，在CPU上平均获得1.2倍端到端和2倍注意力加速，在GPU上平均获得1.6倍端到端和3倍注意力加速，精度损失可忽略

Conclusion: AttnCache通过重用相似注意力图有效加速了LLM预填充阶段的推理性能，为解决自注意力计算瓶颈提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多步推理任务上表现不佳。对于小规模开源模型，强化学习在正确解很少被采样时失败，而监督微调则倾向于通过逐词模仿过度拟合长演示。

Method: SRL框架将问题解决重新定义为生成逻辑动作序列，训练模型在提交每个动作前生成内部推理独白。基于模型动作与专家动作在逐步方式中的相似性提供平滑奖励，即使在所有rollout都不正确时也能提供丰富的学习信号。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于静态数据集训练的工具使用LLM只能模仿固定的工具调用流程，无法在动态环境中探索多种解决方案，导致性能受限。

Method: 使用强化学习生成多个工具调用轨迹，形成树状结构；基于步骤能否产生正确答案和成功工具调用分配奖励；计算分叉相对优势和轨迹相对优势来训练LLM。

Result: 在17种工具的实验环境中，PORTool相比其他训练方法在最终准确性和工具调用步骤数量上都有显著提升。

Conclusion: PORTool通过强化学习探索多种工具调用轨迹，有效提升了LLM在动态工具环境中的性能，证明了步骤奖励机制的必要性和设计稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 跨语言对齐追求多语言表示的统一，但可能导致文化擦除——即模型无法根据查询语言提供文化情境化的不同回答。

Method: 提出了转移-本地化平面评估框架，并基于发现的事实转移和文化知识在不同模型层可分别引导的洞察，提出了Surgical Steering方法，在推理时对特定层应用激活引导。

Result: 研究发现现有CLA方法在六种语言中都一致改善了事实转移，但直接以文化本地化为代价；新方法能在两个竞争维度间实现更好平衡。

Conclusion: 通过在不同模型层分别引导事实转移和文化特定知识，可以克服当前对齐技术的局限性，实现更好的跨语言能力与文化保留的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 偶然甲状腺发现在非甲状腺指征的影像检查中越来越常见，但其患病率、特征和临床后果尚未明确界定。

Method: 回顾性队列研究，使用基于transformer的自然语言处理管道从多个影像模态和身体区域的报告中识别偶然甲状腺发现并提取结节特征。

Result: 在115,683名患者中，7.8%有偶然甲状腺发现，其中92.9%是结节。这些发现与甲状腺结节诊断、活检、甲状腺切除术和甲状腺癌诊断的几率更高相关。

Conclusion: 偶然甲状腺发现很常见，且与导致检测小型低风险癌症的级联反应密切相关，强调了其在甲状腺癌过度诊断中的作用以及标准化报告和更有选择性随访的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在需要与硬件设备交互的领域（如量子编程）研究不足，缺乏专门的评估框架来测试模型生成可执行量子代码的能力。

Method: 开发了QCoder Benchmark评估框架，包含两个关键特性：1）支持量子模拟器环境评估，提供电路深度、执行时间等域特定指标反馈；2）整合真实编程竞赛中的人类代码提交，用于定量和定性分析。

Result: 实验结果表明，GPT-4o等先进模型准确率仅约18.97%，而基于推理的模型如o3达到78%准确率，超过了人类代码的平均成功率（39.98%）。

Conclusion: QCoder Benchmark填补了量子编程评估的空白，展示了推理模型在该领域的优势，并发布了数据集和公共评估API以支持进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 传统1P1S训练导致模型输出多样性不足，限制了Test-Time Scaling的效果

Method: 引入Reasoning Path Divergence指标衡量多步推理路径的语义差异，基于RPD选择多样化解决方案进行1PNS训练

Result: 在Qwen3-4B-Base上实验显示，RPD选择的训练数据带来更多样化输出，pass@16平均提升2.80%，AIME24上提升4.99%

Conclusion: 1PNS训练范式能有效增强模型推理多样性，进一步放大TTS的效果
...
<details>
  <summary>Details</summary>
Motivation: 由于没有同时标注说服技巧和语篇关系的数据集，研究旨在探索两者之间的关系，这对检测在线宣传和错误信息以及理解有效沟通很重要。

Method: 以SemEval 2023 Task 3数据集为基础，使用4个LLM和10种提示开发了40个语篇关系分类器，通过多数投票策略创建了5个银标注数据集。

Result: 统计分析显示六种语篇关系（原因、目的、对比、原因+信念、让步、条件）在说服性文本中起关键作用，特别是在使用负载语言、夸张/最小化、重复和怀疑时。

Conclusion: 研究揭示了语篇关系在说服技巧中的重要作用，为检测在线宣传和错误信息提供了新视角，并增进了对有效沟通的理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有的SSM/GRM方法通常只模拟单个注意力头，可能限制了表达能力，需要一种能模拟多头注意力的更有效架构

Method: 使用混合专家(MoE)实现，不仅在通道混合MLP块中，还在时间混合SSM核中实现多个"注意力头"，创建混合状态空间专家架构

Result: 在语言建模和下游评估中优于类似模型大小和数据预算的transformer和SSM架构，更大变体在数万亿token上训练证实了可扩展性和优越性能

Conclusion: MossNet是高效、高性能循环LLM架构的一个有前景的新方向
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在指令遵循任务中存在弃权率较高的问题，需要提高模型在高质量概率区域的生成比例，改善统计效率。

Method: 通过监督微调将预训练解码器Transformer转换为SDM模型，使用最终层SDM激活层进行二元分类，采用对比输入编码方案和在线生成的困难负样本，估计监督下一个token损失的基变换。

Result: 与强监督基线相比，SDM模型显著减少了弃权率，提高了统计效率。

Conclusion: SDM语言模型能够有效提高指令遵循任务的性能，通过简单的监督微调即可实现，具有实际应用价值。
...
Result: 实验显示指令风格可使准确率变化高达16.7个百分点，CRS与任务准确率强相关，确定性解码产生更稳定的输出，模型规模与跨风格一致性正相关。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在实际控制性方面存在脆弱性，主要问题是更新遗忘——均匀且上下文无关的更新会导致token级波动，抹去早期语义编辑并破坏累积精炼过程。

Method: 提出TTA方法，通过每个token的timestep调度实现软语义token排序：关键token早期冻结，不确定token持续精炼。可以实例化为固定策略或基于任务信号的自适应策略。

Result: 在情感控制任务中，TTA将准确率提高20%以上，困惑度减半，且仅需不到五分之一的时间步；在去毒化任务中，最大毒性从14.5降至12.2，困惑度从32.0降至26.0。
...
Result: 在7个数据集中，WIMHF识别出少量人类可解释特征，这些特征占黑盒模型偏好预测信号的大部分。揭示了不同平台用户的偏好差异，以及潜在的不安全偏好。

Conclusion: WIMHF为实践者提供了以人为中心的分析方法，帮助更好地理解和利用偏好数据，支持有效的数据筛选和细粒度个性化。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG评估基准主要关注局部RAG（在少量文档中检索相关片段），但现实应用需要全局RAG能力（跨整个文档集合聚合分析信息以获得语料库级洞察）。
...
Conclusion: 现有RAG方法在全局任务上表现不佳，GlobalRAG框架通过多工具协作显著提升了全局RAG能力，为处理语料库级查询提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 准确理解隐含含义在人类交流和语言使用中至关重要，语言模型也应具备这种能力。研究探索如何通过语用学理论指导语言模型进行推理。

Method: 提出一种方法，将格赖斯语用学和关联理论等语用学理论概述作为提示提供给语言模型，引导其通过逐步推理过程得出最终解释。

Result: 相比仅提示中间推理而不提供语用学理论的基线方法，该方法使语言模型在语用推理任务上得分最高提升9.6%。即使在提示中仅提及语用学理论名称，也能在较大模型中带来1-3%的性能提升。
...
TL;DR: 本文研究发现预训练语言模型在区分借词和本土词汇方面表现不佳，即使在有明确指令和上下文信息的情况下，模型仍然偏向于使用借词而非本土词汇。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否具备类似人类识别借词的能力，特别是在双语社区中，主导语言持续向少数民族语言施加词汇影响的情况下。

Method: 评估了包括大语言模型在内的多个预训练模型在10种语言中的借词识别能力，使用明确指令和上下文信息进行测试。

Result: 模型在区分借词和本土词汇方面表现很差，证实了现代NLP系统存在偏向借词而非本土等效词的偏见。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在不同语言间性能不均衡，特别是在模型尺寸减小时问题更严重。知识蒸馏在将知识从大模型迁移到小模型方面表现良好，但在多语言环境中的应用研究不足。

Method: 研究了五种蒸馏方法在CLIP和SigLIP2模型上的表现，评估了领域内检索和领域外视觉问答任务。通过控制实验分离了蒸馏方法对跨语言表示一致性和性能稳定性的影响。

Result: 发现某些配置在模型尺寸减半的情况下仍能保持甚至提升多语言检索的鲁棒性，但其他方法无法维持跨任务稳定性，揭示了仅靠聚合准确率无法显示的设计敏感权衡。

Conclusion: 知识蒸馏在多语言视觉语言模型压缩中存在设计敏感的权衡，需要综合考虑跨语言表示一致性和下游任务稳定性，而不仅仅是总体准确率。
...
Method: 分析神经元激活模式，发现正确答案激活更少的独特神经元且跨样本一致性更强，据此提出NAD方法使用激活稀疏性和一致性作为选择标准。

Result: 在数学和科学基准测试中与多数投票相当，在开放式编程任务中优于Avg@64，通过早期停止减少99%的token使用量且质量损失极小。

Conclusion: 内部信号为无标签集成解码提供了可靠、可扩展且高效的指导，神经元激活模式是判断答案正确性的有效指标。
...
<details>
  <summary>Details</summary>
Motivation: 解释为什么LLM在数字嵌入表示上表现准确，但在数字信息处理时却产生错误输出这一矛盾现象。

Method: 探索语言模型如何处理数字，量化这些机制的准确度下限，分析隐藏状态和不同输入上下文中的数字表示。

Result: 发现不同LLM学习了系统化、高精度且可互换的数字表示，能够创建通用探针来追踪输出错误到特定层。

Conclusion: 为理解预训练LLM如何处理数字提供了基础认识，并指出了更精确探针技术在改进LLM架构方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 虽然ChatGPT Atlas在信息检索任务中表现出色，但其在动态交互环境中的性能尚未充分探索，因此使用网页游戏作为测试场景进行评估。

Method: 使用Google的T-Rex Runner、数独、Flappy Bird和Stein.world等浏览器游戏作为测试场景，以游戏性能得分作为量化指标评估不同任务类型下的表现。

Result: Atlas在数独等逻辑推理任务中表现强劲，完成速度显著快于人类基准，但在需要精确时机和运动控制的实时游戏中表现困难，往往无法通过初始障碍。

Conclusion: 虽然Atlas展现出强大的分析处理能力，但在需要实时交互的动态网页环境中仍存在明显限制。
...
Method: ACER首先生成学科目录，然后基于布鲁姆分类法创建问答对，构建综合性教材式课程。使用交错课程安排进行持续预训练，在内容和认知维度上对齐学习。

Result: 在Llama 3.2模型上，ACER在专业MMLU子集上显著提升，微观经济学准确率提高5个百分点，所有目标领域平均提升3个百分点。还促进跨领域知识迁移，非目标领域性能提升0.7点，在ARC和GPQA基准上提升超过2点。

Conclusion: ACER提供了一个可扩展且有效的方法来缩小LLMs在关键领域的差距，同时防止灾难性遗忘并促进知识迁移。
...
<details>
  <summary>Details</summary>
Motivation: 健康相关谬误信息普遍存在且有害，特别是当这些声明扭曲或误解科学发现时难以识别。需要有效方法检测这类谬误论证。

Method: 使用检索增强生成(RAG)生成合成谬误样本，然后用于微调大型语言模型(LLM)，在MISSCI数据集和框架上进行实验。
...
Method: 构建语言模型图来映射模型间关系（基于对话的语义连贯性），然后应用社区检测识别协同模型集群。

Result: 实验表明该方法能发现功能一致的模型组，这些组反映了模型的潜在专长。在特定主题引导下，识别出的协同团队在下游基准测试中优于随机基线，与基于已知模型专长的手动构建团队准确率相当。
...
Method: 研究了预训练语言模型(PLM)和大语言模型(LLM)在科学出版物话语关系分类(DRC)任务中的应用，特别关注上下文（定义为话语结构）对分类效果的帮助。

Result: 实验表明，基于话语结构的上下文信息通常对DRC任务有帮助，并分析了哪些科学话语关系类型最受益于上下文信息。

Conclusion: 上下文信息在科学话语关系分类中具有重要作用，为利用话语层面信息支持AI生成科学主张提供了初步基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs及其基准主要关注知识维度，忽视了教育场景中至关重要的素养能力评估，且在中文背景下多样性不足。
...
Conclusion: LLMs在教育应用方面仍有显著提升空间，OmniEduBench为全面评估LLMs教育能力提供了坚实基础。
...
Method: 提出SSLC方法，将低秩逼近和稀疏优化统一为一个问题，通过迭代优化算法求解。低秩逼近保留模型基本结构，稀疏优化去除非关键权重。

Result: 在LLaMA和Qwen2.5模型(7B-70B)上的实验表明，SSLC无需额外训练就能超越独立方法，达到最先进结果。Qwen2.5被压缩50%且性能不下降，获得至少1.63倍加速。

Conclusion: SSLC为高效LLM部署提供了实用解决方案，通过协同利用稀疏化和低秩逼近的优势实现高效压缩。
...
<details>
  <summary>Details</summary>
Motivation: 解决领域特定LLM缺乏透明度、微调成本高、提示工程需求大、跨领域结果不一致以及高计算需求带来的环境负面影响等问题。
...
TL;DR: CORE-KG框架通过整合类型感知共指消解模块和领域引导结构化提示，显著减少了知识图谱构建中的节点重复和噪声，提升了从复杂法律文本中提取结构化表示的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决法律案例文档中由于非结构化、词汇密集和模糊或变化引用导致的自动知识图谱构建困难，以及现有LLM方法因缺乏引导提取和共指消解而产生的噪声和节点重复问题。

Method: 采用CORE-KG框架，整合类型感知共指消解模块和领域引导结构化提示，通过系统消融研究量化两个关键组件的个体贡献。

Result: 移除共指消解导致节点重复增加28.32%，噪声节点增加4.32%；移除结构化提示导致节点重复增加4.34%，噪声节点增加73.33%。

Conclusion: 研究结果为设计从复杂法律文本中提取结构化表示的鲁棒LLM管道提供了实证见解，强调了共指消解和结构化提示在减少噪声和节点重复中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 希伯来语变音符号恢复对于确保单词发音准确和消除文本歧义至关重要，尽管未变音文本具有高度歧义性，但机器学习方法在该任务上取得了显著进展。

Method: 将任务构建为基于上下文条件的零样本分类问题，使用希伯来视觉语言模型将未变音文本作为图像处理，从动态生成的候选集中为每个单词选择最合适的变音模式。

Result: 在"oracle"设置下（正确变音形式保证在候选集中），系统达到高准确率；通过架构增强和优化训练方法显著提升了系统的泛化能力。
...
TL;DR: 本文研究了Transformer语言模型中线性关系解码器的结构，发现它们不是编码特定关系，而是提取重复的粗粒度语义属性，这解释了其高度可压缩性和有限的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩展Hernandez等人[2023]的单关系研究发现，系统性地探索多个关系解码器的组织结构，解释其可压缩性和泛化模式。

Method: 使用简单的三阶张量网络压缩关系解码器集合，开发交叉评估协议，将每个线性解码器应用于其他关系的主题。

Result: 发现线性映射不编码不同关系，而是提取重复的粗粒度语义属性（如国家属性），关系解码器集合可被高度压缩而不显著损失准确性。

Conclusion: Transformer语言模型中的线性关系解码本质上是基于属性的，而非关系特定的，这解释了其压缩性和仅对语义相近的新关系泛化的原因。
...
TL;DR: InfoFlow是一个解决强化学习中奖励密度问题的框架，通过子问题分解、失败引导提示和双代理精炼三个方法，显著提高了深度搜索场景中的奖励密度。
...
Conclusion: InfoFlow通过系统性地解决奖励密度优化问题，为强化学习在深度搜索场景中的应用提供了有效的解决方案。
...
TL;DR: 提出了一种名为CAST的动态树解码方法，通过考虑GPU配置和批大小等推理成本因素来动态优化树结构，显著提升大语言模型的推理速度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型由于自回归设计和庞大尺寸面临显著的推理延迟问题，现有的推测解码方法如EAGLE-2和EAGLE-3虽然使用动态树结构改进，但往往忽略了GPU设备和批大小等关键系统变量的影响。

Method: CAST动态树解码方法，通过考虑推理成本（包括GPU配置和批大小）来动态优化树结构。

Result: 在六个不同任务和六个不同LLM上的综合实验表明，该方法比传统解码方法快达5.2倍，并且通常比现有最先进技术提升5%到20%。
...
Method: 采用专业化代理框架，将推理分解为全局、页面和元素三个层级，构建结构化的、与查询无关的表示，在推理时选择性激活专业化代理进行多级推理。
...
TL;DR: 本文重新审视编码器-解码器架构的LLM，发现其在扩展性、性能表现和推理效率方面具有竞争力，甚至在某些任务上优于当前主流的仅解码器架构。


<details>
  <summary>Details</summary>
Motivation: 当前LLM研究快速转向仅解码器架构，但缺乏从扩展角度对编码器-解码器架构的严谨比较分析，可能忽略了其潜在优势。

Method: 重新设计编码器-解码器LLM，采用前缀语言建模进行预训练，并与仅解码器架构在不同规模（1.5亿到80亿参数）下进行全面比较。

Result: 编码器-解码器LLM展现出有竞争力的扩展特性和上下文长度外推能力，在指令调优后在下游任务上达到相当甚至更好的性能，同时具有显著更好的推理效率。

Conclusion: 编码器-解码器架构的潜力可能被低估，值得重新审视以开发更强大和高效的LLM。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等数据敏感领域，缺乏高质量领域特定训练语料阻碍了LLMs的专业应用，而领域专家已将领域知识提炼成本体规则。
...
TL;DR: Kimi Linear是一种混合线性注意力架构，首次在各种场景下公平比较中超越全注意力机制，核心是Kimi Delta Attention (KDA)模块和优化的分块算法，在保持高性能的同时显著提升效率。
...
Method: 提出Kimi Delta Attention (KDA)线性注意力模块，扩展Gated DeltaNet并采用更细粒度的门控机制；使用专门的Diagonal-Plus-Low-Rank (DPLR)转移矩阵变体优化分块算法；构建3B激活参数、48B总参数的混合KDA和MLA模型。

Result: 在相同训练配置下，Kimi Linear在所有评估任务中显著超越全MLA模型，KV缓存使用减少高达75%，1M上下文解码吞吐量提升6倍，在长短上下文和RL场景中均表现优异。

Conclusion: Kimi Linear可以作为全注意力架构的直接替代方案，提供更优的性能和效率，特别适用于长输入输出任务，并开源了相关实现和模型。
...
TL;DR: AutoDeco是一种新型架构，通过在标准Transformer上添加轻量级头来动态预测上下文特定的温度和top-p值，实现真正的端到端生成，无需手动调整解码超参数。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的"端到端"标签存在误导性，实际上依赖非可微的解码过程，需要手动调整温度和top-p等超参数，过程繁琐且不灵活。

Method: 在标准Transformer基础上添加轻量级头，在每个步骤动态预测上下文特定的温度和top-p值以及下一个token的logits，将解码转化为参数化的token级过程。

Result: 在八个基准测试中，AutoDeco不仅显著优于默认解码策略，而且达到了与通过"hacking测试集"获得的oracle调整基线相当的性能。模型还展现出基于指令的解码控制能力。

Conclusion: AutoDeco开创了可引导和交互式LLM解码的新范式，使模型能够根据自然语言指令在token级别调整解码策略。
...
TL;DR: 研究LLM在训练过程中如何学习价值对齐，发现监督微调阶段主要确立模型价值观，后续偏好优化很少重新对齐价值观，不同偏好优化算法导致不同对齐结果。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注完全训练模型的对齐评估，忽略了模型学习表达人类价值观的训练动态过程。

Method: 使用Llama-3和Qwen-3不同规模模型，结合流行的监督微调和偏好优化数据集与算法，分析价值漂移的幅度和时间。

Result: 监督微调阶段一般确立模型价值观，后续偏好优化很少重新对齐这些价值观；不同偏好优化算法即使使用相同偏好数据也会导致不同对齐结果。

Conclusion: 研究为理解价值观在训练过程中的学习机制提供了可操作的见解，有助于指导数据筛选以及模型和算法的选择以改进价值对齐。
...
TL;DR: AMO-Bench是一个包含50道奥林匹克数学竞赛难度或更高难度题目的高级数学推理基准，旨在评估大型语言模型的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有数学竞赛基准对顶级LLMs的评估效果逐渐减弱，存在性能饱和问题，需要更具挑战性的基准来推动模型发展。

Method: 创建50道经专家交叉验证达到IMO难度标准的原创问题，仅要求最终答案以便自动评分，避免数据记忆导致的性能泄漏。

Result: 在26个LLMs上的实验结果显示，最佳模型准确率仅为52.4%，大多数模型低于40%，但测试时计算增加显示出有前景的扩展趋势。

Conclusion: AMO-Bench突显了当前LLMs在数学推理方面的巨大改进空间，可作为推动语言模型推理能力发展的研究工具。
...
Method: 给定完整代码库访问权限和特定入口点，模型必须生成仅包含必要组件的最小文件来复现相同命令输出。

Result: 当前最先进模型在可靠解决Gistify任务方面表现困难，特别是对于执行轨迹较长的任务。
...
TL;DR: MemEIC是一种用于大型视觉语言模型持续组合知识编辑的新方法，通过双外部记忆和双LoRA适配器实现跨模态知识编辑，显著提升多模态问题处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑技术通常只关注单一模态（视觉或语言），忽略了LVLMs的多模态特性和知识更新的连续性，导致编辑效果不佳。

Method: 采用混合外部-内部编辑器，包含双外部记忆用于跨模态证据检索和双LoRA适配器实现各模态参数解耦更新，以及选择性激活的脑启发知识连接器进行组合推理。

Result: 实验表明MemEIC在复杂多模态问题上显著提升性能，并能有效保持先前编辑结果，为LVLMs的持续组合知识编辑设立了新基准。

Conclusion: MemEIC成功解决了LVLMs中多模态知识连续编辑的挑战，通过组合编辑方法实现了更好的知识更新效果。
...
### [60] [Metis-SPECS: Decoupling Multimodal Learning via Self-distilled Preference-based Cold Start](https://arxiv.org/abs/2510.25801)
...
### [61] [Do Students Debias Like Teachers? On the Distillability of Bias Mitigation Methods](https://arxiv.org/abs/2510.26038)
*Jiali Cheng,Chirag Agarwal,Hadi Amiri*

Main category: cs.LG

TL;DR: 知识蒸馏会削弱模型的去偏能力，研究发现去偏能力在蒸馏后下降，训练去偏模型不会从教师知识中受益，并提出三种改进方案。


<details>
  <summary>Details</summary>
Motivation: 探索知识蒸馏对模型去偏能力的影响，特别是在处理分布外数据时对抗伪相关性的鲁棒性。

Method: 在自然语言推理和图像分类任务上进行广泛实验，分析知识蒸馏对去偏能力的影响机制。

Result: 知识蒸馏总体上削弱了模型的去偏能力；训练去偏模型不会从教师知识中受益；不同偏见的鲁棒性存在显著差异；识别出导致蒸馏后不同行为的内部注意力模式和电路。
...
### [62] [Deep sequence models tend to memorize geometrically; it is unclear why](https://arxiv.org/abs/2510.26745)
*Shahriar Noroozizadeh,Vaishnavh Nagarajan,Elan Rosenfeld,Sanjiv Kumar*

Main category: cs.LG

TL;DR: 本文挑战了序列建模中参数化记忆的传统关联视图，提出了几何视图，证明Transformer模型能够合成自己的原子事实几何结构，将复杂的推理任务简化为易于学习的几何任务。


<details>
  <summary>Details</summary>
Motivation: 传统上将参数化记忆抽象为实体共现的暴力查找，但作者发现这种关联视图无法解释某些Transformer推理实例，因此探索记忆的几何存储方式。

Method: 通过分析Transformer推理实例，并与Node2Vec建立联系，研究神经网络嵌入几何的基本特性，特别是谱偏置如何自然产生几何结构。

Result: 发现Transformer模型能够学习优雅的几何结构，即使这种结构不比暴力查找关联更简洁，且这种几何源于自然产生的谱偏置。

Conclusion: 几何视图为参数化记忆提供了新视角，鼓励研究人员在知识获取、容量、发现和遗忘等领域重新审视默认直觉。
...
### [63] [Remote Labor Index: Measuring AI Automation of Remote Work](https://arxiv.org/abs/2510.26787)
...
TL;DR: AI在知识推理基准测试中进步迅速，但实际经济价值转化有限。Remote Labor Index(RLI)基准显示AI代理在经济价值项目中的自动化率仅为2.5%。


<details>
  <summary>Details</summary>
Motivation: 衡量AI在知识推理基准测试中的进步如何转化为实际经济价值和自动化能力，为AI驱动的劳动力自动化提供实证基础。

Method: 引入Remote Labor Index(RLI)基准，这是一个多领域、包含真实世界经济价值项目的测试框架，用于评估端到端AI代理在实际环境中的表现。
...
### [64] [Defeating the Training-Inference Mismatch via FP16](https://arxiv.org/abs/2510.26788)
...
<details>
  <summary>Details</summary>
Motivation: 解决RL微调LLM时的不稳定性问题，该问题源于训练和推理策略之间的数值不匹配，而现有方法通过算法修正或工程对齐未能根本解决。

Method: 通过分析发现问题的根本原因在于BF16浮点精度引入的舍入误差，因此提出简单地将精度从BF16改为FP16，无需修改模型架构或学习算法。

Result: 使用FP16统一精度能够实现更稳定的优化、更快的收敛速度，并在多样化任务、算法和框架中表现出更强的性能。

Conclusion: FP16相比BF16在RL微调中具有明显优势，建议重新考虑精度权衡，这一简单改动可显著提升RL微调的效果。
...
### [65] [FakeZero: Real-Time, Privacy-Preserving Misinformation Detection for Facebook and X](https://arxiv.org/abs/2510.25932)
*Soufiane Essahli,Oussama Sarsar,Imane Fouad,Anas Motii,Ahmed Bentajer*

Main category: cs.CR

TL;DR: FakeZero是一个完全客户端、跨平台的浏览器扩展，能够在用户滚动时标记Facebook和X上的不可靠帖子，所有计算都在本地进行，不泄露个人数据。


<details>
  <summary>Details</summary>
Motivation: 社交媒体以空前速度传播信息，加速了错误信息的传播并威胁公共话语。需要一种保护隐私的解决方案来检测虚假信息。

Method: 采用三阶段训练课程：基线微调、域自适应训练（增强焦点损失、对抗增强和后训练量化）。使用DistilBERT-Quant和TinyBERT-Quant模型，所有计算通过Chromium消息API在本地运行。
...
Conclusion: 高质量虚假新闻检测在严格资源预算下是可行的，性能损失适中。该扩展可为政策制定者提供有价值的工具，并在用户同意下为研究人员收集大规模虚假新闻数据集。
...
### [66] [SIRAJ: Diverse and Efficient Red-Teaming for LLM Agents via Distilled Structured Reasoning](https://arxiv.org/abs/2510.26037)
*Kaiwen Zhou,Ahmed Elgohary,A S M Iftekhar,Amin Saied*

Main category: cs.CR

TL;DR: SIRAJ是一个通用的红队测试框架，用于发现黑盒LLM代理的安全漏洞。它采用动态两步流程：生成多样化的种子测试用例，然后基于执行轨迹迭代构建对抗攻击。通过模型蒸馏方法优化成本，8B蒸馏模型攻击成功率提升100%，超越671B模型。
...
Method: 采用动态两步流程：1) 基于代理定义生成覆盖各种风险结果、工具使用轨迹和风险源的多样化种子测试用例；2) 基于先前尝试的执行轨迹迭代构建和优化基于模型的对抗攻击。使用模型蒸馏方法，利用教师模型的结构化推理训练更小但同样有效的模型。

Result: 种子测试用例生成方法使风险结果和工具调用轨迹的覆盖率提高2-2.5倍。蒸馏的8B红队模型攻击成功率提高100%，超越671B Deepseek-R1模型。消融实验验证了迭代框架、结构化推理和红队模型泛化能力的有效性。

Conclusion: SIRAJ框架能够有效发现LLM代理的安全漏洞，通过模型蒸馏方法在保持攻击效果的同时显著降低成本，为LLM代理的安全部署提供了重要保障。
...
### [67] [PVMark: Enabling Public Verifiability for LLM Watermarking Schemes](https://arxiv.org/abs/2510.26274)
*Haohua Duan,Liyao Xiang,Xin Zhang*

Main category: cs.CR

TL;DR: PVMark是一个基于零知识证明的插件，使LLM水印检测过程可公开验证，解决了现有水印方案因使用私密密钥而导致的信任问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM水印方案存在信任问题：非公开的水印检测无法证明其忠实执行检测过程，这源于私密密钥的困境——公开密钥会遭受移除攻击，私密密钥则使检测过程不透明。

Method: 基于零知识证明构建PVMark插件，通过证明水印检测的'正确执行'来建立约束条件，包括映射、随机数生成、比较和求和等操作。实现了多种变体，涵盖三种水印方案、三种哈希函数和四种ZKP协议。

Result: 实验结果表明，PVMark能有效为最先进的LLM水印方案提供公开可验证性，且不影响水印性能，有望实际部署。

Conclusion: PVMark成功解决了LLM水印检测的信任困境，通过零知识证明实现了公开可验证的检测过程，同时保护了私密密钥的安全性。
...
### [68] [Through the Judge's Eyes: Inferred Thinking Traces Improve Reliability of LLM Raters](https://arxiv.org/abs/2510.25860)
*Xingjian Zhang,Tianhong Gao,Suliang Jin,Tianhao Wang,Teng Ye,Eytan Adar,Qiaozhu Mei*

Main category: cs.AI

TL;DR: 提出人机协作框架，从仅标签标注中推断思维轨迹，通过拒绝采样方法重构推理过程，用于微调开放LLM评估器和优化专有LLM标注指南，显著提升LLM与人类评估一致性。
...
Method: 使用人机协作框架和简单有效的拒绝采样方法，从仅标签标注中大规模重构思维轨迹，应用于微调开放LLM评估器和优化专有LLM标注指南。

Result: 在多个数据集上，该方法显著提高了LLM与人类评估的一致性，优化的标注指南也增加了不同LLM模型间的一致性。

Conclusion: LLM可以作为人类思维轨迹的实用代理，使仅标签语料库扩展为思维轨迹增强资源，提升LLM评估器的可靠性。
...
### [69] [Approximating Human Preferences Using a Multi-Judge Learned System](https://arxiv.org/abs/2510.25884)
*Eitán Sprejer,Fernando Avalos,Augusto Bernardi,Jose Pedro Brito de Azevedo Faustino,Jacob Haimes,Narmeen Fatimah Oozeer*

Main category: cs.AI

TL;DR: 提出一个基于角色的偏好建模框架，通过聚合多个基于评分标准的评判者输出来解决LLM评判者校准困难、评分标准敏感性和偏见问题


<details>
  <summary>Details</summary>
Motivation: 解决LLM评判者难以校准、存在评分标准敏感性、偏见和不稳定性等问题，这对于构建可靠的RLHF奖励模型和有效的模型路由系统至关重要

Method: 提出基于角色的偏好标签合成方法，以及两种聚合器实现：广义加性模型(GAM)和多层感知器(MLP)

Result: 评估了该方法相对于简单基线的性能，并通过案例研究评估了其对人类和LLM评判者偏见的鲁棒性

Conclusion: 该框架能够有效建模多样化的基于角色的偏好，为LLM评判者校准提供可行解决方案
...
### [70] [Reasoning Curriculum: Bootstrapping Broad LLM Reasoning from Math](https://arxiv.org/abs/2510.26143)
*Bo Pang,Deqian Kong,Silvio Savarese,Caiming Xiong,Yingbo Zhou*

Main category: cs.AI

TL;DR: 提出Reasoning Curriculum两阶段课程学习法，先在数学领域通过RL培养推理能力，然后在多领域联合RL中迁移和巩固这些技能，无需专用奖励模型即可提升LLM的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习主要关注数学和代码领域，但缺乏在其他领域激发推理能力的方法。

Method: 两阶段课程学习：第一阶段在数学领域进行RL训练，使用可验证奖励；第二阶段在多领域数据上进行联合RL，迁移和巩固推理技能。

Result: 在Qwen3-4B和Llama-3.1-8B模型上的多领域测试显示一致性能提升，消融实验证明两个阶段都必要。

Conclusion: 数学优先的推理激发能增加解决复杂问题所需的关键认知行为，该方法为通用推理提供了简洁易用的解决方案。
...
### [71] [One Model to Critique Them All: Rewarding Agentic Tool-Use via Efficient Reasoning](https://arxiv.org/abs/2510.26167)
*Renhao Li,Jianhong Tu,Yang Su,Hamid Alinejad-Rokny,Derek F. Wong,Junyang Lin,Min Yang*

Main category: cs.AI

TL;DR: ToolRM是一个专为工具使用场景设计的轻量级生成式奖励模型家族，通过构建多样化的偏好数据集ToolPref-Pairwise-30K，显著提升了函数调用任务的性能表现。
...
Result: ToolRM在成对奖励判断中准确率提升高达14.28%，显著优于Claude 4和OpenAI o3等前沿模型，在ACEBench上减少输出token使用超过66%。

Conclusion: ToolRM不仅适用于训练目标，还能泛化到更广泛的评判任务，包括Best-of-N采样和自我纠正，为未来研究提供了数据和模型检查点。
...
### [72] [Context Engineering 2.0: The Context of Context Engineering](https://arxiv.org/abs/2510.26493)
*Qishuo Hua,Lyumanshan Ye,Dayuan Fu,Yang Xiao,Xiaojie Cai,Yunze Wu,Jifan Lin,Junfei Wang,Pengfei Liu*

Main category: cs.AI

TL;DR: 本文系统梳理了情境工程的概念、历史发展和设计考量，认为该领域可追溯至20世纪90年代，经历了从人机交互到人-智能体交互的演变，旨在为AI系统提供概念基础。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能发展，机器需要更好地理解人类的情境和目的，情境工程应运而生。本文旨在填补这一概念的系统性研究空白，为AI系统提供理论基础。

Method: 通过历史回顾和概念分析，梳理情境工程的发展阶段（从早期人机交互到现代人-智能体交互），提供系统性定义并探讨关键设计考量。

Result: 建立了情境工程的概念框架，明确了其历史演变脉络，提出了实践中的关键设计考虑因素，为未来AI系统发展奠定基础。

Conclusion: 情境工程是AI系统发展的重要基础，本文为其提供了概念框架和历史视角，是推动系统性情境工程研究的起点。
...
### [73] [Normative Reasoning in Large Language Models: A Comparative Benchmark from Logical and Modal Perspectives](https://arxiv.org/abs/2510.26606)
*Kentaro Ozeki,Risako Ando,Takanobu Morishita,Hirohiko Abe,Koji Mineshima,Mitsuhiro Okada*

Main category: cs.AI

TL;DR: 本文系统评估了大语言模型在规范推理领域的表现，发现虽然LLMs通常遵循有效推理模式，但在特定类型的规范推理中存在不一致性，并表现出类似人类推理的认知偏差。
...
Method: 构建了一个新数据集，涵盖规范和认知领域的广泛推理模式，同时纳入影响人类推理的非形式认知因素，通过比较LLMs在规范模态和认知模态中的推理表现来评估其能力。

Result: LLMs总体上遵循有效推理模式，但在特定规范推理类型中存在显著不一致性，并表现出类似人类推理中的认知偏差。

Conclusion: 这些发现突显了在LLMs的规范推理中实现逻辑一致性的挑战，为提升其可靠性提供了见解。
...
### [74] [The Era of Agentic Organization: Learning to Organize with Language Models](https://arxiv.org/abs/2510.26658)
...
Method: 提出思考协议，由组织者动态分配子查询给工作者，合并中间知识并生成连贯解决方案，思考结构可通过强化学习进一步优化。

Result: 相比并行思维，AsyncThink推理延迟降低28%，数学推理准确性提高，且无需额外训练即可有效处理未见任务。

Conclusion: 异步思维范式能够有效组织并发思考过程，在提升性能的同时具备良好的泛化能力，为智能体协作提供了新的技术路径。
...
### [75] [Cross-Platform Evaluation of Reasoning Capabilities in Foundation Models](https://arxiv.org/abs/2510.26732)
*J. de Curtò,I. de Zarzà,Pablo García,Jordi Cabot*

Main category: cs.AI

TL;DR: 该论文提出了一个跨平台的基础模型推理能力评估框架，在三种计算范式下评估了15个模型在79个问题上的表现，挑战了传统的规模扩展假设，并强调了训练数据质量的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏基础设施无关的基础模型推理能力评估基准，需要建立可复现的跨平台评估方法来系统比较不同模型在不同计算环境下的表现。

Method: 采用三阶段实验设计：1) 在MareNostrum 5超算上建立基线；2) 在大学集群和Nebius AI Studio上验证基础设施无关性；3) 在79个问题上进行扩展评估，涵盖8个学术领域。

Result: 研究结果挑战了传统规模扩展假设，发现训练数据质量比模型规模更重要，并为教育、生产和研究场景提供了模型选择指导。

Conclusion: 提出的三基础设施方法和79问题基准能够纵向跟踪基础模型推理能力的发展，为模型评估提供了可靠的框架。
...
### [76] [Enhancing Underwater Object Detection through Spatio-Temporal Analysis and Spatial Attention Networks](https://arxiv.org/abs/2510.25797)
*Sai Likhith Karri,Ansh Saxena*

Main category: cs.CV

TL;DR: 该研究评估了时空建模和空间注意力机制在深度学习模型中对水下目标检测的有效性。通过比较标准YOLOv5、时间增强的T-YOLOv5以及添加CBAM模块的T-YOLOv5，发现时空建模显著提高了动态海洋环境中的检测精度。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决水下目标检测在动态海洋环境中面临的挑战，如突然运动、部分遮挡和渐变运动等复杂情况，探索时空建模和注意力机制如何提升检测性能。

Method: 第一阶段评估时间增强的T-YOLOv5与标准YOLOv5的性能；第二阶段开发了添加卷积块注意力模块(CBAM)的T-YOLOv5增强版本。通过比较三种模型的性能来验证时空建模和注意力机制的有效性。

Result: 测试结果显示，YOLOv5的mAP@50-95为0.563，而T-YOLOv5和T-YOLOv5+CBAM分别达到0.813和0.811，显著优于标准模型。T-YOLOv5+CBAM在复杂场景中表现更好，但在简单场景中精度有所损失。

Conclusion: 时空建模显著提高了水下目标检测的可靠性，T-YOLOv5相比标准模型有显著改进，而添加CBAM的版本在挑战性场景中表现更优，但在简单场景中需要权衡精度损失。
...
### [77] [CAVE: Detecting and Explaining Commonsense Anomalies in Visual Environments](https://arxiv.org/abs/2510.26006)
*Rishika Bhagwatkar,Syrielle Montariol,Angelika Romanou,Beatriz Borges,Irina Rish,Antoine Bosselut*

Main category: cs.CV

TL;DR: CAVE是首个真实世界视觉异常基准，支持异常描述、解释和证明三个开放任务，为评估视觉语言模型在异常检测和理解方面的能力提供了全面框架。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉中的异常检测仅限于工业缺陷或合成的异常，无法捕捉真实世界异常的丰富性和不可预测性，而人类能够自然地识别、推理和解释环境中的异常。

Method: CAVE基准包含细粒度标注，用于视觉定位和基于视觉表现、复杂性、严重性和常见性对异常进行分类，这些标注灵感来自认知科学研究。
...
TL;DR: 该研究评估了视觉语言模型在判断视频时间方向（正向或反向播放）方面的能力，发现现有模型在时间推理方面存在显著缺陷，表现接近随机猜测，远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在多模态任务中表现出色，但其对视频中时间信息的理解能力仍然薄弱且缺乏充分评估。研究者希望通过判断时间方向这一简单但具有揭示性的任务来探索这一差距。

Method: 研究者引入了AoT-PsyPhyBENCH基准测试，该基准使用与人类行为研究相同的刺激物和基线，评估模型在自然视频中推断时间方向的能力。

Result: 对开源和专有、推理和非推理型视觉语言模型的综合评估显示，大多数模型表现接近随机水平，即使在物理不可逆过程（如自由落体、扩散/爆炸）和因果手动动作（如除法/加法）等人类几乎能立即识别的任务上，最佳模型也远远落后于人类准确率。

Conclusion: 当前多模态系统存在根本性差距：虽然它们能够捕捉丰富的视觉语义关联，但缺乏时间连续性和因果理解所需的归纳偏差。研究发布代码和数据以促进视觉语言模型在物理和时间推理能力方面的进一步发展。
...
### [79] [Counteracting Matthew Effect in Self-Improvement of LVLMs through Head-Tail Re-balancing](https://arxiv.org/abs/2510.26474)
*Xin Guo,Zhiheng Xi,Yiwen Ding,Yitao Zhai,Xiaowei Shi,Xunliang Cai,Tao Gui,Qi Zhang,Xuanjing Huang*

Main category: cs.CV

TL;DR: 论文发现自改进过程中存在马太效应：模型在简单问题上表现良好但在复杂问题上表现不佳，导致优化不平衡。提出了分布重塑和轨迹重采样两种策略来解决这个问题。


<details>
  <summary>Details</summary>
Motivation: 自改进过程中模型对简单查询(头部数据)能生成高质量轨迹，但对复杂查询(尾部数据)表现不佳，导致优化不平衡，阻碍模型处理复杂推理任务的能力。

Method: 提出了四种高效策略，从两个角度：分布重塑和轨迹重采样，在探索学习的自改进过程中实现头尾重新平衡。
...
### [80] [Are Video Models Ready as Zero-Shot Reasoners? An Empirical Study with the MME-CoF Benchmark](https://arxiv.org/abs/2510.26802)
*Ziyu Guo,Xinyan Chen,Renrui Zhang,Ruichuan An,Yu Qi,Dongzhi Jiang,Xiangtai Li,Manyuan Zhang,Hongsheng Li,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: 该论文对视频生成模型Veo-3作为零样本推理器的能力进行了全面评估，发现其在短时空间一致性、细粒度定位和局部动态一致性方面表现良好，但在长时因果推理、严格几何约束和抽象逻辑方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 研究视频生成模型是否具备作为零样本推理器在复杂视觉推理场景中应用的能力，特别是评估其世界知识和推理能力。

Method: 通过构建MME-CoF基准测试，在12个维度上系统评估Veo-3模型的推理行为，包括空间、几何、物理、时间和具身逻辑等方面。

Result: 视频模型在短时空间一致性、细粒度定位和局部动态一致性方面表现出有前景的推理模式，但在长时因果推理、严格几何约束和抽象逻辑方面存在明显限制。
...
### [81] [ORBIT -- Open Recommendation Benchmark for Reproducible Research with Hidden Tests](https://arxiv.org/abs/2510.26095)
...
Method: ORBIT提供标准化的评估框架，包括可复现的数据集划分和透明的公开排行榜设置。ClueWeb-Reco是基于8700万高质量网页浏览序列的合成数据集，作为隐藏测试挑战模型的泛化能力。

Result: 在公开基准测试中，推荐系统普遍有所改进，但个体性能差异较大。隐藏测试结果显示现有方法在大规模网页推荐中存在局限性，LLM集成显示出改进潜力。

Conclusion: ORBIT基准测试揭示了现有推荐方法的局限性，特别是在大规模网页推荐场景中，并强调了LLM集成的改进潜力。
...
### [82] [Nexus: Execution-Grounded Multi-Agent Test Oracle Synthesis](https://arxiv.org/abs/2510.26423)
*Dong Huang,Mingzhe Du,Jie M. Zhang,Zheng Lin,Meng Luo,Qianru Zhang,See-Kiong Ng*

Main category: cs.SE

TL;DR: Nexus是一个新颖的多智能体框架，用于生成非回归测试中的测试预言，通过专业智能体的协作审议、验证和迭代自优化过程，显著提升了测试预言的准确性和下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 测试预言生成是软件工程中长期存在的挑战，传统方法难以准确判断被测函数在给定输入下的预期行为。

Method: Nexus采用多智能体框架，包含四个具有不同测试哲学的专业智能体进行协作审议，通过验证阶段生成候选实现并在沙箱中执行测试，对失败的预言进行自动调试和修正的迭代自优化循环。

Result: 在七个不同基准测试上的评估表明，Nexus显著优于现有最先进方法，例如在LiveCodeBench上将GPT-4.1-Mini的测试级预言准确率从46.30%提升至57.73%，在HumanEval上错误检测率从90.91%提升至95.45%，自动程序修复成功率从35.23%提升至69.32%。

Conclusion: Nexus通过多智能体协作和迭代自优化机制，有效解决了测试预言生成问题，显著提升了测试准确性和下游软件工程任务的性能。
...
### [83] [SecureReviewer: Enhancing Large Language Models for Secure Code Review through Secure-aware Fine-tuning](https://arxiv.org/abs/2510.26457)
*Fang Liu,Simiao Liu,Yinghao Zhu,Xiaoli Lian,Li Zhang*

Main category: cs.SE

TL;DR: SecureReviewer是一个专门用于增强LLM在代码审查中识别和解决安全问题的能力的新方法，包括构建安全代码审查数据集、安全感知微调策略、集成RAG技术减少幻觉，并提出了SecureBLEU评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有自动化代码审查方法主要关注通用目的审查，在识别和解决安全相关问题方面效果有限，且面临数据稀缺和评估指标不足的挑战。

Method: 构建安全代码审查数据集，采用安全感知微调策略训练LLM，集成RAG技术增强可靠性，并开发SecureBLEU评估指标。

Result: 实验结果表明，SecureReviewer在安全漏洞检测准确性和生成的审查评论质量方面均优于现有最先进基线方法。

Conclusion: SecureReviewer有效提升了LLM在代码审查中识别和解决安全问题的能力，为安全代码审查提供了实用的解决方案。
...
### [84] [Rethinking Text-to-SQL: Dynamic Multi-turn SQL Interaction for Real-world Database Exploration](https://arxiv.org/abs/2510.26495)
*Linzhuang Sun,Tianyu Guo,Hao Liang,Yuying Li,Qifeng Cai,Jingxuan Wei,Bihui Yu,Wentao Zhang,Bin Cui*

Main category: cs.DB

TL;DR: 提出了DySQL-Bench基准测试，用于评估文本到SQL模型在动态多轮交互场景中的性能，相比静态单轮任务更具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL系统在现实交互场景中表现不足，用户意图会随着中间结果而演变，需要多轮查询优化，特别是在金融和商业分析等应用中。

Method: 通过自动化的两阶段流水线构建基准：任务合成（基于数据库表结构树表示指导LLM生成任务）和验证（交互导向过滤和专家验证）。还提出了模拟真实交互的多轮评估框架。

Result: 构建了覆盖13个领域、1,072个任务的基准，人类评估确认合成数据100%正确。GPT-4o仅达到58.34%总体准确率和23.81%的Pass@5指标，显示基准难度很高。

Conclusion: DySQL-Bench填补了动态文本到SQL评估的空白，揭示了现有模型在交互场景中的局限性，为未来研究提供了重要基准。
...
TL;DR: 提出了一种新的TTS可懂度评估方法SP-MCQA，通过口语段落多选题问答来评估合成语音中关键信息的准确性，并发布了8.76小时的新闻风格基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有TTS可懂度评估过度依赖词级准确率指标如WER，无法捕捉真实语音的复杂性或反映人类理解需求，存在评估瓶颈。

Method: 提出口语段落多选题问答(SP-MCQA)主观评估方法，并构建SP-MCQA-Eval基准数据集用于评估合成语音中关键信息的准确性。

Result: 实验表明低WER并不能保证高关键信息准确率，传统指标与实际可懂度存在差距；即使SOTA模型在文本规范化和语音准确性方面仍有不足。

Conclusion: 当前许多系统在WER指标上表现优异，但在真实世界可懂度方面可能不足，迫切需要更高级、更贴近生活的评估标准。
