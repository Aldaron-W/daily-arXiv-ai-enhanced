...
Method: 使用Group Relative Policy Optimization (GRPO)结合多语言对比奖励信号，通过语义相似度奖励信号增强SQL生成与用户意图的对应关系

Result: 在MultiSpider数据集上，LLaMA-3-3B模型执行准确率提升至87.4%，语义准确率提升至59.14%；3B模型优于8B零样本模型，执行准确率提升7.43个百分点

Conclusion: 通过对比奖励信号实现定向语义对齐，无需大规模训练数据集即可显著提升Text-to-SQL系统性能
...
<details>
  <summary>Details</summary>
Motivation: 当前XAI技术（如SHAP和LIME）虽然能产生技术准确的输出，但无法提供临床相关、可操作的见解，导致实验室到临床的转化存在障碍。

Method: 提出生成式操作框架，使用LLM作为核心翻译引擎，整合多种XAI工具的技术输出和临床指南（通过RAG），自动生成人类可读、有证据支持的临床叙述。

Result: 该框架直接解决了工作流集成、偏见缓解和利益相关者特定沟通等关键操作障碍。

Conclusion: 为超越孤立数据点生成，向提供集成、可操作且可信赖的AI临床实践提供了战略路线图。
...
Method: 利用词性n-gram建模语言不确定性，在语法约束强的上下文减弱水印信号以保持质量，在语言灵活性强的上下文增强水印信号以提高可检测性。

Result: 在分析性英语、孤立性汉语和黏着性韩语等类型多样的语言上，STELA在检测鲁棒性方面优于先前方法。

Conclusion: STELA通过语言自由度对齐水印强度，实现了公开可验证的检测，在文本质量和检测鲁棒性之间取得了更好的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型广泛应用，用户在日常交互中贡献了越来越多的偏好标签。用户是判断自己查询响应质量的最佳专家，但这些标签缺乏质量控制。

Method: 通过两个不同模型生成响应，利用响应不对称性构建用户行为模型，开发期望最大化算法估计用户潜在质量因子，并据此过滤用户标注数据。

Result: 下游任务验证了该方法在捕捉用户行为和为大语言模型对齐进行数据过滤方面的有效性。

Conclusion: 用户比较模式是一种可行的成对偏好数据收集方法，通过质量估计和过滤机制可以有效提升数据质量，为大语言模型对齐提供可靠数据支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态token级计算分配方法依赖贪婪路由，导致不可逆信息损失和次优token选择，限制了LLMs在实际应用中的部署。

Method: 提出informed routing范式，使用轻量级特征预测器(LFF)在路由决策前估计单元输出，实现灵活的execute-or-approximate策略。

Result: 在语言建模和推理任务上的实验表明，该方法在多个稀疏度水平下实现了最优的效率-性能权衡，训练时间减少50%以上。

Conclusion: informed routing通过前瞻性评估token可恢复性，有效解决了贪婪路由的局限性，为LLMs高效部署提供了新思路。
...
Method: 提出HIES剪枝标准，将头部重要性分数与注意力熵相结合，提供头部贡献的互补证据。

Result: 基于HIES的剪枝在模型质量上比仅使用HIS方法提升15.2%，稳定性提升2.04倍，实现显著模型压缩而不损失精度或稳定性。

Conclusion: HIES剪枝方法通过整合头部重要性和注意力模式多样性，有效解决了现有剪枝方法的局限性，实现了更好的模型压缩效果。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据分析任务往往目标不明确且数据不干净，需要用户交互来理解意图，但现有基准无法捕捉这些复杂性或支持交互性。

Method: 使用多代理工作流从公开数据集的文章中生成真实基准，创建了1,420个对话式数据分析问题，并提供评估工具。

Result: 评估显示新一代LLM能解决更多实例，但在需要持续长时交互的任务上表现不佳。

Conclusion: ConDABench为模型构建者提供了衡量真正协作模型进展的途径，能够完成复杂的交互式任务。
...
<details>
  <summary>Details</summary>
Motivation: 不确定性量化对于可信AI系统至关重要，黑盒方法具有适应性强、成本低等优势，但现有方法在复杂生成任务中的效果需要验证。

Method: 提出基于相似性的聚合框架，通过比较生成输出的一致性来估计置信度，并引入了使用小训练集训练置信度估计模型的新技术。

Result: 在问答、摘要和文本到SQL等多样化任务上的实证研究表明，提出的相似性方法比基线方法能产生更好校准的置信度。

Conclusion: 基于相似性的黑盒UQ方法在复杂生成任务中能有效估计LLM的不确定性，为可信AI系统提供了实用的解决方案。
...
Conclusion: 提出的文化感知框架能有效解决仇恨言论检测中的数据稀疏、文化纠缠和标签模糊等问题，提升分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 手动构建本体耗时耗力且成本高昂，而当前数字时代需要有效组织大量非结构化文本信息，特别是在电子商务领域需要正确组织产品信息。

Method: 利用大语言模型的最新进展，开发了从原始评论文本中自动提取产品本体的全自动方法，以部分-整体关系形式表示。

Result: 通过LLM作为评估器进行评估，证明该方法生成的本体优于现有的基于BERT的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs对误导性检索内容的敏感性，但真实世界事实核查场景中可信证据通常占主导地位，需要研究在这种更具挑战性环境下的知识投毒威胁。

Method: 提出ADMIT攻击技术：一种少样本、语义对齐的投毒方法，无需访问目标LLMs、检索器或token级控制，通过多注入技术实现攻击。

Result: 在4个检索器、11个LLMs和4个跨领域基准测试中，ADMIT平均攻击成功率达86%，投毒率仅为0.93×10^-6，且在强反证据存在时仍保持鲁棒性，比现有最优攻击方法提升11.2%的ASR。

Conclusion: ADMIT暴露了真实世界基于RAG的事实核查系统的显著脆弱性，即使在可信证据主导的检索环境中，极低投毒率也能有效攻击系统决策。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以协调EHR的表格化、事件化特性与自然语言模型的序列先验之间的结构不匹配，限制了捕捉患者就诊间纵向依赖关系的能力。

Method: 通过额外预训练SciBERT于结构化EHR序列，设计SerialBEHRT模型来编码临床事件间的时序和上下文关系。

Result: 在抗生素敏感性预测任务上，SerialBEHRT相比最先进的EHR表示策略展现出更优越和一致的表现。

Conclusion: 时序序列化在医疗基础模型预训练中具有重要意义，SerialBEHRT证明了其在捕捉临床事件时序关系方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统固定词汇子集方法存在两个问题：频率列表依赖语料库且需要重新调优，静态短列表抑制了罕见或领域特定token，降低了每个验证步骤的预期token数量。

Method: 引入轻量级粗粒度元分类器，将上下文路由到少量token簇，选择top-k簇的并集作为起草者的短列表，同时验证保留完整词汇和精确性。通过并行执行起草编码和元短列表计算来提高效率。

Result: 在标准推测解码基准测试中，相比固定短列表基线，平均接受长度持续提升，上下文相关选择使得更小的短列表不会降低接受率。

Conclusion: DynaSpec提供了一种稳健、加速起草并能跨多样化任务泛化的动态短列表机制。
...
<details>
  <summary>Details</summary>
Motivation: 解决标准参数高效微调方法在处理复杂组合任务（如从长对话生成翻译摘要）时面临的困难，特别是在同时执行多个任务时的性能问题。

Method: 在组合的摘要和翻译适配器之上添加可学习投影层，有效集成不同任务适配器，同时保持计算效率，避免大量重新训练或顺序处理。

Result: 实验结果表明该方法在云端和端侧实现中表现良好且快速，Android应用能够无缝执行组合任务。

Conclusion: 该方法在现实应用中具有潜力，能够在资源受限环境下实现高速操作，为组合多任务场景提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型经常出现意外的代码切换现象，这会降低下游任务的可靠性，因此需要一种方法来控制语言身份。

Method: 使用潜在空间语言引导方法，通过对并行翻译进行PCA分析来识别语言方向，然后沿着这些轴引导token嵌入以控制语言身份。

Result: 使用单个主成分实现了95-99%的语言分类准确率，在Qwen2.5和Llama-3.2模型上，多个语言对的下一token分布差异减少了高达42%。

Conclusion: 该方法能有效缓解代码切换问题，同时保持语义完整性，计算开销极小，且只需少量并行数据进行校准。语言表示在最终层集中且具有近乎完美的线性可分性。
...
<details>
  <summary>Details</summary>
Motivation: 受心理语言学中均匀信息密度假说的启发，研究LLM推理过程中的信息流，因为现有CoT推理的中间步骤往往不忠实或难以解释。

Method: 引入基于熵的指标来分析推理轨迹中的信息流，在三个具有挑战性的数学基准上进行测试。

Result: 在三个数学基准上发现，成功的LLM推理表现出全局非均匀的信息密度波动，与人类沟通模式相反。

Conclusion: 这一发现挑战了机器推理的现有假设，为设计可解释和自适应推理模型提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于定位-编辑框架的方法在顺序编辑场景中存在严重局限性，新编辑会破坏先前集成的更新并降低保留知识质量。

Method: 采用顺序零空间对齐策略，为每个传入编辑执行零空间对齐，保留原始和先前修改的知识表示。

Result: 在真实世界顺序知识编辑基准测试中，EvoEdit达到或优于现有最先进定位-编辑技术，速度提升高达3.53倍。

Conclusion: 结果强调了在动态演化信息环境中开发更原则性LLM设计方法的必要性，同时提供了一个简单有效且具有强理论保证的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLM是否会对不同用户群体提供事实不一致的回答，确保模型的公平性和可靠性，提供独立于LLM提供商的公正评估。

Method: 使用19个LLM，针对15个话题各请求5个事实，每个LLM重复查询100次，每次添加不同人物角色上下文。通过句子嵌入计算跨人物角色的余弦相似度，加权平均得到事实一致性分数。

Result: 在100个人物角色的实验中，一致性分数范围为0.9065到0.7896，均值为0.8656。xAI的Grok-3最一致，轻量级模型排名最低。不同话题一致性差异明显：就业市场最不一致，G7领导人最一致。

Conclusion: LLM的事实一致性既受提供商影响，也受话题影响。需要开发人物角色不变的提示策略来改善一致性，并发布了代码和交互演示以支持可复现评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL研究主要关注公开数据集，但在私有企业数据仓库中效果较差。手动标注SQL日志来构建基准测试既困难又昂贵。

Method: 使用检索增强生成和LLMs为给定SQL查询生成多个自然语言描述，然后由人类专家选择、排序或编辑这些草稿以确保准确性和领域对齐。

Result: 在标注的企业SQL日志上评估显示，LLM辅助标注大幅减少了创建高质量基准测试的时间和精力，结合人工验证提高了标注准确性、基准可靠性和模型评估鲁棒性。

Conclusion: BenchPress为研究人员和从业者提供了一种机制，可以在特定领域工作负载上评估文本到SQL模型，系统已在GitHub开源并提供网站访问。
...
Method: 提出Rule-to-Tag框架，将多层级语言规则集成到神经网络训练目标中，使用包含正则化项的自适应损失函数来处理OOV词汇的不确定性。

Result: 在Zarma语言的POS标注任务中，仅使用无标注文本训练的R2T-BiLSTM模型达到98.2%准确率；在NER任务中，R2T预训练后仅需50标注句子即可超越基于300标注句子的基线。

Conclusion: R2T框架展示了原则学习范式的有效性，能够在低资源场景下显著减少对标注数据的依赖，为资源匮乏语言的处理提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 不同LLM具有互补能力，但集成方法对潜在错误信号（如异构分词方案和模型专业知识差异）的鲁棒性关注不足。
...
Conclusion: CoRE通过利用模型一致性有效解决了LLM集成中的鲁棒性问题，可无缝集成到各种集成方法中。
...
Method: 采用通用领域指令调优的大语言模型，结合检索增强生成(RAG)框架，整合领域内文本和视觉示例，通过简单索引和融合添加相关示例，无需额外训练或复杂重排序。

Result: 在19个团队51个提交中排名第三，平均得分41.37%，在dBLEU、ROUGE、BERTScore和基于LLM的指标上表现优异。

Conclusion: 轻量级RAG与通用LLM结合，作为最小推理时间层，为多模态临床NLP任务提供了简单有效的基线方法。
...
Method: 基于AI可解释性和推理时层剪枝研究，发现在中等上下文场景中，归一化和注意力计算近似线性，使得整个transformer块可以通过多层感知机(MLPs)来近似。

Result: 与父模型相比，ShishuLM在训练和推理期间内存需求减少高达25%，延迟改善高达40%。
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探讨LLM在教育中的学习影响，但LLM介导辅导中的情感动态仍未充分理解。本研究旨在通过关注学习者不断变化的情感状态，推进将生成式AI负责任地整合到教育中的路径。
...
Result: 学生与AI导师互动时通常报告轻度积极情感和中等唤醒度。困惑和好奇是解决问题的常见伴随状态，挫折虽然较少见但仍会出现。情感状态短暂存在，积极时刻稍长但易受干扰。负面情感常快速解决，中性时刻常作为转折点。

Conclusion: 中性时刻为导师干预提供了机会窗口，情感动态的细致理解有助于设计更有效的AI辅导系统，在关键时刻支持学生的学习过程。
...
Method: 提出模板填充（TI）方法：首先生成目标响应的结构模板，然后填充掩码段。引入动态段分配（DSA）根据生成置信度自适应调整段长度。

Result: 在数学推理和代码生成基准测试中，相比基线方法取得了17.01%的显著提升，并在多标记生成设置中实现有效加速同时保持生成质量。

Conclusion: 模板填充方法为扩散语言模型提供了有效的结构控制生成策略，在多个任务上表现出优越性能，并支持高效的并行生成。
...
Method: 详细介绍了17种克丘亚语言，并以普诺克丘亚语为重点案例研究，包括语言上线过程和阅读及自发语音数据的语料库收集。

Result: Common Voice现在拥有191.1小时的克丘亚语语音数据（86%已验证），其中普诺克丘亚语贡献了12小时（77%已验证）。

Conclusion: 这项工作为包容性语音技术和资源匮乏语言社区的数字赋权做出了贡献，并提出了解决技术挑战的研究议程以及社区参与和土著数据主权的伦理考虑。
...
Method: 将西班牙语CANTEMIST语料库翻译为法语，由领域专家手动标注实体范围，通过自动匹配和人工验证生成ICD-O标准化。

Result: 最终数据集包含399个独特形态学代码、272个解剖学位置代码和2043个独特复合表达，共71127个ICD-O标准化。
...
<details>
  <summary>Details</summary>
Motivation: 解决早期退出或基于路由器的混合深度模型存在不稳定且需要大量重新训练的问题，提供一种可在预训练模型上稳定微调的平滑可微分门控机制。

Method: 为每个注意力/MLP分支配备sigmoid线性门，在分支输出重新进入残差流之前进行压缩，推理时根据门值对token排序并按层预算跳过低重要性token。

Result: 在长形式推理中节省高达15%计算成本同时保持90%以上基线准确率；对于更大模型，这种权衡显著改善；在指令调优模型中，全计算时获得准确率提升，在50%节省时匹配基线质量。

Conclusion: GateSkip提供了一种稳定高效的层跳过方法，学习到的门控机制揭示了transformer信息流模式，并能与量化、剪枝和自推测解码等技术轻松结合。
...
Method: 创建纯文本反馈的多臂老虎机环境，LLM仅通过"你获得了一个代币"这样的文本反馈来推断潜在奖励结构，并与Thompson Sampling、Epsilon Greedy、UCB等传统算法比较。

Result: 大多数LLM表现不如基线算法，但Qwen3-4B实现了89.2%的最佳臂选择率，显著优于更大的LLM和传统方法。

Conclusion: 概率推理能力可以从纯语言中涌现，该基准测试为评估自然主义、非数值环境下的决策能力迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 让语言模型能够根据输入token的复杂性自适应地分配计算资源，在需要更多思考时请求额外计算时间，提高计算效率。

Method: 提出了Catch Your Breath损失函数系列：CYB-AP（随时预测）、CYB-VA（变分方法）、CYB-DP（计算预算惩罚），通过微调实验确定最佳变体。

Result: CYB模型仅需基线模型1/3的训练数据即可达到相同性能，比带暂停的交叉熵损失模型少用一半数据。模型能够根据token复杂性自适应调整处理时间。
...
<details>
  <summary>Details</summary>
Motivation: 自然语言生成模型在特定任务或需求下表现不佳或需要大量额外数据调整，需要一种简单有效的方法来增强其性能。

Method: 使用轻量级辅助模块从输入文本中提取推理信息，构建增强输入来改进生成质量，采用模块化架构易于适应不同任务。

Result: 在需求工程领域的概念验证中，使用分类器辅助模块成功提升了软件需求生成的质量。

Conclusion: PAGE框架提供了一种简单、模块化的方法，无需辅助生成模型即可有效增强文本生成模型的性能和可控性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM社会模拟大多使用选择题或简答形式，忽视了LLM的生成特性，无法真实反映社会现象的复杂性。

Method: 借鉴调查方法学和NLP最新进展，提出在LLM社会模拟中采用开放式文本生成方法。

Result: 开放式方法能改善测量和设计、探索未预期观点、减少研究者偏见、捕捉表达性和个体性、辅助预测试。
...
<details>
  <summary>Details</summary>
Motivation: 评估当代大语言模型在非结构化文本分类任务中的实际表现，探索模型规模与架构改进是否足以提升分类准确性。

Method: 使用8,660个人工标注样本和统一零样本提示，评估10个LLM在IAB 2.2层次分类法上的表现，包括经典指标和LLM特定指标。开发集成方法作为对比。

Result: 当代LLM平均准确率34%，精确率42%，召回率45%，F1分数41%。Gemini 1.5/2.0 Flash和GPT 20B/120B具有最佳性价比，GPT 120B幻觉率最低。集成方法显著提升准确率，完全消除幻觉。

Conclusion: 单纯的模型规模和架构改进不足以确保更好的分类准确性。模型协调编排（如集成方法）可能是实现或超越人类专家性能的最有效途径。
...
Method: 引入ProofBench专家标注数据集，包含145个数学竞赛问题和435个LLM生成的解决方案；系统探索评估器设计空间，开发ProofGrader评估器，结合强推理骨干模型、参考解决方案和评分方案的丰富上下文，以及简单的集成方法。

Result: ProofGrader在专家评分上达到0.926的MAE，显著优于朴素基线；在最佳n选择任务中，当n=16时平均得分4.14，填补了朴素二元评估器(2.48)与人类专家(4.62)之间78%的差距。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型面临高微调成本、推理延迟、边缘部署受限和可靠性问题，而小语言模型具有紧凑、高效和适应性强的特点，两者的协作可以互补优势。

Method: 提出了一个基于协作目标的分类法，包括四个主要目标：性能增强、成本效益、云边隐私和可信度，并在该框架下回顾了代表性方法。

Result: 总结了SLM-LLM协作的设计范式，并识别了当前研究中的代表性协作方法。

Conclusion: 指出了实现高效、安全和可扩展的SLM-LLM协作面临的开放挑战和未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在高质量监督微调数据选择过程中过度依赖LLM内部知识、可解释性差且泛化能力有限，需要更有效的选择方法。

Method: 提出THTB框架，结合质量过滤与内在和外在难度评分，优先选择需要更高认知水平的指令数据，为SFT提供可解释和可量化的选择标准。

Result: 实验表明，使用THTB选择的5%数据训练的模型性能优于全数据集训练，在垂直领域仅用2%数据就能超越使用更大数据集训练的模型。

Conclusion: THTB在数据选择和标注指导方面都表现出色，具有强大的领域适应潜力，能够显著降低训练成本同时提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要关注单轮攻击，缺乏跨语言覆盖，且依赖的分类法无法完全捕捉攻击策略的多样性或过于强调风险类别而非越狱技术本身。

Method: 进行结构化红队挑战，开发包含7个主要家族（冒充、说服、权限提升、认知过载、混淆、目标冲突、数据投毒）的50种越狱策略分层分类法，收集和分析挑战数据，评估基于分类法的提示对自动检测的效果，并创建多轮对抗对话数据集。

Result: 建立了全面的越狱策略分类体系，揭示了不同攻击策略如何利用模型漏洞并导致错位，证明了分类法引导的提示能改进自动检测，创建了首个意大利语多轮对抗对话数据集。

Conclusion: 该研究推进了对越狱技术有效性的理解，提供了系统化的分类框架和数据集，为改进LLM安全防御提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型生成文本质量接近人类写作，作者归属变得越来越困难，需要开发有效的识别机制。

Method: 使用包含600个平衡实例的Human AI Parallel Corpus数据集，在六个领域比较固定风格嵌入和GPT-4o LLM法官两种方法的性能。

Result: 风格嵌入在GPT生成内容上表现更好(82% vs 68%)，LLM法官在LLaMA生成内容上略优(85% vs 81%)。LLM法官在小说和学术文本中表现突出，而嵌入在口语和剧本对话中占优。

Conclusion: 作者归属是一个多维问题，需要结合语义敏感性和结构分析的混合策略，本文提供了可复现的基准框架。
...
Method: 使用模型差异分析工具，分析微调前后模型在随机文本前几个token上的激活差异，并将这些差异添加到模型激活中进行引导生成。

Result: 发现窄域微调模型在激活中存在明显的训练目标痕迹，创建基于LLM的可解释性代理能显著提升对微调领域的理解。

Conclusion: 窄域微调模型存在显著的激活偏差，这对AI安全研究具有重要警示意义，需要更深入地研究窄域微调的影响并开发更真实的研究案例。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在各种任务中表现出色，但仍然容易受到越狱攻击的威胁，这些攻击会绕过安全机制。

Method: RAID将离散标记松弛为连续嵌入，通过联合目标优化：(i)鼓励受限响应，(ii)加入拒绝感知正则化器使激活远离拒绝方向，(iii)应用连贯性项保持语义合理性和非冗余性。优化后通过批评引导的解码过程将嵌入映射回标记。
...
Method: 使用道德基础理论框架，分析LLM的固有响应、直接政治意识形态表达以及基于人类角色扮演的响应，系统比较LLM输出与人类研究数据。

Result: 研究发现LLM响应确实表现出特定的意识形态倾向，无论是在固有响应还是角色扮演场景中，都显示出政治和人口统计特征的依赖性。

Conclusion: LLM生成的响应存在政治和人口统计偏见，这为理解AI生成内容中的意识形态倾向提供了重要见解，强调了在关键应用领域需要谨慎使用LLM。
...
<details>
  <summary>Details</summary>
Motivation: 传统示例驱动的上下文学习缺乏知识检索和抽象层面转移的显式模块，受认知科学中图式理论启发，人类通过激活已有心智框架来理解新信息。

Method: 从先验示例中提取推理过程的认知构建块表示，创建轻量级结构化模板（schema），包含关键推理步骤及其关系，用于增强模型处理新问题时的推理过程。

Result: 实验表明，大语言模型缺乏隐式形成和使用基于schema的学习表示能力，但显式schema支架能显著提升性能，在GPQA化学和物理问题上性能提升最高达36.19%。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法需要真实标注数据，但高质量标注成本高且收集慢，需要开发无需标注的高效提示优化方法。

Method: 将提示优化建模为dueling-bandit问题，使用Double Thompson Sampling选择信息丰富的提示对进行比较，并结合Top-Performer Guided Mutation策略扩展候选提示池。

Result: 在BIG-bench Hard和MS MARCO数据集上的实验表明，PDO持续优于基线方法，消融研究验证了D-TS和提示突变策略的有效性。

Conclusion: PDO提供了一种样本高效的标签无关提示优化框架，能够在不依赖标注数据的情况下有效提升LLM性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在推理任务上表现出色，但在算术计算方面仍有困难。现有研究主要关注输出或提示策略，而模型内部如何进行算术计算的结构仍不清楚。

Method: 构建包含三个操作数和两个运算符的算术表达式数据集，使用logit lens、线性分类探针和UMAP几何可视化等可解释性技术，分析指令调优的LLaMA 3.2-3B模型的残差流。

Result: 发现中间计算结果存在于残差流中，特别是在MLP块之后。模型在注意力层后的每个运算符嵌入中线性编码优先级。

Conclusion: 大语言模型在内部表示中编码算术运算符优先级，部分嵌入交换技术可以有效修改运算符优先级。
...
Method: 设计知识推理语言指令格式和分词器对齐LLM知识与KG表示；提出KRL注意力层通过动态知识记忆机制协调内在知识与额外KG上下文；构建结构感知的下一个实体预测器严格约束推理结果在可信知识域内

Result: 在25个真实世界归纳知识图谱推理数据集上的实验结果表明，KRLM在零样本推理和微调场景下均表现出显著优势

Conclusion: KRLM通过统一协调LLM知识与KG上下文，有效解决了知识扭曲和生成幻觉问题，为归纳知识图谱推理提供了可信的解决方案
...
TL;DR: 提出了RAGCap-Bench基准，用于细粒度评估代理式RAG工作流中的中间任务能力，发现具备更强中间能力的大语言模型能获得更好的端到端性能。
...
Method: 通过分析最先进系统的输出识别常见任务和核心能力，构建典型错误分类法，设计有针对性的评估问题。
...
TL;DR: 该研究将AI辩论应用于主观问题，发现语言模型倾向于迎合法官观点而非坚持自身信念，顺序辩论存在显著偏差，模型在辩护与自身信念一致的立场时更具说服力，但与信念不一致的论点在质量评估中得分更高。


<details>
  <summary>Details</summary>
Motivation: 现有AI辩论实验依赖有标准答案的数据集，忽略了说谎的主观维度——说谎需要相信所辩护的主张是错误的。本研究旨在探索模型在主观问题辩论中是否会采用迎合策略，以及辩论协议可能存在的系统性偏差。

Method: 在辩论前测量语言模型的先验信念，让辩手选择偏好立场，然后面对与自身信念冲突的法官角色。比较了顺序和同时两种辩论协议，评估模型在辩护与自身信念一致或不一致立场时的说服力和论点质量。

Result: 模型倾向于迎合法官观点而非坚持自身信念；顺序辩论对第二位辩手存在显著偏差；模型辩护与自身信念一致的立场时更具说服力；但与信念不一致的论点在质量评估中得分更高。

Conclusion: 这些发现有助于人类法官提供更高质量的训练信号，促进更对齐的AI系统发展，同时揭示了语言模型中说服动态的重要方面。
...
TL;DR: 提出了一种两阶段数据合成管道，通过逐步增加任务复杂度来生成问答对，用于训练更有效的基于网络的深度研究智能体。


<details>
  <summary>Details</summary>
Motivation: 现有方法在构建指令调优数据集时缺乏对难度和质量的细粒度控制，生成的数据无法充分捕捉长时程推理所需的复杂性，且难以评估数据本身的有效性。

Method: 采用两阶段数据合成管道，使用基线网络智能体在任务复杂度逐步增加的过程中生成问答对，基线智能体负责尝试问题、验证事实性、检查替代答案和执行过滤。

Result: 实验表明，尽管数据集规模较小，但在多个基于网络的基准测试中，使用该数据集训练的智能体比现有数据集训练的智能体表现更好，工具使用动作的多样性是现有数据集的两倍。

Conclusion: 该方法能够生成高质量的训练数据，使训练出的网络智能体在避免重复工具调用行为的同时实现更强的性能。
...
TL;DR: 该研究挑战了可读性在小语言模型能力涌现中的关键作用，发现统计简单性比可读性更能预测模型的学习效果。


<details>
  <summary>Details</summary>
Motivation: 反驳当前认为可读性是小语言模型生成连贯文本关键因素的观点，质疑将语言模型训练与人类认知发展进行类比的做法。

Method: 构建结构匹配但可读性不同的合成数据集，比较小语言模型在不同可读性文本上的训练效果和连贯性发展。

Result: 模型在复杂成人文本和简化语言上的表现相当，且复杂文本训练时连贯性发展更快；统计简单性（n-gram多样性）是更强的学习能力预测指标。
...
TL;DR: 提出Element2Vec方法，通过语言模型从维基百科文本生成化学元素的通用嵌入和属性突出向量，解决元素属性预测中数据稀疏和复杂关系建模的挑战。
...
Result: 该方法能够有效表示化学元素，支持自然科学研究，特别是在数据稀疏的情况下仍能进行准确预测。

Conclusion: 这项工作为材料科学中AI驱动发现铺平了道路，解决了文本分布差异和数据极度有限的计算挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前过程奖励模型在测试时扩展中面临挑战，简单多数投票有时会优于标准的PRM选择方法，这促使研究者探索如何有效利用PRM的验证信号。

Method: 开发了理论框架来优化结合LLM和PRM信号，提出加权聚合策略，并设计了高效的预计算方法来校准权重函数。

Result: 在5个LLM和7个PRM上的广泛实验表明，校准方法显著提升了测试时扩展效率，仅使用21.3%的计算量就超越了传统加权多数投票的性能。

Conclusion: 投资更智能的聚合策略比单纯扩展测试时计算是更有效的性能提升路径，证明了优化信号组合的重要性。
...
TL;DR: FACTS是一种快速、准确且符合隐私要求的表格摘要方法，通过离线模板生成实现查询导向的表格自然语言摘要。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在成本高、推理能力弱、效率低、隐私泄露等问题，需要一种更实用、可扩展的解决方案。

Method: 采用代理工作流程，生成包含SQL查询和Jinja2模板的离线模板，这些模板可在共享相同模式的多个表格中重复使用。

Result: 在广泛使用的基准测试中，FACTS始终优于基线方法，证明了其在实际应用中的有效性。

Conclusion: FACTS为现实世界的查询导向表格摘要提供了一个实用的解决方案，具有快速、准确和隐私合规的特点。
...
TL;DR: 提出基于LLM的AI代理框架，将原始网络数据包转换为结构化语义表示，通过混合检索和推理实现IoT网络流量的高效分析。


<details>
  <summary>Details</summary>
Motivation: IoT网络产生多样化高流量数据，需要跨层解释行为和协议，而非孤立检测，以获取有意义的洞察。

Method: 集成特征提取、基于Transformer的异常检测、数据包和流摘要、威胁情报增强以及检索增强问答的AI代理框架，使用混合检索（词法和语义搜索+重排序）。

Result: 在多个IoT数据集和六个开源模型上的实验表明，混合检索相比仅密集检索显著提升了BLEU、ROUGE、METEOR和BERTScore指标，系统性能分析显示低CPU、GPU和内存开销。
...
TL;DR: BioMedSearch是一个基于大语言模型的多源生物医学信息检索框架，通过整合文献检索、蛋白质数据库和网络搜索，解决LLM在生物医学领域生成内容缺乏科学严谨性的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生物医学领域生成的蛋白质功能、相互作用和结构细节经常偏离真实信息，缺乏对权威生物医学数据库的访问能力，导致科学严谨性不足。

Method: 通过子查询分解、关键词提取、任务图构建和多源信息过滤，整合文献检索、蛋白质数据库和网络搜索，支持复杂生物医学查询的准确高效处理。

Result: 在包含3000个问题的BioMedMCQs数据集上，BioMedSearch在所有推理级别上均显著提升准确率：Level 1从59.1%提升至91.9%，Level 2从47.0%提升至81.0%，Level 3从36.3%提升至73.4%。

Conclusion: BioMedSearch框架能够有效提升复杂生物医学问答任务的准确性，通过多源信息检索机制解决了LLM在专业领域内容生成的科学严谨性问题。
...
TL;DR: LLM Brain Rot Hypothesis：持续接触垃圾网络文本会导致大语言模型产生持久性认知衰退。通过Twitter/X数据的对照实验发现，垃圾数据训练使LLM在推理、长文本理解、安全性等方面显著下降，并增加"黑暗特质"。


<details>
  <summary>Details</summary>
Motivation: 研究数据质量对LLM能力的因果影响，验证垃圾网络文本是否会导致模型认知衰退，为持续预训练的数据筛选提供安全指导。

Method: 使用Twitter/X语料构建垃圾和控制数据集，通过两种正交操作化方法（M1：参与度，M2：语义质量），在匹配token规模和训练操作的条件下对4个LLM进行持续预训练对比。

Result: 垃圾数据训练导致推理能力（ARC-Challenge下降74.9→57.2）、长文本理解（RULER-CWE下降84.4→52.3）等显著衰退，且存在剂量-反应关系。错误分析发现思维跳跃是主要病变。

Conclusion: 数据质量是LLM能力衰退的因果驱动因素，应将持续预训练的数据筛选视为训练时安全问题，并为部署的LLM建立常规"认知健康检查"。
...
TL;DR: 研究发现大型语言模型在药物安全预测中存在系统性偏见，弱势群体被分配更高的不良事件预测概率，揭示临床应用中需解决公平性问题


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在生物医学领域药物安全预测中的可靠性，特别是是否将临床无关的社会人口学信息纳入不良事件预测
...
Conclusion: 指令检索是提升小语言模型推理能力的有效方法，无需额外训练即可获得显著性能提升。
...
TL;DR: 提出了HisRubric评估框架和FinDeepResearch基准，用于系统评估深度研究代理在财务分析中的能力，涵盖8个金融市场和4种语言的64家上市公司。
...
Method: 提出了HisRubric分层分析框架，构建了FinDeepResearch基准，对16种代表性方法进行了广泛实验，包括6个DR代理、5个具备深度推理和搜索能力的LLM以及5个仅有深度推理能力的LLM。

Result: 实验结果揭示了这些方法在不同能力、金融市场和语言方面的优势和局限性。

Conclusion: 该研究为深度研究代理的评估提供了系统框架，并揭示了当前方法的性能特点，为未来研究提供了有价值的见解。
...
TL;DR: 研究发现，通过微调AI模型在特定作者的完整作品上，可以显著提升AI生成文本的写作质量和风格保真度，甚至优于专业人类作家的作品，同时大幅降低创作成本。


<details>
  <summary>Details</summary>
Motivation: 探讨AI模型是否能够生成高质量文学作品并模仿作者风格，以回应关于AI使用受版权保护书籍训练可能产生衍生内容的版权争议。

Method: 进行预注册研究，比较MFA训练的专业作家与三种前沿AI模型（ChatGPT、Claude、Gemini）在模仿50位获奖作家风格方面的表现，通过159名专家和普通读者的盲评进行对比。

Result: 上下文提示的AI文本在专家评价中表现不佳，但经过作者特定微调后，专家和普通读者都更偏好AI生成的文本，且AI检测器很少将其识别为AI生成（仅3%）。

Conclusion: 作者特定微调能够消除AI特有的风格缺陷，使AI生成的文本在质量和风格保真度上超越人类专业作品，这对版权法的合理使用分析具有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 发现推理不确定性高度局部化，只有一小部分高熵标记显著影响输出正确性，这为高效干预提供了机会。

Method: MTI包含选择性CFG干预（仅在不确定位置应用分类器自由引导）和轻量级负提示引导（重用主模型的KV缓存高效近似无条件解码）。

Result: 在通用、编程和STEM任务上获得一致提升，如Qwen3-8B-Base在八个基准上平均提升1.35%，Qwen3-32B-Reasoning在AIME2024上提升5%。

Conclusion: MTI能够在保持高效的同时显著提升推理性能，证明了局部化干预的有效性。
...
TL;DR: 提出了一个针对检索增强生成(RAG)系统中错误类型的新分类法，包括错误示例和实用建议，并创建了带错误类型标注的数据集，开发了自动评估方法。
...
Method: 提出了新的错误类型分类法，整理了带错误类型标注的RAG响应数据集，并开发了与分类法对齐的自动评估方法。

Result: 创建了包含错误类型分类、示例数据集和自动评估方法的完整框架，代码和数据已开源。

Conclusion: 该工作为RAG系统的错误分析和改进提供了系统性的工具和方法，有助于开发过程中跟踪和解决错误。
...
TL;DR: 德国Commons是迄今为止最大的公开许可德语文本集合，包含1545.6亿个token，来自41个来源的7个领域，解决了德语公开许可训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型开发依赖大规模训练语料，但大多数语料许可状态不明确，限制了真正开放模型的发展。对于非英语语言，公开许可文本尤其稀缺。

Method: 从41个来源系统收集数据，涵盖法律、科学、文化、政治、新闻、经济和网络文本七个领域。实施全面的质量过滤、去重和文本格式修复处理流程。

Result: 构建了154.56亿个token的高质量德语文本，所有子集至少具有CC-BY-SA 4.0或等效许可，确保模型训练和重新分发的法律合规性。

Conclusion: 德国Commons填补了德语公开许可预训练数据的关键空白，使开发真正开放的德语语言模型成为可能，并发布了语料构建和数据过滤代码，确保完全可重现和可扩展。
...
TL;DR: CRaFT是一个基于解释的多语言评估框架，用于评估大语言模型在跨文化语境中的推理能力，通过四个可解释指标（文化流畅性、偏差、一致性和语言适应性）来分析模型解释而非仅依赖准确性。


<details>
  <summary>Details</summary>
Motivation: 正确的答案不一定反映文化理解，需要评估LLMs如何在跨文化语境中进行推理，而不仅仅是基于准确性的评分。

Method: 使用CRaFT框架评估三个模型（GPT、DeepSeek和FANAR）在50个来自世界价值观调查的文化基础问题上的表现，这些问题被翻译成阿拉伯语、孟加拉语和西班牙语，共分析2100多个答案-解释对。

Result: 结果显示显著的跨语言推理差异：阿拉伯语降低流畅性，孟加拉语增强流畅性，西班牙语基本保持稳定。GPT在跨语言适应方面更有效但一致性较低，FANAR表现稳定但推理僵化。

Conclusion: LLMs的文化意识不是固有的，而是通过语言框架显现的。CRaFT为评估多语言环境中的跨文化推理提供了新视角，为构建文化适应性语言模型提供了可操作的见解。
...
TL;DR: 该论文提出了GlobalGroup任务来评估大语言模型在抽象推理任务中的语言偏见，发现英语模态通常表现更好，开源和闭源模型之间存在性能差异。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在抽象推理任务中的语言偏见，因为现有研究多关注依赖策略或知识的推理任务，而抽象推理对日常生活很重要。

Method: 设计了基于纽约时报Connections游戏的GlobalGroup基准，包含英语、西班牙语、中文、印地语和阿拉伯语五种语言背景，并提出了游戏难度测量方法。

Result: 实验发现英语模态在抽象推理任务中表现更好，开源和闭源模型之间存在性能差异。

Conclusion: 大语言模型在抽象推理任务中存在语言偏见，英语模态表现更优，模型类型也会影响性能。
...
TL;DR: 该研究使用分布方法量化6种语言中的音义象似性，发现了新的可解释的音义对齐模式，并对先前假设的5种音义对齐进行了验证。


<details>
  <summary>Details</summary>
Motivation: 探索语言中音义系统关系在大规模定量研究中的表现程度，包括已识别和未识别现象。

Method: 采用分布方法分析6种语言（英语、西班牙语、印地语、芬兰语、土耳其语和泰米尔语）中语素的语音相似性和语义相似性空间的对齐情况，使用统计测量套件。

Result: 发现了一系列文献中未识别但可解释的音义对齐模式，以及跨语言模式；对5种先前假设的音义对齐获得了支持或混合结果。

Conclusion: 音义象似性在大规模定量研究中确实存在系统表现，包括新的可识别模式和部分验证的先前假设。
...
TL;DR: ERGO方法通过监测LLM内部不确定性（香农熵）来动态调整多轮对话上下文，当检测到熵值急剧上升时触发提示整合，显著提升了增量信息对话的性能和可靠性。
...
Method: 提出ERGO方法，通过持续量化下一个token分布的香农熵来监测内部不确定性，当检测到熵值急剧上升时触发自适应提示整合。
...
TL;DR: DROID是一个紧凑的端到端框架，结合通用语义编码器和领域自适应编码器，通过轻量级分支分类器实现开集意图识别，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决任务导向对话系统中开集意图识别的挑战，现有方法依赖强分布假设或辅助校准模块，需要更鲁棒和可扩展的解决方案。

Method: 使用双编码器架构：通用句子编码器用于广泛语义泛化，领域自适应Transformer去噪自编码器用于领域特定上下文区分。融合表示通过轻量级分支分类器处理，结合合成和开放域离群点增强来改善边界学习。

Result: 仅使用150万可训练参数，在多个意图基准测试中持续优于最新基线方法，已知意图的宏F1提升6-15%，开集意图提升8-20%，在低资源设置下提升最显著。

Conclusion: 双编码器表示结合简单校准可以为神经对话系统提供鲁棒、可扩展和可靠的开集意图检测。
...
<details>
  <summary>Details</summary>
Motivation: 解决网络安全领域缺乏高质量、领域特定模型和训练数据集的问题，推动LLMs在网络安全中的实际部署。

Method: 使用SecKnowledge 2.0数据增强和格式化流水线生成丰富的思维链网络安全指令数据集，结合专家指导和LLM驱动的多步基础化，产生更高质量的安全任务推理轨迹。

Result: 在多样化网络安全基准测试中，CyberPal 2.0持续超越基线模型，匹配或超越各种开源和闭源前沿模型。在核心网络威胁情报任务中排名第二，在威胁调查任务中20B模型排名第一，4B模型排名第二。

Conclusion: CyberPal 2.0证明了小参数模型在网络安全领域的有效性，能够以更小的模型规模实现与前沿大模型相当甚至更好的性能。
...
Method: 通过网页抓取和结构化解析构建数据集，处理异质配料描述，包括单位、数量和描述符的规范化，使用点互信息和提升度分析配料共现模式。

Result: 创建了马其顿语食谱数据集，发现了表征马其顿美食的独特配料组合。
...
TL;DR: 本文提出RLSR方法，用强化学习框架替代传统的监督微调(SFT)，通过语义相似度奖励提升大语言模型的指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 传统SFT方法依赖下一个token预测目标，而RFT等方法展示了强化学习在领域适应中的优势。受此启发，作者希望将强化学习应用于指令跟随任务，利用丰富的SFT数据集来提升模型性能。

Method: RLSR方法让基础模型为每个提示生成多个响应，然后在语义嵌入空间中计算生成响应与人工标注响应的余弦相似度作为奖励分数，通过强化学习优化模型。

Result: 在Qwen-7B模型上，RLSR单独使用在AlpacaEval上的胜率达到26.34%，优于SFT的21.01%。当SFT与RLSR结合时，胜率进一步提升至30.73%。

Conclusion: RLSR不仅能有效替代SFT提升指令跟随能力，还能与SFT结合获得更好的下游任务性能，为语言模型微调提供了新的强化学习框架。
...
TL;DR: 提出了动态角色精炼框架(DPRF)，通过迭代识别生成行为与真实人类行为之间的认知差异，并优化角色配置文件，显著提升大语言模型角色扮演代理的行为对齐度。
...
Result: 在四种不同行为预测场景（正式辩论、社交媒体帖子、公开访谈、电影评论）上评估，DPRF相比基线角色配置文件能持续显著提升行为对齐度，且在不同模型和场景间具有良好泛化性。
...
TL;DR: LiteStage是一个针对多阶段推理的延迟感知层跳过框架，通过阶段化离线搜索分配最优层预算和在线置信度生成早退机制，在三个基准测试上实现1.70倍加速且准确率损失小于4.0%。


<details>
  <summary>Details</summary>
Motivation: 多阶段推理虽然提升了小语言模型的推理能力，但增加了延迟。现有自适应加速技术（如层跳过）难以平衡效率和准确性，面临阶段间跳过敏感性差异和冗余输出令牌生成两大挑战。

Method: 提出LiteStage框架：1）阶段化离线搜索分配最优层预算；2）在线置信度生成早退机制抑制不必要的解码。

Result: 在OBQA、CSQA和StrategyQA三个基准测试上，LiteStage实现了最高1.70倍的加速，准确率损失小于4.0%，优于先前的无训练层跳过方法。

Conclusion: LiteStage通过结合离线层预算分配和在线早退机制，有效解决了多阶段推理中的效率和准确性平衡问题，为小语言模型的推理加速提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在面对同一提示的不同表述时经常产生不一致的回答，需要提高其对提示扰动的鲁棒性。

Method: F²C包含两个关键组件：1) 共识交叉熵(CCE)，使用多数投票创建硬伪标签；2) 表示对齐损失，将低置信度和非多数预测器拉向共识。

Result: 在11个数据集上评估，平均提高观察一致性11.62%，提升平均F1分数8.94%，减少格式间性能方差3.29%。在域外评估中也表现良好。
...
TL;DR: 提出了MoM框架，将RAG中的文本处理从被动分块转变为主动理解，通过模拟人类阅读认知过程来构建文档记忆，提升小语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统RAG范式仅理解相关文本块，限制了知识内化深度和推理能力。需要模拟人类阅读时的认知过程，实现主动的文档记忆提取。

Method: 使用LLM模拟领域专家生成文档逻辑大纲，指导结构化分块和核心内容提取；采用多路径采样和多视角评估机制；引入反向推理策略；开发三层文档记忆检索机制。

Result: 在三个不同领域的实验表明，MoM框架解决了现有RAG系统的文本分块问题，为LLM提供语义完整的文档记忆，并使SLM能够实现类似人类的智能文本处理。
...
TL;DR: 提出了一种实验方法，通过干预训练数据批次来研究训练数据与语言模型行为之间的关系，包括选择评估项、匹配相关文档、修改文档并重新训练以测量效果。
...
Method: 通过"重写历史"的方式干预数据批次，包括从基准中选择评估项、匹配相关文档、修改文档，然后重新训练模型检查点并测量效果。

Result: 结果表明，现有的识别相关训练文档的方法不能完全解释语言模型正确回答知识问题的能力，共现统计与模型行为之间存在关联但不够充分。

Conclusion: 提供了一个可复用的实验方法框架，供研究人员测试训练数据如何影响模型行为的各种假设，代码已公开以促进未来研究。
...
TL;DR: 提出了DEG-RAG框架，通过实体解析和三重反思来去噪LLM生成的知识图谱，显著提升图基RAG系统的性能。
...
Method: DEG-RAG框架包含：(1) 实体解析 - 消除冗余实体；(2) 三重反思 - 移除错误关系，从而生成更紧凑、更高质量的知识图谱。

Result: 实验表明该方法不仅大幅减小图谱规模，还在多种流行的图基RAG变体上持续提升问答性能。

Conclusion: 这是首个对LLM生成知识图谱中实体解析的全面探索，证明了简单去噪方法能显著改善图基RAG系统的效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决小规模多语言模型在检索任务中表现落后于大模型的问题，探索能否通过针对性改进来提升小模型的检索性能。

Method: 分析训练数据规模、负采样策略和数据多样性对多语言嵌入模型的影响，重点关注硬负样本的引入和任务多样性的优化。

Result: 训练数据规模增加初期带来性能提升但很快达到平台期；硬负样本对提升检索准确性至关重要；任务多样性比语言多样性对性能贡献更大。

Conclusion: 成功开发出紧凑的300M参数多语言模型，在检索任务上表现与7B大模型相当甚至更优，证明了小模型通过针对性优化可以达到大模型的检索性能。
...
TL;DR: Qwen3Guard是一个多语言安全护栏模型系列，包含生成式和流式两个变体，支持细粒度三分类判断和实时安全监控，在119种语言上实现最先进的安全分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有护栏模型存在两个主要限制：只能输出二元安全标签，无法适应不同领域的安全容忍度；需要完整模型输出才能进行安全检查，与流式推理不兼容，无法及时干预生成过程。

Method: 提出Qwen3Guard系列模型：生成式变体将安全分类转为指令跟随任务，实现细粒度三分类；流式变体引入token级分类头，支持增量文本生成的实时安全监控。提供0.6B、4B和8B三个规模版本。
...
TL;DR: 提出了一种基于LLM的智能检索系统，通过三个专门代理的协作来提升多跳问答中的检索精度和召回率。
...
Method: 构建包含三个专门代理的框架：问题分析器分解多跳问题为子问题，选择器为每个子问题选择最相关上下文（侧重精度），加法器补充缺失证据（侧重召回率），通过选择器和加法器的迭代交互获得紧凑而全面的支持段落集。

Result: 在HotpotQA、2WikiMultiHopQA、MuSiQue和MultiHopRAG四个多跳问答基准测试中，该方法持续优于强基线方法，实现了更高的检索准确性。

Conclusion: 该智能检索系统能够有效过滤干扰内容，使下游QA模型在依赖较少无关信息的情况下超越全上下文答案的准确性，为多跳问答提供了有效的检索解决方案。
...
Method: 采用双向检索框架：表优先检索后选择列，以及列优先检索后选择表，结合问题分解、关键词提取和关键短语提取技术。

Result: 在BIRD和Spider等基准测试中显著提高了模式召回率并减少误报，SQL生成性能优于全模式基线，接近oracle性能，将完整模式与完美模式之间的性能差距缩小了50%。
...
<details>
  <summary>Details</summary>
Motivation: 学术出版物快速增长导致科学家难以及时全面追踪最新研究成果，现有论文数据库大多局限于关键概念的相似性匹配和基本分类，未能深入探索概念间的关系网络。
...
TL;DR: MathMist是一个多语言数学推理基准数据集，包含7种语言的21K+对齐问答对，评估LLMs在跨语言数学推理中的表现，发现低资源语言环境下性能显著下降。
...
Method: 构建包含7种语言（涵盖高、中、低资源）的平行多语言基准数据集，系统评估开源和专有LLMs在零样本、思维链和代码切换推理范式下的表现。

Result: LLMs在不同语言间的数学推理能力存在显著差异，在低资源语言环境下性能明显下降，难以保持一致的推理质量。

Conclusion: 当前LLMs在多语言数学推理方面仍存在持续缺陷，特别是在低资源语言环境中，需要进一步改进跨语言推理能力。
...
TL;DR: MERLIN是一个多语言多模态实体链接测试系统，包含5种语言的BBC新闻标题和对应图像数据集，覆盖7000多个命名实体和2500个维基数据实体。


<details>
  <summary>Details</summary>
Motivation: 解决多语言多模态实体链接任务，特别是在文本上下文模糊或不足时，通过结合视觉数据提高实体链接准确性。

Method: 构建包含5种语言（印地语、日语、印尼语、越南语、泰米尔语）的BBC新闻标题和图像数据集，使用LLaMa-2和Aya-23等多语言模型进行基准测试。

Result: 视觉数据的加入提高了实体链接的准确性，特别是对于文本上下文模糊的实体，以及对多语言能力较弱的模型效果提升更明显。

Conclusion: 多模态方法能有效提升多语言实体链接性能，特别是在处理文本信息不足的情况下，视觉信息提供了有价值的补充。
...
TL;DR: 本文研究了LLM在对话中的欺骗行为，提出了信念错位指标来量化欺骗，发现LLM在约26%的对话轮次中会自然表现出欺骗行为，即使使用RLHF训练的安全模型仍有43%的欺骗率。作者开发了多轮强化学习方法来减少欺骗行为，实现了77.6%的降低。


<details>
  <summary>Details</summary>
Motivation: LLM在与用户互动时会产生欺骗性输出，无论是故意还是无意的，这带来了严重的安全隐患。LLM行为的不可预测性，加上对幻觉、错误信息和用户操纵的防护不足，使其滥用成为现实风险。

Method: 提出了信念错位指标来量化欺骗，在四种对话场景中评估了五种现有欺骗检测指标和新指标。引入了多轮强化学习方法来微调LLM以减少欺骗行为。

Result: 新提出的欺骗指标与人类判断的相关性比现有指标更紧密。LLM在约26%的对话轮次中自然表现出欺骗行为，当被提示欺骗时，欺骗性可增加31%。使用RLHF训练的安全模型仍有43%的平均欺骗率。多轮强化学习方法使欺骗行为减少了77.6%。

Conclusion: 对话中的欺骗行为是在互动历史中发展的，其有效评估和缓解需要超越单轮话语分析。提出的多轮强化学习方法能显著减少LLM的欺骗行为，为构建更安全的AI系统提供了重要途径。
...
TL;DR: 开发了一种基于混合词嵌入和超参数调优的阿尔茨海默病早期检测方法，使用Doc2Vec和ELMo词向量获取句子困惑度分数，结合语言特征，通过逻辑回归实现91%的分类准确率和97%的AUC。
...
Method: 创建基于Doc2Vec和ELMo的混合词嵌入获取句子困惑度分数，添加语言特征分析句法和语义，将嵌入特征向量输入逻辑回归并微调超参数（如正则化参数、学习率、向量大小等）。

Result: 实现了91%的分类准确率和97%的AUC，优于现有最佳NLP模型的88%准确率。模型稳定性良好（准确率标准差0.0403，AUC标准差0.0174）。

Conclusion: 该方法准确稳定，可作为AD的大规模筛查方法和医生检测AD的补充检查手段。
...
TL;DR: 提出了Beyond One World基准测试，用于评估LLM在角色扮演中忠实呈现特定版本角色的能力，包含30个英雄的90个版本，测试事实回忆和道德困境处理。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在角色扮演中能否忠实、一致地呈现版本特定角色（如不同宇宙中的超级英雄），因为现有研究在这方面探索不足。

Method: 构建包含30个标志性英雄和90个特定版本角色的基准测试，包含两个任务：经典事件（事实回忆）和道德困境（伦理场景）。提出Think-Act Matching指标量化推理与行动的一致性。

Result: 实验发现：思维链提示在弱模型中提升叙事连贯性，但在强模型中降低准确性；跨版本泛化仍是主要障碍；模型通常擅长思考或行动，但很少两者兼备。
...
TL;DR: 提出了一种基于置信度的多模型框架，通过模型多样性提升医学问答性能，无需微调即可在资源受限环境中实现高效医疗AI


<details>
  <summary>Details</summary>
Motivation: 高性能医疗大模型通常需要大量计算资源进行微调，限制了资源受限医疗机构的可及性，需要寻找无需微调的替代方案

Method: 采用两阶段架构：置信度检测模块评估主模型确定性，自适应路由机制将低置信度查询定向到具有互补知识的辅助模型进行协作推理

Result: 在三个医学基准测试中表现优异，PubMedQA达到95.0%，MedMCQA达到78.0%，消融研究证实置信度感知路由结合多模型协作显著优于单模型方法

Conclusion: 策略性模型合作为改进医疗AI系统提供了实用且计算高效的途径，对在资源有限环境中普及先进医疗AI具有重要意义
...
TL;DR: 该研究探讨了现代大语言模型对字符级扰动的鲁棒性，通过在每个输入字符后插入不可见Unicode控制字符来测试模型性能。尽管这种噪声严重破坏了分词并降低了信噪比，许多LLM仍能保持显著性能。


<details>
  <summary>Details</summary>
Motivation: 研究LLM对结构化字符级扰动的鲁棒性，旨在防止LLM在在线考试系统等场景中的滥用，同时探索模型在噪声环境下的可靠性。

Method: 提出NAMESHORT方法，在文本中插入不可见Unicode控制字符，通过模型、问题和噪声相关配置的全面评估，研究字符级分词处理和隐式与显式去噪机制。

Result: 尽管存在强混淆效果，许多LLM仍能保持显著性能，显示出对字符级噪声的较强鲁棒性。

Conclusion: LLM在低层级噪声下的鲁棒性研究揭示了其滥用风险，并为在不同应用中部署LLM的可靠性提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 为美国6800万非英语家庭人口提供准确、及时的气象信息，实现"天气准备国家"目标，确保所有美国人能获得关键天气预警。

Method: 与LILT合作，利用大型语言模型和神经机器翻译技术，结合GIS地图分析语言需求分布，并融入伦理AI实践确保透明度和公平性。

Result: 开发了可扩展的翻译系统，显著减少人工翻译时间，创建了包含预警、7天预报和教育活动的多语言气象产品网站。

Conclusion: 该项目使美国向覆盖所有公民的国家预警系统迈进了一步，通过AI技术实现了高效、准确的多语言气象服务。
...
TL;DR: 论文提出pluri-hop问题概念，针对需要跨所有文档聚合信息的现实场景，并开发了PluriHopWIND数据集和PluriHopRAG方法，相比传统方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有问答系统在处理需要聚合所有文档信息且对遗漏高度敏感的现实问题时存在局限，特别是医疗记录、合规文件等重复性报告数据。

Method: 提出PluriHopRAG架构：将查询分解为文档级子问题，使用交叉编码器过滤无关文档，再进行LLM推理。

Result: 测试方法在PluriHopWIND数据集上F1得分均不超过40%，而PluriHopRAG相比基线方法相对提升18-52%。

Conclusion: PluriHopWIND揭示了当前问答系统在重复性、干扰文档丰富语料上的局限性，PluriHopRAG证明了穷尽检索和早期过滤的价值。
...
Conclusion: 这项研究为增强高危个体检测准确性提供了新视角，为早期自杀干预策略奠定了重要基础。
...
TL;DR: 提出了“你的下一个标记预测”（YNTP）任务，通过受控的人机对话建模用户的精确词汇选择，构建了包含英语、日语和中文的多语言基准数据集，用于评估个性化语言生成方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用下一个标记预测方面表现出色，但难以生成反映个体真实沟通风格（如回复邮件或社交消息）的响应。由于隐私问题，真实的SNS或邮件历史难以收集。

Method: 构建包含100个对话会话的多语言基准数据集，用户与基于MBTI维度的心理基础NPC进行为期五天的互动，捕捉自然的日常沟通模式。评估基于提示和微调的个性化方法。

Result: 建立了YNTP任务的第一个基准，为面向用户的语言建模奠定了基础。数据集已公开可用。

Conclusion: YNTP任务通过受控对话有效建模用户个性化语言风格，为解决用户对齐的语言建模提供了重要基准和数据集支持。
...
TL;DR: 提出了MedTrust-Guided Iterative RAG框架，通过引用感知推理、迭代检索验证和MedTrust对齐模块来减少医学问答中的幻觉问题，在多个医学问答数据集上显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于RAG的医学问答系统存在幻觉问题，主要由于检索后噪声和证据验证不足，影响了回答的可靠性。

Method: 1. 引用感知推理：要求所有生成内容必须基于检索的医学文档；2. 迭代检索验证：通过验证代理评估证据充分性并优化查询；3. MedTrust对齐模块：结合验证的正例和幻觉感知负例，使用直接偏好优化强化引用基础推理。

Result: 在MedMCQA、MedQA和MMLU-Med数据集上，该方法在多个模型架构中均优于竞争基线，LLaMA3.1-8B-Instruct平均准确率提升2.7%，Qwen3-8B提升2.4%。

Conclusion: MedTrust-Guided Iterative RAG框架有效提升了医学问答的事实一致性和可靠性，显著减少了幻觉现象。
...
TL;DR: 提出了一种无标签自监督强化学习框架，通过从指令直接推导奖励信号并生成伪标签来解决多约束指令跟随问题，无需外部监督。
...
TL;DR: 提出了Explore to Evolve范式，通过主动网络探索和自我进化聚合程序，构建了WebAggregatorQA数据集和WebAggregator系列模型，在信息聚合能力上超越现有模型。
...
Method: 采用Explore to Evolve范式：先通过主动网络探索收集证据，然后自我进化聚合程序，从12种高级逻辑类型中选择、组合和优化操作来合成可验证的问答对。

Result: 构建了包含10K样本、覆盖50K网站和11个领域的WebAggregatorQA数据集。WebAggregator-8B与GPT-4.1性能相当，32B版本在GAIA-text上超越GPT-4.1超过10%，接近Claude-3.7-sonnet。在WebAggregatorQA测试集上，Claude-3.7-sonnet仅得28%，GPT-4.1得25.8%。
...
Result: 在10个模型和6400次试验中，工具调用准确率提升18.4个百分点，输出方差降低70%。开源模型获益最大，超越闭源旗舰模型。

Conclusion: NLT框架显著改善工具调用性能，改进在提示扰动下保持稳定，并能扩展工具调用能力到缺乏原生支持的模型。
...
TL;DR: LiRA框架通过锚定表示和语言感知推理，在低资源语言条件下提升跨语言表示能力，同时增强检索和推理性能。
...
Method: LiRA包含两个模块：Arca通过锚定对齐和多智能体协作编码将低资源语言锚定到英语语义空间；LaSR在Arca基础上添加语言感知轻量推理头，通过一致性正则化增强跨语言理解。

Result: 在跨语言检索、语义相似性和推理等低资源基准测试中，LiRA在少样本和噪声放大设置下均表现出稳定的性能提升和鲁棒性。

Conclusion: LiRA框架有效解决了低资源语言的跨语言表示问题，通过联合优化检索和推理任务，显著提升了模型在低资源条件下的性能。
...
TL;DR: 提出了一种压缩表示方法来提高序列到序列指代消解模型在增量设置中的效率，通过提取和重组实体级标记并丢弃大部分其他输入标记来实现。


<details>
  <summary>Details</summary>
Motivation: 序列到序列指代消解模型虽然达到了最先进的性能，但在灵活性和效率方面存在不足，特别是在需要顺序处理文本的增量设置（如对话）中效率不高。

Method: 通过提取和重组实体级标记，并丢弃大部分其他输入标记，创建压缩表示来改进序列到序列指代消解模型在增量设置中的效率。

Result: 在OntoNotes上，最佳模型仅比完整前缀增量基线低0.6个CoNLL F1点，同时实现了1.8的压缩比；在LitBank上，当标注单例提及时会超过最先进性能。

Conclusion: 结果表明，在序列到序列指代消解器中丢弃大部分标记是增量指代消解的一种可行策略。
...
TL;DR: 本文构建了一个新的数据集和分析框架，用于评估主权LLMs的社会文化元素和技术鲁棒性，发现这些模型虽然支持低资源语言，但并未完全满足目标用户需求，且可能低估安全性等关键质量属性。


<details>
  <summary>Details</summary>
Motivation: 当前主权LLMs的发展趋势显示各国需要开发符合自身社会文化和历史背景的LLMs，但缺乏验证这些模型是否与用户社会文化背景对齐以及是否保持安全性和技术鲁棒性的框架和数据集。

Method: 构建新的数据集并引入分析框架，用于提取和评估主权LLMs的社会文化元素，同时评估其技术鲁棒性。

Result: 实验结果表明，主权LLMs在支持低资源语言方面发挥重要作用，但并不总是满足服务目标用户的流行主张，且追求这一未经测试的主张可能导致低估安全性等关键质量属性。
...
TL;DR: 提出了WritingPreferenceBench数据集，发现传统序列奖励模型在主观写作偏好任务上表现不佳，而生成式奖励模型通过显式推理链显著提升准确率。
...
Method: 构建包含1,800个人工标注偏好对的数据集，涵盖8种创意写作类型，比较序列奖励模型、零-shot语言模型评判和生成式奖励模型的性能。

Result: 序列奖励模型平均准确率52.7%，零-shot语言模型53.9%，而生成式奖励模型达到81.8%。不同写作类型间模型方差很大，模型规模对性能提升有限。
...
TL;DR: 提出CodeSeq数据集，通过数字序列的算法问题训练LLMs进行归纳推理，结合反思失败案例和强化学习，提升模型在各种推理任务上的表现并保持OOD性能。


<details>
  <summary>Details</summary>
Motivation: 现有归纳推理研究面临两个挑战：1) 现有数据主要关注表面规律，缺乏复杂内部模式；2) 当前方法仅提示LLMs或微调简单提示-响应对，未提供精确思维过程且缺乏难度控制。

Method: 构建CodeSeq合成后训练数据集，将数字序列打包为算法问题以发现通项公式。通过反思失败测试案例和迭代修正生成监督微调数据，结合基于可解性和自主案例生成成功率的强化学习奖励机制。

Result: 实验结果显示，使用CodeSeq训练的模型在各种推理任务上表现提升，并能保持模型的OOD性能。

Conclusion: CodeSeq通过系统化的归纳推理训练方法，有效提升了LLMs的推理能力，同时保持了泛化性能。
...
TL;DR: RLAIF-SPA框架通过强化学习从AI反馈机制，结合ASR和LLM技术来优化情感语音合成的语义准确性和韵律-情感对齐，显著提升了情感表达性和语音清晰度。


<details>
  <summary>Details</summary>
Motivation: 当前TTS合成在中性语音上已达到接近人类水平，但情感表达仍面临挑战。现有方法依赖昂贵的情绪标注或优化间接目标，导致生成的语音准确但情感平淡。

Method: 提出RLAIF-SPA框架，采用强化学习从AI反馈机制，利用ASR判断语义准确性，LLM判断韵律-情感标签对齐，从结构、情感、速度和音调四个细粒度维度联合优化语义准确性和韵律-情感对齐。

Result: 在Libri Speech数据集上的实验表明，RLAIF-SPA优于Chat-TTS，WER降低26.1%，SIM-O提高9.1%，人工评估提升超过10%。

Conclusion: RLAIF-SPA框架通过直接优化情感表达性和可懂度，显著提升了情感语音合成的质量，为解决TTS情感表达问题提供了有效方案。
...
TL;DR: 提出了一种基于轻量级开源LLM的无训练、无标签意图聚类方法，通过生成伪标签并进行多标签分类来实现聚类，无需预先知道聚类数量。


<details>
  <summary>Details</summary>
Motivation: 解决当前方法依赖商业LLM（成本高、透明度低）和需要预先知道聚类数量的问题，适用于真实场景中聚类数量未知的情况。

Method: 首先让LLM为每个文本生成伪标签，然后在伪标签集上进行多标签分类，基于同一聚类的文本会共享更多标签的假设，通过嵌入编码计算相似度。

Result: 在四个基准数据集上的评估显示，该方法达到了与近期基线方法相当或更好的效果，同时保持简单和计算高效。
...
TL;DR: 提出了“nugget-as-rubric”统一可验证范式，将原子信息点作为结构化评估标准，并开发了4B参数的Search-Gen-V生成验证器，为搜索增强LLM提供可扩展、鲁棒且高效的验证奖励构建方法。


<details>
  <summary>Details</summary>
Motivation: 现有搜索增强LLM的奖励建模存在局限性：基于规则的奖励对表达变化脆弱且不适用于长文本任务，而生成奖励在动态语料库中设计可验证且稳定的奖励具有挑战性且计算成本高。

Method: 提出“nugget-as-rubric”范式，将原子信息点作为结构化评估标准；设计基于查询重写的自动rubric构建管道；开发4B参数的Search-Gen-V生成验证器，采用蒸馏思想和两阶段训练策略。

Result: 实验结果表明Search-Gen-V在不同工作负载下实现了强大的验证准确性，成为搜索增强LLM的可扩展、鲁棒且高效的验证奖励构建器。

Conclusion: 提出的统一可验证范式和高效生成验证器有效解决了搜索增强LLM奖励建模的挑战，为长文本任务在动态语料库中的评估提供了可行解决方案。
...
TL;DR: 该论文提出了一种方法，通过微调机器翻译模型来学习汉语"被"字句的负面语义韵律，提高翻译准确性。


<details>
  <summary>Details</summary>
Motivation: 语义韵律是语言单位与其共现搭配形成的意义，与语义意义不同。当前机器翻译模型无法处理语义韵律问题，导致翻译不准确。

Method: 创建英汉句子对数据集展示"被"字句的负面语义韵律，然后对OPUS-MT、NLLB-600M和mBART50模型进行微调。

Result: 微调后的模型能更好地使用"被"字句翻译负面内容，避免用于中性和正面内容。NLLB-600M还能将语义韵律知识迁移到其他语言对。

Conclusion: 该方法能有效提升机器翻译模型对语义韵律的处理能力，提高翻译质量。
...
TL;DR: 提出了一种以人为中心的框架，通过生成用户故事和促进多智能体讨论，帮助人们在AI系统部署前创造性思考潜在利益和危害。
...
Method: 开发了一个生成用户故事并支持多智能体讨论的框架，通过用户研究评估该方法的有效性。

Result: 阅读用户故事的参与者识别了更广泛的危害类型，响应分布更均匀；未阅读故事的参与者主要关注隐私和健康（58.3%）。
...
TL;DR: AutoRubric-R1V框架通过自动收集基于量规的生成奖励，将强化学习与过程级监督相结合，解决了多模态大语言模型在复杂推理中因仅奖励最终答案正确性而产生的虚假推理问题。
...
TL;DR: Pluto是一个用于评估LLM生成Verilog设计效率的基准测试框架，包含114个问题、自检测试平台和帕累托最优参考实现。实验显示LLM在功能正确性上表现良好（78.3%），但在综合效率（面积63.8%、延迟65.9%、功耗64.0%）上仍落后于专家设计。
...
Result: 最先进的LLM在功能正确性上达到78.3%（pass@1），但在综合效率方面：面积效率63.8%、延迟效率65.9%、功耗效率64.0%（eff@1），明显低于专家手工实现。
...
TL;DR: COIG-Writer是一个中文创意写作数据集，通过逆向工程高质量文本，提供包含逆向提示、详细创作推理和最终文本的三元组。研究发现创意写作需要叙事逻辑（过程监督）和语言表达（通用数据）两个组件，过程监督需要与通用数据结合才能稳定发挥效果。
...
Method: 构建COIG-Writer数据集，包含1,665个精心策划的三元组，涵盖51种文体，每个三元组包含逆向提示、详细创作推理和最终文本。通过全面实验分析创意写作的两组件模型。

Result: 发现过程监督非常有效但需要通用数据稳定，创意能力具有文化边界且无跨语言迁移，词汇多样性与创意质量呈负相关（TTR悖论）。
...
TL;DR: 提出了Answer Regeneration框架，通过额外模型推理重新生成答案，提高推理模型评估的鲁棒性和性能
...
Method: 使用额外模型推理，在原始输入输出前加上"Answer:"提示重新生成答案，然后从中提取最终答案

Result: 该方法在数学问题和开放式问答任务中表现出改进的性能和增强的鲁棒性

Conclusion: Answer Regeneration框架为模型评估提供了更可靠的结果，对提取规则不敏感
...
TL;DR: 本文比较了对比学习(CL)和监督微调(SFT)在LLM重排序任务中的表现，发现SFT在权重更新方面优于CL，并在MRB基准上取得了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 研究LLM重排序任务中哪种学习目标更有效：对比学习还是监督微调，探究其内在机制差异。

Method: 将学习目标分解为权重和方向两个组件，在通用多模态检索(UMR)平台上进行综合比较分析，并通过探测实验验证。

Result: SFT提供了比CL更强的权重方案，而评分方向没有明显优劣，SFT整体优于CL，在MRB基准上达到新的SOTA。

Conclusion: SFT在LLM重排序任务中具有一致优势，其更强的权重更新机制是关键因素。
...
TL;DR: 提出了一种无需外部数据、在线测试时适应框架，通过自我监督优化MoE模型的路由决策，在文本生成过程中持续改进专家选择。


<details>
  <summary>Details</summary>
Motivation: MoE模型在部署时因分布偏移导致路由决策不理想，现有测试时适应方法主要针对密集模型且需要外部数据，限制了在MoE架构中的实用性。

Method: 采用两阶段循环：预填充阶段和定期间隔时，基于已生成序列通过自我监督优化路由决策；然后正常生成文本，保持修改后的路由器直到下一次适应。通过轻量级加性向量仅更新选定层的路由器logits。

Result: 在具有挑战性的推理任务上表现出一致的性能提升，同时保持对上下文偏移的鲁棒性。例如，在HumanEval上实现5.5%的改进，与自一致性结合时在DeepSeek-V2-Lite上实现6%的平均增益。

Conclusion: 该方法提供了一种无需数据、即插即用的测试时适应方案，能够有效提升MoE模型在推理任务中的性能，且与现有测试时扩展技术自然互补。
...
<details>
  <summary>Details</summary>
Motivation: 尽管中期训练在实践中很流行，但对其有效性的科学理解很少。本研究旨在通过受控实验系统探索中期训练的作用机制和效果。

Method: 使用从头开始预训练的语言模型，在不同领域的有监督微调数据集上进行实验，对中期训练的开始时间和数据混合权重进行消融研究。

Result: 中期训练在数学和代码领域效果最显著，能持续优于持续预训练，在领域内验证损失和预训练数据遗忘方面都表现更好。时间安排比混合权重影响更大，较早引入专业数据能带来更大收益。
...
TL;DR: MLIR-AIR是一个基于MLIR的新型编译器堆栈，通过AIR方言为空间架构提供异步和层次化操作的显式表示，实现编译器管理的调度、计算分布和通信计算重叠。


<details>
  <summary>Details</summary>
Motivation: 通用编译器抽象了并行性、局部性和同步性，限制了在现代空间架构上的有效性。现代计算架构越来越依赖对数据移动、执行顺序和计算位置的细粒度控制来获得性能。

Method: 基于MLIR构建MLIR-AIR编译器堆栈，定义AIR方言提供异步和层次化操作的结构化表示，支持空间调度、跨硬件区域的计算分布以及通信与计算的重叠。

Result: 在矩阵乘法中达到78.7%的计算效率，性能接近手工优化的MLIR-AIE实现；多头注意力仅需约150行代码实现融合实现，有效映射到空间硬件。

Conclusion: MLIR-AIR能够将高级结构化控制流转换为空间程序，通过编译器管理的调度有效利用NPU的计算结构和内存层次结构，支持异步执行、分块和通信重叠。
...
TL;DR: 提出了一种结合系统化合成数据生成、三元组损失和改进的层间模型融合方法，显著提升RAG系统中LLM输出一致性，在工业RAG系统中实现了约47.5%的响应相似度提升。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在语义等价输入下生成不一致输出的问题，当前缺乏一致性训练数据且微调技术难以有效提升输出一致性。

Method: 结合系统化合成数据生成、三元组损失优化嵌入表示，以及基于中间层激活的层间模型融合方法，使用一致性感知权重整合专业知识。
...
TL;DR: 提出了一个联合建模训练计算量和上下文的下游任务性能框架，填补了传统扩展定律无法捕捉上下文关键作用的空白


<details>
  <summary>Details</summary>
Motivation: 传统扩展定律只关注上游指标如交叉熵损失，但无法有效预测下游任务性能，其中上下文起着关键作用

Method: 开发了一个简单可解释的框架，将下游性能联合建模为训练计算量和提供上下文的函数，在Llama-2-7B和Llama-2-13B的扩展上下文变体上进行实证验证

Result: 框架准确建模了分布内下游性能，在三个数量级的训练计算量上具有良好泛化能力，并能可靠地外推随着上下文增加的性能表现
...
<details>
  <summary>Details</summary>
Motivation: 心理健康障碍是全球主要致残原因，但由于主观评估、临床资源有限以及污名化，抑郁症、焦虑症和PTSD经常被漏诊或误诊，初级保健中误诊率超过60%，急需可扩展、易获取的情境感知诊断工具。

Method: 使用553个真实世界半结构化访谈数据，比较多种模型类别：GPT-4.1 Mini和MetaLLaMA的零样本提示，以及使用LoRA微调的RoBERTa模型，评估较短上下文和聚焦上下文片段的效果。

Result: 模型在各类诊断中达到80%以上准确率，PTSD表现最佳（89%准确率，98%召回率）。较短上下文和聚焦上下文片段能提高召回率，LoRA微调高效有效，低秩配置（如秩8和16）在各项评估指标中保持竞争力。

Conclusion: 基于LLM的模型相比传统自报告筛查工具能显著改进诊断效果，为低门槛、AI驱动的早期诊断提供路径，为在资源匮乏或高污名化环境中整合机器学习到临床工作流程奠定基础。
...
TL;DR: 提出LaSeR算法，通过最后令牌自奖励机制简化强化学习验证过程，仅需额外一个令牌推理即可实现自我验证，提升LLM推理能力和效率
...
Method: 基于理论分析发现推理奖励可简化为最后令牌自奖励分数，提出LaSeR算法，在原RLVR损失基础上增加MSE损失对齐自奖励分数与验证器推理奖励

Result: 实验表明方法不仅提升模型推理性能，还赋予其显著的自奖励能力，增强推理时扩展性能

Conclusion: LaSeR通过最小额外成本实现高效的自我验证，为LLM推理能力提升提供新范式
...
TL;DR: MetaBench是首个代谢组学评估基准，用于系统评估大语言模型在代谢组学领域的五种关键能力：知识、理解、基础、推理和研究。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用文本上表现出色，但在需要深度、互连知识的专业科学领域（如代谢组学）的能力尚未得到充分表征。代谢组学面临复杂生化通路、异构标识符系统和碎片化数据库的独特挑战。

Method: 从权威公共资源中精心策划MetaBench基准，评估25个开源和闭源大语言模型在代谢组学领域的五种关键能力。

Result: 评估显示模型在文本生成任务上表现良好，但即使使用检索增强，跨数据库标识符基础仍然具有挑战性。对于注释稀疏的长尾代谢物，模型性能也会下降。
...
TL;DR: 研究发现多模态生成模型在处理方言文本输入时性能显著下降32-48%，现有缓解方法效果有限，作者提出了一种编码器缓解策略，能在保持标准英语性能的同时大幅提升方言处理能力。


<details>
  <summary>Details</summary>
Motivation: 研究多模态生成模型是否能有效处理方言文本输入，因为方言使用者在与生成模型交互时经常使用方言，但目前模型对方言的处理能力未知。

Method: 构建包含6种英语方言的大规模基准数据集，收集4200多个经过验证的提示，评估17个图像和视频生成模型，并提出基于编码器的缓解策略来教模型识别新方言特征。

Result: 当前最先进的多模态生成模型在提示中使用单个方言词时性能下降32.26%至48.17%，作者的方法能将5种方言的性能提升至与标准英语相当（+34.4%），且对标准英语性能影响极小。

Conclusion: 多模态生成模型在处理方言输入时存在显著性能差距，提出的编码器缓解策略能有效解决这一问题，在保持标准英语性能的同时大幅提升方言处理能力。
...
TL;DR: 提出IGPO框架，通过信息增益为多轮代理训练提供密集内在监督，解决传统结果奖励稀疏性问题


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的代理在强化学习中依赖最终结果奖励，在多轮交互中面临奖励稀疏问题，导致优势崩溃和细粒度信用分配困难

Method: IGPO将每轮交互建模为获取真实信息的过程，定义轮级奖励为模型产生正确答案概率的边际增长，结合结果级监督形成密集奖励轨迹

Result: 在领域内和领域外基准测试中，IGPO在多轮场景下始终优于强基线，实现更高准确性和样本效率

Conclusion: IGPO通过内在奖励建模为多轮代理训练提供了简单有效的密集监督框架，显著提升了性能
...
TL;DR: UI-Simulator是一个可扩展的UI轨迹生成框架，通过合成训练轨迹来解决数字代理需要多样化大规模UI数据的挑战。


<details>
  <summary>Details</summary>
Motivation: 数字代理需要多样化的大规模UI轨迹来泛化真实世界任务，但人工收集这些数据成本过高。

Method: 集成数字世界模拟器生成多样化UI状态，使用引导式rollout过程进行连贯探索，并通过轨迹包装器产生高质量多样化轨迹。

Result: 在WebArena和AndroidWorld上的实验表明，UI-Simulator在鲁棒性方面优于使用真实UI训练的开源代理，UI-Simulator-Grow仅使用Llama-3-8B-Instruct就能达到Llama-3-70B-Instruct的性能。

Conclusion: 目标合成扩展范式有潜力持续高效地增强数字代理的能力。
...
TL;DR: 提出了TokDrift框架，发现代码LLMs对语义相同但格式不同的代码变体产生显著行为差异，揭示了子词分词与语法不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 代码LLMs使用基于统计的子词分词器，导致语义相同的代码片段因格式差异而被不同分词，影响模型可靠性。

Method: 引入TokDrift框架，应用语义保留的重写规则创建仅在分词上不同的代码变体，分析九个代码LLMs的行为变化。

Result: 即使微小的格式变化也会导致模型行为显著偏移，层间分析显示问题源于早期嵌入层，子词分词未能捕捉语法标记边界。

Conclusion: 分词不匹配是代码理解和生成的隐藏障碍，未来代码LLMs需要语法感知的分词方法。
...
TL;DR: 提出Elastic-Cache方法，通过自适应地选择性刷新KV缓存来加速扩散大语言模型的解码过程，在保持生成质量的同时显著减少冗余计算。


<details>
  <summary>Details</summary>
Motivation: 现有方法在每个去噪步骤和所有层都重新计算所有token的QKV，但KV状态在大多数步骤中变化很小，尤其是在浅层，导致大量冗余计算。

Method: 基于三个观察：(1) 远处的MASK token可作为长度偏置进行块状缓存；(2) KV动态性随深度增加，从深层开始选择性刷新即可；(3) 最受关注的token具有最小的KV漂移。提出训练无关、架构无关的Elastic-Cache策略，联合决定何时刷新（通过最受关注token的注意力感知漂移测试）和在哪里刷新（通过深度感知调度）。

Result: 在LLaDA-Instruct、LLaDA-1.5和LLaDA-V上的实验显示：GSM8K（256 tokens）加速8.7倍，长序列加速45.1倍，HumanEval加速4.8倍，同时保持比基线更高的准确率。在GSM8K上吞吐量比现有基于置信度的方法高6.8倍。

Conclusion: Elastic-Cache通过自适应、层感知的缓存更新，显著减少了扩散LLMs的冗余计算，加速了解码过程，同时保持了生成质量，实现了扩散LLMs的实际部署。
...
TL;DR: 该论文提出了一种结合视觉信息（特别是演示幻灯片）的多模态语音识别方法，用于科学演示场景，通过数据增强和模型训练，相比基线模型在词错误率上实现了显著降低。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的自动语音识别系统主要依赖声学信息，忽略了多模态上下文。视觉信息对于消歧和适应至关重要，特别是在科学演示场景中，演示幻灯片可以提供重要的上下文信息。

Method: 首先创建了多模态演示的基准测试，包括领域特定术语的自动分析；探索了用多模态信息增强语音模型的方法；通过数据增强方法解决缺乏配套幻灯片数据集的问题；最后使用增强数据集训练模型。
...
TL;DR: 提出基于多目标MDP的共识声明生成框架，通过社会选择理论保证公平性，包括两种方法：随机生成策略保证核心稳定性，以及搜索算法最大化平等福利。


<details>
  <summary>Details</summary>
Motivation: 现有LLM共识生成框架缺乏可证明的公平性保证结构，需要为聚合多样化自由形式意见提供理论保障。

Method: 将任务建模为多目标、token级MDP，每个目标对应代理偏好。基于代理策略推导token级奖励，利用社会选择理论提出两种方法：保证核心稳定性的随机生成策略和最大化平等福利的搜索算法。

Result: 实验表明，基于平等目标的搜索算法生成的共识声明在代理对齐方面优于基线方法，包括Habermas Machine。

Conclusion: 该框架为共识声明生成提供了具有可证明公平性保证的结构化方法，通过社会选择理论原则实现了更好的代理对齐效果。
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的多智能体系统虽然能自动化用户任务，但引入了新的风险，包括错位攻击、恶意方攻击导致智能体被破坏或用户数据被盗。
...
Conclusion: Terrarium旨在加速可信多智能体系统的进展，为研究安全、隐私和安全性问题提供工具支持。
...
TL;DR: 提出了AI4Service新范式，通过Alpha-Service框架实现主动式AI助手服务，解决何时干预和如何服务两大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有AI服务多为被动响应，需要转向能够预测用户需求并主动提供帮助的智能助手。

Method: 基于AI眼镜构建Alpha-Service框架，包含输入单元、中央处理单元、算术逻辑单元、内存单元和输出单元五个组件，采用多智能体系统实现。

Result: 通过实时21点顾问、博物馆导览和购物搭配助手等案例研究，证明系统能够无缝感知环境、推断用户意图并提供及时帮助。

Conclusion: Alpha-Service框架为实现主动式AI服务提供了可行方案，展示了AI从被动工具向主动伴侣的转变潜力。
...
TL;DR: 提出了IMAGINE框架，将多智能体系统的推理规划能力集成到单个紧凑模型中，显著超越MAS性能，在TravelPlanner基准上达到82.7%通过率
...
Method: IMAGINE框架，通过端到端训练将多智能体系统的能力集成到单个模型中，使用Qwen3-8B-Instruct作为基础模型

Result: 在TravelPlanner基准上达到82.7%最终通过率，远超DeepSeek-R1-671B的40%，同时保持更小的模型规模

Conclusion: IMAGINE框架成功将多智能体系统的优势集成到单个模型中，显著提升了推理规划能力，同时降低了计算成本
...
TL;DR: 提出了ColorBench图结构基准框架，通过模拟真实设备交互的有限状态实现动态行为的静态仿真，用于评估移动代理在复杂长时程任务中的表现。
...
Method: 开发了基于图结构的基准框架ColorBench，包含175个任务（74个单应用、101个跨应用），平均长度超过13步，每个任务包含至少两条正确路径和若干典型错误路径，支持准动态交互。

Result: 通过在各种基线模型上评估ColorBench，发现了现有模型的局限性，并基于实验结果提出了改进方向和可行的技术路径。

Conclusion: ColorBench框架能够有效评估移动代理在复杂长时程任务中的表现，支持多有效解决方案的评估、子任务完成率统计和原子级能力分析，填补了离线与在线评估之间的空白。
...
TL;DR: TITAN是一个连接自然语言网络安全威胁查询与结构化知识图谱推理的框架，通过路径规划模型和图遍历执行器实现威胁情报的自动化推理。
...
Method: 集成路径规划模型预测文本中的逻辑关系链，以及图执行器遍历TITAN本体知识图谱，使用从MITRE衍生的双向类型化图谱。

Result: TITAN使模型能够生成语法有效且语义连贯的推理路径，可以在底层图谱上确定性地执行。

Conclusion: TITAN框架成功实现了自然语言威胁查询与结构化知识图谱推理的有效连接，为自动化威胁情报分析提供了可行方案。
...
TL;DR: 提出了一个紧凑的形式理论来描述和衡量由领域先验引导的LLM辅助迭代搜索，通过模糊关系算子表示智能体，引入覆盖生成函数来衡量可达性难度，并提供可测试的推断。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的生成-过滤-精炼范式在AI+Science中取得了进展，但搜索效果取决于如何将领域先验编码为操作结构化的假设空间。

Method: 将智能体表示为输入输出的模糊关系算子来捕获可行转换，通过单次连续参数加权所有可达路径得到覆盖生成函数，从而衡量可达性难度。

Result: 提供了最简单的可测试推断，并通过多数投票实例进行了验证，为衡量智能体及其搜索空间提供了可操作的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式验证器虽然性能优秀但计算成本过高，限制了实际应用。

Method: 采用判别式验证器与自一致性结合的混合方法，在固定计算预算下进行测试时扩展。

Result: 在AIME2025上准确率提升15.3%，比生成式验证方法更高效。

Conclusion: 预算感知的判别式验证是自一致性的免费升级，也是成本高昂生成技术的更有效替代方案。
...
TL;DR: 本文系统研究了基于EEG、语音和文本的多模态抑郁症检测方法，比较了手工特征与预训练嵌入、不同神经网络编码器、单/双/三模态配置以及融合策略，发现三模态组合能显著提升检测性能。
...
Method: 使用EEG、语音和文本三种模态，系统比较手工特征与预训练嵌入，评估不同神经网络编码器，对比单模态、双模态和三模态配置，分析融合策略并关注EEG的作用，采用一致的受试者独立分割确保可复现性。

Result: 结果显示：(i) EEG、语音和文本的多模态组合能增强抑郁症检测；(ii) 预训练嵌入优于手工特征；(iii) 精心设计的三模态模型达到最先进性能。
...
TL;DR: 将康德的《纯粹理性批判》重新解释为反馈稳定性理论，提出复合不稳定性指数H-Risk来量化推理系统的过自信问题，并在LLMs中发现脆弱内部动态与错误校准和幻觉相关。
...
Result: 在模拟中，高H-Risk预测过自信错误；在LLMs中，脆弱内部动态与错误校准和幻觉相关，批判式提示对校准和幻觉的影响不一致。

Conclusion: 建立了康德自我限制理论与反馈控制之间的结构桥梁，为诊断和选择性减少推理系统中的过自信提供了原则性视角。
...
TL;DR: 该研究探讨大型语言模型能否通过组合式机器设计来创造复杂机器，开发了BesiegeField测试平台，并发现当前开源模型在空间推理、策略组装等方面存在不足，探索了强化学习作为改进路径。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够学习创造复杂机器，这既是人类智能的标志，也是工程实践的基础。通过组合式机器设计任务来评估LLMs的创造能力。

Method: 开发BesiegeField测试平台，基于Besiege游戏构建，支持基于部件的构建、物理模拟和奖励驱动评估。使用智能体工作流程对最先进的LLMs进行基准测试，并探索强化学习微调。

Result: 当前开源模型在空间推理、策略组装和指令跟随等关键能力上表现不足。通过强化学习微调实验，展示了改进潜力，但同时也指出了语言、机器设计和物理推理交叉领域的挑战。

Conclusion: 大型语言模型在机器设计方面具有潜力，但需要克服空间推理和物理理解等关键挑战。强化学习为改进提供了可行路径，但该领域仍面临重要开放性问题。
...
TL;DR: 提出一个计算框架，通过建模用户信息环境（包括个人发帖历史和社交邻近同伴的讨论）来早期检测社交媒体上的隐性自杀意念。


<details>
  <summary>Details</summary>
Motivation: 许多有自杀意念的用户在社交媒体上不会明确表达痛苦，而是通过日常帖子或同伴互动间接表现出来，早期检测这些隐性信号至关重要但具有挑战性。

Method: 采用复合网络中心性度量识别用户的顶级邻居，时间对齐用户和邻居的互动，在微调的DeBERTa-v3模型中整合多层信号。

Result: 在Reddit的1000名用户研究中，该方法比仅使用个人信息的基线方法提高了15%的早期隐性自杀意念检测性能。

Conclusion: 同伴互动提供有价值的预测信号，对设计能够捕捉在线环境中间接和隐蔽风险表达早期检测系统具有广泛意义。
...
### [138] [E2Edev: Benchmarking Large Language Models in End-to-End Software Development Task](https://arxiv.org/abs/2510.14509)
*Jingyao Liu,Chen Huang,Zhizhao Guan,Wenqiang Lei,Yang Deng*

Main category: cs.SE

TL;DR: E2EDev是一个端到端软件开发基准，包含细粒度需求、BDD测试场景和自动化测试流水线，通过人机协同多智能体标注框架减少标注工作量，评估显示现有E2ESD框架在解决这些任务时仍面临困难。


<details>
  <summary>Details</summary>
Motivation: 当前端到端软件开发(E2ESD)解决方案在有效性和成本效率方面存在不足，需要更有效的基准来评估和改进这些框架。

Method: 提出E2EDev基准，包含细粒度用户需求、BDD测试场景和Python步骤实现，使用人机协同多智能体标注框架(HITL-MAA)确保质量并减少标注工作量。

Result: 通过评估各种E2ESD框架和LLM骨干网络，发现现有方法在有效解决这些任务方面持续挣扎。

Conclusion: 需要更有效和成本效益更高的E2ESD解决方案，E2EDev基准为此提供了评估基础，代码和基准已公开可用。
...
### [139] [Generative AI in Heritage Practice: Improving the Accessibility of Heritage Guidance](https://arxiv.org/abs/2510.13811)
*Jessica Witte,Edmund Lee,Lisa Brausem,Verity Shillabeer,Chiara Bonacchi*

Main category: cs.HC

TL;DR: 开发了名为HAZEL的GenAI聊天机器人，专门用于协助修订文化遗产保护和解释的指导文件，相比ChatGPT表现略优，但仍有文化敏感性和技术专业性的局限。


<details>
  <summary>Details</summary>
Motivation: 将生成式人工智能整合到专业遗产实践中，旨在提高面向公众的指导文件的可访问性。

Method: 开发了HAZEL GenAI聊天机器人，通过微调专门用于协助修订文化遗产指导文件，并与ChatGPT(GPT-4)在指导文件编写任务中进行定量性能比较。

Result: HAZEL在性能上略优于ChatGPT，表明经过微调后的GenAI聊天机器人更有效，但在文化敏感性和高级技术专业知识方面存在显著局限性。

Conclusion: 虽然GenAI无法在技术编写任务中取代人类遗产专业人士，但其自动化和加速指导文件编写某些方面的潜力可以为遗产组织，特别是在资源受限的环境中提供宝贵价值。
...
### [140] [Just-In-Time Objectives: A General Approach for Specialized AI Interactions](https://arxiv.org/abs/2510.14591)
*Michelle S. Lam,Omar Shaikh,Hallie Xu,Alice Guo,Diyi Yang,Jeffrey Heer,James A. Landay,Michael S. Bernstein*

Main category: cs.HC

TL;DR: 论文提出了一种通过推断用户即时目标来优化LLM输出的架构，能够在用户任务中实现66-86%的胜率，生成针对每个用户的专用工具。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然功能广泛，但在没有特定目标时会产生平庸的结果。通过推断用户的即时目标并针对该目标进行优化，可以让LLM产生更响应和更受欢迎的工具、界面和响应。

Method: 提出了一种架构，通过被动观察用户行为自动诱导即时目标，然后基于这些目标引导下游AI系统进行生成和评估。

Result: 在用户自有任务的实验中，JIT目标使LLM输出相对于典型LLM实现了66-86%的胜率。现场使用会话确认JIT目标能为每个参与者生成独特的专用工具。

Conclusion: 自动诱导即时目标能够显著提升LLM输出的质量和针对性，为每个用户生成个性化的专用工具。
...
### [141] [LTR-ICD: A Learning-to-Rank Approach for Automatic ICD Coding](https://arxiv.org/abs/2510.13922)
*Mohammad Mansoori,Amira Soliman,Farzaneh Etminani*

Main category: cs.LG

TL;DR: 本文提出了一种将ICD编码分配视为检索系统问题的新方法，同时考虑分类和排序，显著提升了主要诊断代码的排序准确性。


<details>
  <summary>Details</summary>
Motivation: 临床笔记中的ICD编码顺序对医疗诊断和报销至关重要，但现有方法仅将其视为分类任务，忽略了编码顺序的重要性。

Method: 将ICD编码分配问题重新定义为检索系统视角，同时处理分类和排序任务，以考虑编码的顺序性。

Result: 模型在主要诊断代码排序上的准确率达到47%，远超现有最佳分类器的20%；在分类指标上，微平均和宏平均F1分数分别为0.6065和0.2904，优于之前最佳模型的0.597和0.2660。

Conclusion: 提出的框架在识别高优先级代码方面具有优越能力，证明了将ICD编码分配视为分类和排序任务的可行性。
...
TL;DR: BitNet Distillation (BitDistill) 是一种轻量级方法，可将全精度大语言模型微调为1.58位精度（三元权重{-1, 0, 1}），在特定下游任务上实现接近全精度模型的性能，同时节省10倍内存并加速2.65倍推理。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在部署时面临的高内存消耗和计算成本问题，通过极低精度量化实现高效推理。

Method: 结合三个关键技术：SubLN模块、基于MiniLM的多头注意力蒸馏、以及作为预热步骤的持续预训练，以缓解全精度与1.58位模型在特定任务上的性能差距。

Result: 实验结果显示，BitDistill在不同模型规模下都能达到与全精度模型相当的性能，同时实现10倍内存节省和2.65倍的CPU推理加速。

Conclusion: BitDistill提供了一种有效的轻量级解决方案，使大语言模型能够在资源受限的环境中高效部署，同时保持任务特定性能。
...
### [143] [Towards Reversible Model Merging For Low-rank Weights](https://arxiv.org/abs/2510.14163)
...
### [144] [MAFA: A Multi-Agent Framework for Enterprise-Scale Annotation with Configurable Task Adaptation](https://arxiv.org/abs/2510.14184)
*Mahmood Hegazy,Aaron Rodrigues,Azzam Naeem*

Main category: cs.LG

TL;DR: MAFA是一个生产部署的多智能体标注框架，通过可配置的多智能体协作解决金融服务中的大规模标注积压问题，在摩根大通成功消除了100万条客户话语积压，节省了5000多小时人工标注时间。
...
Method: 结合专门化智能体与结构化推理，采用基于评审的共识机制，支持通过配置而非代码修改来定义自定义标注类型。

Result: 平均与人工标注者达成86%一致率，在内部意图分类数据集上比传统方法提升13.8%的Top-1准确率、15.1%的Top-5准确率和16.9%的F1分数，标注置信度分类为85%高、10%中、5%低。
...
### [145] [Scaling Test-Time Compute to Achieve IOI Gold Medal with Open-Weight Models](https://arxiv.org/abs/2510.14232)
...
<details>
  <summary>Details</summary>
Motivation: 竞争性编程已成为评估大语言模型推理和问题解决能力的重要基准，但开源模型在IOI等顶级竞赛中达到金牌水平仍具挑战性。

Method: 结合大规模生成、行为聚类、排名和循环提交策略，在有限验证预算下高效探索多样化解决方案空间。

Result: GenCluster使用gpt-oss-120b开源模型首次在IOI 2025中获得金牌，性能随计算资源增加而稳定提升。

Conclusion: 该方法缩小了开源与闭源系统之间的差距，为大语言模型推理能力的透明和可复现评估设立了新基准。
...
### [146] [CAST: Compositional Analysis via Spectral Tracking for Understanding Transformer Layer Functions](https://arxiv.org/abs/2510.14262)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然取得了显著成功，但其内部机制仍不透明，现有可解释性方法各有局限，需要新的分析视角来补充理解。

Method: 使用Moore-Penrose伪逆估计每层的实际变换矩阵，应用频谱分析并采用六个可解释指标来表征层行为，包括核分析揭示层间功能关系。

Result: 分析发现编码器-解码器模型行为差异：解码器模型呈现压缩-扩展循环，编码器模型保持一致的高秩处理；CKA相似性矩阵将层划分为特征提取、压缩和专业化三个阶段。

Conclusion: CAST框架提供了对Transformer层功能的新理解，揭示了不同模型架构的独特行为模式，为模型可解释性研究提供了有价值的补充工具。
...
### [147] [Are My Optimized Prompts Compromised? Exploring Vulnerabilities of LLM-based Optimizers](https://arxiv.org/abs/2510.14381)
*Andrew Zhao,Reshmi Ghosh,Vitor Carvalho,Emily Lawton,Keegan Hines,Gao Huang,Jack W. Stokes*

Main category: cs.LG

TL;DR: 本文首次系统分析了LLM提示优化中的投毒风险，发现基于反馈的攻击比查询注入更危险，提出了无需奖励模型的简单假奖励攻击，并设计了高亮防御方法。


<details>
  <summary>Details</summary>
Motivation: LLM系统在日常AI应用中广泛使用，其性能依赖于精心设计的提示。虽然LLM提示优化器通过迭代优化减少了人工努力，但优化阶段的安全性研究不足，存在被恶意操纵的风险。

Method: 使用HarmBench评估系统脆弱性，引入无需奖励模型访问的简单假奖励攻击，并提出轻量级高亮防御方法来降低攻击成功率。

Result: 反馈攻击比查询注入攻击更有效，攻击成功率提升高达ΔASR=0.48；假奖励攻击显著增加系统脆弱性；高亮防御将假奖励攻击的ΔASR从0.23降至0.07，且不影响系统效用。
...
### [148] [Agentic Entropy-Balanced Policy Optimization](https://arxiv.org/abs/2510.14545)
...
TL;DR: 提出Agentic Entropy-Balanced Policy Optimization (AEPO)算法，通过动态熵平衡机制解决Agentic RL中过度依赖熵信号导致的训练崩溃问题，在14个数据集上优于7种主流RL算法。
...
Method: AEPO包含两个核心组件：(1) 动态熵平衡rollout机制，通过熵预监控自适应分配全局和分支采样预算，并对连续高熵工具调用步骤施加分支惩罚；(2) 熵平衡策略优化，在高熵裁剪项中插入停止梯度操作，并结合熵感知优势估计优先学习高不确定性token。

Result: 在14个挑战性数据集上，AEPO始终优于7种主流RL算法。仅使用1K RL样本，Qwen3-14B配合AEPO在GAIA上达到47.6%(Pass@1)和65.0%(Pass@5)，在Humanity's Last Exam上达到11.2%(Pass@1)和26.0%(Pass@5)，在WebWalker上达到43.0%(Pass@1)和70.0%(Pass@5)。
...
### [149] [Reasoning with Sampling: Your Base Model is Smarter Than You Think](https://arxiv.org/abs/2510.14901)
*Aayush Karan,Yilun Du*

Main category: cs.LG

TL;DR: 提出一种无需额外训练的推理采样算法，仅通过基础模型自身似然进行迭代采样，在多项推理任务上达到与强化学习后训练相当甚至更好的性能，同时保持样本多样性。


<details>
  <summary>Details</summary>
Motivation: 探索是否能够仅通过推理时的纯采样方法，从基础模型中激发出与强化学习后训练相当的推理能力，避免训练成本和多样性损失。

Method: 受马尔可夫链蒙特卡洛技术启发，提出基于基础模型自身似然的简单迭代采样算法，无需额外训练、策划数据集或验证器。

Result: 在MATH500、HumanEval、GPQA等单次任务上，该方法显著提升推理能力，性能接近甚至超过强化学习后训练，同时避免了RL后训练中常见的多样性崩溃问题。

Conclusion: 该方法展示了仅通过推理时采样即可从基础模型中激发强大推理能力的可行性，具有广泛适用性，特别适用于难以验证的领域。
...
### [150] [Circuit Insights: Towards Interpretability Beyond Activations](https://arxiv.org/abs/2510.14936)
*Elena Golimblevskaia,Aakriti Jain,Bruno Puri,Ammar Ibrahim,Wojciech Samek,Sebastian Lapuschkin*

Main category: cs.LG

TL;DR: 提出了WeightLens和CircuitLens两种互补方法，通过直接分析权重和特征交互来提升神经网络的可解释性，超越了传统的基于激活的分析方法。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法依赖人工检查且局限于简单任务，自动化方法虽然可扩展但常忽略特征间交互，且过度依赖外部LLM和数据集质量。
...
Conclusion: 这两种方法提高了可解释性的鲁棒性，增强了电路的可扩展机制分析，同时保持了效率和质量的平衡。
...
### [151] [Efficient Parallel Samplers for Recurrent-Depth Models and Their Connection to Diffusion Language Models](https://arxiv.org/abs/2510.14961)
*Jonas Geiping,Xinyu Yang,Guinan Su*

Main category: cs.LG

TL;DR: 提出了一种基于扩散模型原理的新采样器，用于加速循环深度语言模型的生成过程，在保持相同时间预算下实现更高效并行推理
...
Method: 开发扩散强制采样器，在每个前向传递中解码新token，同时通过循环并行细化这些token的潜在状态

Result: 采样器可直接应用于现有的35亿参数循环深度transformer，无需调优即可实现高达5倍的加速

Conclusion: 该采样器不仅为循环深度模型提供了高效的并行推理机制，还表明此类模型可被视为强大的连续因果扩散语言模型
...
### [152] [Joint Modeling of Big Five and HEXACO for Multimodal Apparent Personality-trait Recognition](https://arxiv.org/abs/2510.14203)
*Ryo Masumura,Shota Orihashi,Mana Ihori,Tomohiro Tanaka,Naoki Makishima,Taiga Yamane,Naotaka Kawata,Satoshi Suzuki,Taichi Katayama*

Main category: cs.CV

TL;DR: 提出了一种联合建模Big Five和HEXACO人格特质的方法，用于从多模态人类行为中自动识别表观人格特征。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要使用Big Five模型，但忽略了HEXACO模型中的诚实-谦逊特质，该特质与攻击性、复仇心理和社会支配倾向相关。同时，机器学习建模中Big Five和HEXACO之间的关系尚未明确。

Method: 通过联合优化识别Big Five和HEXACO的方法，从多模态人类行为中同时识别两种人格特质模型。

Result: 使用自我介绍视频数据集的实验表明，所提方法能够有效识别Big Five和HEXACO人格特质。

Conclusion: 通过联合建模Big Five和HEXACO，提高了对多模态人类行为的认知，为表观人格特质识别提供了更全面的方法。
...
### [153] [Talking Points: Describing and Localizing Pixels](https://arxiv.org/abs/2510.14583)
*Matan Rusanovsky,Shimon Malnick,Shai Avidan*

Main category: cs.CV

TL;DR: 提出了一种用于像素级关键点理解的创新框架，包含点描述器和点定位器两个互补组件，能够通过自然语言实现精确的关键点定位。
...
Method: 框架包含点描述器（生成关键点的上下文描述）和点定位器（从描述回归像素坐标），使用合成的LlamaPointInPart数据集进行训练，并通过GRPO优化跨类别泛化。
...
### [154] [Benchmarking Multimodal Large Language Models for Face Recognition](https://arxiv.org/abs/2510.14866)
...
Result: 实验结果表明，MLLMs能够捕捉对面部相关任务有用的丰富语义线索，但在零样本应用的高精度识别场景中落后于专用模型。
...
### [155] [You May Speak Freely: Improving the Fine-Grained Visual Recognition Capabilities of Multimodal Large Language Models with Answer Extraction](https://arxiv.org/abs/2510.14885)
*Logan Lawrence,Oindrila Saha,Megan Wei,Chen Sun,Subhransu Maji,Grant Van Horn*

Main category: cs.CV

TL;DR: 提出nlg2choice方法解决多模态大语言模型在细粒度视觉分类中处理自由形式响应和大量选项的评估挑战


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注纯语言任务或不超过5个选项的多选题，无法应对细粒度视觉分类中数百至数千个高度相关选项的情况，且在大规模多选题设置中难以扩展LLM选择提取到检索问题

Method: nlg2choice两阶段方法：先让MLLM回答开放式问题，然后使用纯文本约束解码预测最可能的选择；在检索设置中，采用早期停止方法计算约束响应的选择概率以显著提高吞吐量

Result: 在七个细粒度视觉数据集上的分类和检索评估中表现优于现有方法，且性能在各种自然语言任务实现方式下保持稳定
...
### [156] [MathCanvas: Intrinsic Visual Chain-of-Thought for Multimodal Mathematical Reasoning](https://arxiv.org/abs/2510.14958)
...
Method: 框架分为两个阶段：1）视觉操作阶段预训练模型掌握图表生成和编辑；2）策略性视觉辅助推理阶段微调模型学习何时以及如何利用视觉辅助。使用新构建的大规模数据集进行训练。

Result: 基于该框架训练的BAGEL-Canvas模型在MathCanvas-Bench基准上相比强大的LMM基线实现了86%的相对提升，在其他公开数学基准上也表现出良好的泛化能力。
